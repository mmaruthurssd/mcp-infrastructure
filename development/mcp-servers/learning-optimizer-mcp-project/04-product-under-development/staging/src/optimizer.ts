...
import { TrackedIssue, DomainConfig, OptimizationTrigger, OptimizationResult } from './types.js';
import { IssueTracker } from './issue-tracker.js';
import { Categorizer } from './categorizer.js';
import { DuplicateDetector, DuplicateGroup } from './duplicate-detector.js';
...
export class OptimizerEngine {
  ...
  async checkOptimizationTriggers(
    ...
    const highImpactIssues = issues.filter(
      i => i.frequency >= config.optimizationTriggers.highImpactThreshold && !i.promoted
    );
  ...
  async optimize(
    ...
    const kb = await this.issueTracker.parseKnowledgeBase(
      `${this.issueTracker['projectRoot']}/${knowledgeBaseFile}`
    );
    ...
    const highImpactIssues = issues.filter(
      i =>
        i.frequency >= config.optimizationTriggers.highImpactThreshold &&
        !i.promoted &&
        !i.promotionPending &&
    ...
    await this.issueTracker.saveKnowledgeBase(
      `${this.issueTracker['projectRoot']}/${knowledgeBaseFile}`,
      kb
    );
   ...
   */
  private estimatePreventionRate(issue: TrackedIssue): string {
    // Higher frequency = more consistent pattern = higher prevention rate
    if (issue.frequency >= 5) return '~95-100%';
    if (issue.frequency >= 3) return '~85-95%';
    return '~70-85%';
  }
}
