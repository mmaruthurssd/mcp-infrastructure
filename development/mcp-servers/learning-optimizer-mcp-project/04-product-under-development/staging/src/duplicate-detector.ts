...
export class DuplicateDetector {
  ...
  detectDuplicates(issues: TrackedIssue[]): DuplicateGroup[] {
    ...
    for (let i = 0; i < issues.length; i++) {
      if (processed.has(issues[i].issueNumber)) continue;
      if (issues[i].mergedInto) continue; // Skip already merged issues

      const primaryIssue = issues[i];
      const duplicates: TrackedIssue[] = [];

      for (let j = i + 1; j < issues.length; j++) {
        if (processed.has(issues[j].issueNumber)) continue;
        if (issues[j].mergedInto) continue;
   ...
   */
  private areDuplicates(issue1: TrackedIssue, issue2: TrackedIssue): boolean {
   ...
   */
  private extractErrorPattern(text: string): string | null {
