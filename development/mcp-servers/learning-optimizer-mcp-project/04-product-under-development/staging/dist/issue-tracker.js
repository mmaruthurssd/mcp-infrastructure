...
export class IssueTracker {
     ...
     */
    async trackIssue(knowledgeBaseFile, issue) {
        ...
        if (existingIssue) {
            ...
            existingIssue.temporalDistribution.spanDays = Math.floor((lastDate.getTime() - firstDate.getTime()) / (1000 * 60 * 60 * 24));
     ...
     */
    findSimilarIssue(issues, symptom) {
        ...
        return issues.find(issue => {
            ...
            // Exact match
            if (normalizedIssueSymptom === normalizedSymptom)
                return true;
            // Contains match (for error messages)
            if (normalizedSymptom.length > 30 && normalizedIssueSymptom.includes(normalizedSymptom))
                return true;
            if (normalizedSymptom.length > 30 && normalizedSymptom.includes(normalizedIssueSymptom))
                return true;
            return false;
        });
    }
     ...
     */
    async parseKnowledgeBase(filePath) {
        try {
     ...
     */
    async saveKnowledgeBase(filePath, kb) {
        let content = '';
        // Write header
    ...
    formatIssue(issue) {
        ...
        if (issue.promoted)
            md += ' (PROMOTED)';
        if (issue.mergedInto)
            md += ` [MERGED INTO #${issue.mergedInto}]`;
        md += `\n\n`;
     ...
     */
    async getIssues(knowledgeBaseFile) {
        const filePath = `${this.projectRoot}/${knowledgeBaseFile}`;
        const kb = await this.parseKnowledgeBase(filePath);
        return kb.issues;
    }
