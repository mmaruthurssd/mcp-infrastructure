...
import type {
  ...
  Task,
  ProjectOverview,
} from '../types/hierarchical-entities.js';
 ...
 */
export function calculateAverageProgress(childProgressList: number[]): number {
  if (childProgressList.length === 0) return 0;
 ...
 */
export function aggregateStatus(childStatusList: ProgressStatus[]): ProgressStatus {
  if (childStatusList.length === 0) return 'not-started';

  // Check for blocking conditions (highest priority)
  if (childStatusList.some(status => status === 'blocked')) return 'blocked';

  // Check if all completed
  if (childStatusList.every(status => status === 'completed')) return 'completed';

  // Check if all not-started
  if (childStatusList.every(status => status === 'not-started')) return 'not-started';

  // Check for in-progress
  if (childStatusList.some(status => status === 'in-progress')) return 'in-progress';

  // Check for planning
  if (childStatusList.some(status => status === 'planning')) return 'planning';

  // Check for on-hold
  if (childStatusList.some(status => status === 'on-hold')) return 'on-hold';
...
export function buildProgressBreakdown(
  children: Array<{ id: string; name: string; progress?: ProgressInfo }>
): ProgressBreakdown {
...
export function calculateProjectProgress(
  components: Array<{ id: string; name: string; progress: ProgressInfo }>
): ProgressAggregationResult {
 ...
 */
export function calculateComponentProgress(
  majorGoals: Array<{ id: string; name: string; progress: ProgressInfo }>
): ProgressAggregationResult {
 ...
 */
export function calculateMajorGoalProgress(
  subGoals: Array<{ id: string; name: string; progress?: ProgressInfo }>
): ProgressAggregationResult {
...
export function calculateSubGoalProgress(
  taskWorkflows: Array<{ workflowId: string; workflowName: string; progress?: ProgressInfo }>
): ProgressAggregationResult {
 ...
 */
export function calculateTaskWorkflowProgress(tasks: Task[]): ProgressAggregationResult {
  if (tasks.length === 0) {
    return {
      percentage: 0,
      completedItems: 0,
 ...
 */
export function createProgressInfo(result: ProgressAggregationResult): ProgressInfo {
  return {
 ...
 */
export function calculateFullHierarchyProgress(
  ...
  // Calculate sub-goal progress from task workflows
  const subGoalProgressMap: Record<string, ProgressAggregationResult> = {};
  ...
  // Calculate major goal progress from sub-goals
  const goalProgressMap: Record<string, ProgressAggregationResult> = {};
  ...
  const componentProgressMap: Record<string, ProgressAggregationResult> = {};
 ...
 */
export function getParentReference(
  context: NavigationContext
): { id: string; type: 'project' | 'component' | 'sub-area' | 'major-goal' | 'sub-goal' | 'task-workflow' } | null {
  // Task → Task Workflow
 ...
 * // ]
 */
export function getParentChain(context: NavigationContext): Array<{
  id: string;
 ...
 */
export function calculateEntityDepth(context: NavigationContext): number {
  let depth = 0;

  if (context.projectId) depth++;
  if (context.componentId) depth++;
  if (context.subAreaId) depth++;
  if (context.majorGoalId) depth++;
  if (context.subGoalId) depth++;
  if (context.workflowId) depth++;
  if (context.taskId) depth++;
...
export function isAncestor(
  ancestorContext: NavigationContext,
  descendantContext: NavigationContext
): boolean {
  // Must be in same project
  if (ancestorContext.projectId !== descendantContext.projectId) return false;

  // Check component match
  if (ancestorContext.componentId) {
    if (ancestorContext.componentId !== descendantContext.componentId) return false;
  }

  // Check sub-area match
  if (ancestorContext.subAreaId) {
    if (ancestorContext.subAreaId !== descendantContext.subAreaId) return false;
  }

  // Check major goal match
  if (ancestorContext.majorGoalId) {
    if (ancestorContext.majorGoalId !== descendantContext.majorGoalId) return false;
  }

  // Check sub-goal match
  if (ancestorContext.subGoalId) {
    if (ancestorContext.subGoalId !== descendantContext.subGoalId) return false;
  }

  // Check workflow match
  if (ancestorContext.workflowId) {
    if (ancestorContext.workflowId !== descendantContext.workflowId) return false;
  }
...
export function findCommonAncestor(
  context1: NavigationContext,
  context2: NavigationContext
): NavigationContext | null {
  // Must be in same project
  if (context1.projectId !== context2.projectId) return null;

  const commonAncestor: NavigationContext = {
...
// ============================================================================
// CASCADE UPDATES & VERSION CONTROL
// ============================================================================
...
export interface ChangeImpact {
  ...
  impactedChildren: string[]; // IDs of directly impacted children
  requiresReview: boolean; // Does this change need human review?
  autoUpdateable: boolean; // Can this be automatically updated?
}
 ...
 */
export function analyzeProjectOverviewImpact(
  oldVersion: Partial<ProjectOverview>,
  newVersion: Partial<ProjectOverview>,
  components: Component[]
): ChangeImpact[] {
  ...
  if (
    oldVersion.constraints?.timeline?.targetEndDate !==
 ...
 */
export function analyzeComponentImpact(
  oldVersion: Partial<Component>,
  newVersion: Partial<Component>,
  majorGoals: MajorGoal[]
): ChangeImpact[] {
  const impacts: ChangeImpact[] = [];
 ...
 */
export function createCascadeNotification(impact: ChangeImpact): string {
 ...
 */
export function executeCascadeUpdate(
    ...
    if (impact.autoUpdateable && !dryRun) {
      try {
        ...
        result.warnings.push(
          `Failed to update children of ${impact.entityId}: ${error}`
        );
      }
    } else if (impact.requiresReview) {
      // Changes requiring review get a warning
      result.warnings.push(
        `${impact.entityType} ${impact.entityId} change requires manual review of ${impact.impactedChildren.length} children`
      );
    }

    if (dryRun) {
      result.warnings.push(
        `[DRY RUN] Would update: ${impact.impactedChildren.join(', ')}`
      );
    }
  }
 ...
 */
export function validateCascadeUpdate(impacts: ChangeImpact[]): {
  safe: boolean;
  warnings: string[];
  blockers: string[];
} {
  ...
  for (const impact of impacts) {
    ...
    if (impact.changeType === 'structure') {
      blockers.push(
        `Structural change to ${impact.entityId} may break existing references`
      );
    }
    ...
    if (!impact.autoUpdateable && impact.impactedChildren.length > 5) {
      warnings.push(
        `${impact.impactedChildren.length} entities require manual review - this may take significant time`
      );
    }
    ...
    if (impact.changeType === 'constraints') {
      warnings.push(
        `Constraint changes may create circular dependencies - verify affected entities`
      );
    }
  }
 ...
 * @returns Validation result
 */
export function validateProgress(progress: ProgressInfo): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  if (progress.completedItems > progress.totalItems) {
    errors.push(
      `Completed items (${progress.completedItems}) exceeds total (${progress.totalItems})`
    );
  }
  ...
  if (progress.percentage === 100 && progress.status !== 'completed') {
    warnings.push(
      `Progress is 100% but status is "${progress.status}" (should be "completed")`
    );
  }
  ...
  if (
    ...
    warnings.push(
      `Progress is ${progress.percentage}% but status is "not-started"`
    );
  }
  ...
  if (progress.breakdown) {
    ...
    if (breakdownCount !== progress.totalItems) {
      warnings.push(
        `Breakdown has ${breakdownCount} items but totalItems is ${progress.totalItems}`
      );
    }
  }
...
export function detectOrphanedEntities(
  ...
  const orphaned: NavigationContext[] = [];
