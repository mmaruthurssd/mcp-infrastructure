...
export class RoadmapBuilder {
   ...
   */
  private static sortGoalsByDependency(goals: GoalSummary[]): GoalSummary[] {
    const sorted: GoalSummary[] = [];
    ...
    while (remaining.length > 0) {
      ...
      for (let i = remaining.length - 1; i >= 0; i--) {
        ...
        const dependenciesMet = !goal.dependencies || goal.dependencies.every(depId =>
          sorted.some(g => g.id === depId)
        );
   ...
   */
  private static groupGoalsByPhase(goals: GoalSummary[]): Phase[] {
    const phases: Phase[] = [];
    const tierPhaseMap: Record<string, string[]> = {
    ...
    // Group by tier
    const byTier: Record<string, GoalSummary[]> = {
      'Now': [],
      'Next': [],
      'Later': [],
      'Someday': [],
   ...
   */
  private static createMilestones(phase: Phase, allGoals: GoalSummary[]): Milestone[] {
   ...
   */
  private static generateMilestoneName(goals: GoalSummary[], phaseName: string): string {
    if (goals.length === 1) {
      return goals[0].name;
    }
    ...
    for (const goal of milestoneGoals) {
      if (goal.dependencies) {
        for (const depId of goal.dependencies) {
          ...
          const dependencyMilestone = previousMilestones.find(m =>
            m.goals.includes(depId)
          );
   ...
   */
  private static calculatePhaseDuration(goals: GoalSummary[]): string {
  ...
  private static generateTimeline(
    ...
    let totalWeeks = 0;
    for (const phase of phases) {
      if (phase.duration === 'TBD') continue;
