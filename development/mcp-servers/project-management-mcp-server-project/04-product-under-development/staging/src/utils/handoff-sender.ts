...
import {
  ...
  GoalToSpecHandoff,
  SpecToTasksHandoff,
  TaskCompletionHandoff,
  ProgressUpdateHandoff,
 ...
 */
export function createGoalToSpecHandoff(
  projectPath: string,
  goalData: {
    goalId: string;
    goalName: string;
 ...
 */
export function createSpecToTasksHandoff(
  projectPath: string,
  specData: {
    specificationPath: string;
    goalId: string;
    goalName: string;
    tasks: Array<{ description: string; estimatedHours?: number; dependencies?: string[] }>;
 ...
 */
export function createTaskCompletionHandoff(
  projectPath: string,
  taskData: {
    workflowName: string;
    goalId: string;
 ...
 */
export function createProgressUpdateHandoff(
  projectPath: string,
  progressData: ProgressUpdateHandoff
): Handoff<ProgressUpdateHandoff> {
  const metadata: HandoffMetadata = {
    version: HANDOFF_VERSION,
    handoffId: uuidv4(),
 ...
 */
export function createSubgoalCompletionHandoff(
  projectPath: string,
  data: any
): Handoff<any> {
  const metadata: HandoffMetadata = {
 ...
 */
export function sendHandoff(handoff: Handoff): string {
...
export class HandoffBuilder {
  private handoff: Partial<Handoff>;
  ...
  build(): Handoff {
    ...
    return this.handoff as Handoff;
  }
