...
function generateRoadmapLayout(
  ...
  const goalsByTier: Record<string, GoalForDiagram[]> = {
    Now: [],
    Next: [],
    Later: [],
    Someday: [],
  ...
  for (let i = 0; i < tiers.length; i++) {
    ...
    builder.createTierHeader({
      tier: tier as 'Now' | 'Next' | 'Later' | 'Someday',
      x,
      y: 50,
      width: 300,
 ...
 */
function generateKanbanLayout(
  potentialGoals: any[],
  selectedGoals: GoalForDiagram[],
  archivedGoals: any[],
  builder: DrawioXmlBuilder,
  ...
  // Define columns
  const columns: Array<{ name: string; goals: any[] }> = [];
 ...
 */
export function generateGoalsDiagram(
  projectPath: string,
  ...
  if (options.tier) {
    selectedGoals = selectedGoals.filter((g: any) => g.tier === options.tier);
  }
  if (options.priority) {
    selectedGoals = selectedGoals.filter((g: any) => g.priority === options.priority);
  }
  ...
  switch (diagramType) {
    ...
    case 'kanban':
      generateKanbanLayout(
        options.includePotential ? scanResult.potentialGoals : [],
        selectedGoals,
        options.includeArchived ? [] : [],  // TODO: Scan archived goals
        builder,
        options
      );
      break;
 ...
 */
export function saveDiagram(xml: string, outputPath: string): void {
