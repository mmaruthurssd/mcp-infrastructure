...
import {
  ...
  extractProjectName,
  extractProjectType,
} from '../utils/information-extraction';
...
export interface StartProjectOverviewConversationInput {
  ...
  projectName?: string; // Optional: Pre-fill project name
  initialDescription?: string; // Optional: Seed conversation with initial description
}
 ...
 * Start PROJECT OVERVIEW conversation
 */
export async function startProjectOverviewConversation(
  input: StartProjectOverviewConversationInput
): Promise<StartProjectOverviewConversationOutput> {
  try {
...
export interface ContinueConversationOutput {
  ...
  confirmationPreview?: string; // If moving to confirmation phase
  conversationComplete: boolean;
  extractedData?: ExtractedInformation;
  error?: string;
}

export async function continueProjectOverviewConversation(
...
async function extractInformationFromResponse(
  ...
  const {
    ...
    extractResources,
    suggestComponents,
  } = await import('../utils/information-extraction');
  ...
  switch (questionData.id) {
    ...
    case 5: // Components
      if (text.includes('skip') || text.includes('not sure') || text.includes('suggest')) {
        ...
        if (state.extracted.projectType) {
          const suggested = suggestComponents(
            state.extracted.projectType,
...
function generateConfirmationPreview(extracted: ExtractedInformation): string {
