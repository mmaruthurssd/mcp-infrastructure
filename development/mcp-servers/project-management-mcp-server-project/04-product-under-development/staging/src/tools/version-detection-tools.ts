...
async function loadUserPreferences(projectPath: string): Promise<UserPreferences> {
...
async function saveUserPreferences(projectPath: string, prefs: UserPreferences): Promise<void> {
  const prefsPath = path.join(projectPath, PREFERENCES_FILE);
 ...
 */
export async function detectProjectVersion(
  params: z.infer<typeof DetectProjectVersionSchema>
): Promise<{
 ...
 */
export async function checkMigrationRecommended(
  params: z.infer<typeof CheckMigrationRecommendedSchema>
): Promise<{
  recommended: boolean;
  reason: string;
  goalCount?: number;
...
export async function getVersionInfo(
  params: z.infer<typeof GetVersionInfoSchema>
): Promise<{
  version: ProjectVersion;
  confidence: number;
  markers: {
  ...
  const v100OnlyFeatures: string[] = [];
  const migrationFeatures: string[] = [];

  Object.entries(features).forEach(([feature, available]) => {
    if (feature.includes('Migration') || feature.includes('migration')) {
      if (available) migrationFeatures.push(feature);
    } else if (available) {
 ...
 */
export async function setMigrationPreference(
  params: z.infer<typeof SetMigrationPreferenceSchema>
): Promise<{
  success: boolean;
  message: string;
