...
import {
  ComponentToGoalParams,
  ComponentToGoalResult,
  ...
  static execute(params: ComponentToGoalParams): ComponentToGoalResult {
    try {
      ...
      propagateComponentChange(
        ...
        'converted_to_goal',
        component.stage,
        archived ? 'ARCHIVED' : component.stage
      );
...
${
  component.dependencies?.requires && component.dependencies.requires.length > 0
    ? component.dependencies.requires.map((d: string) => `- ${d}`).join('\n')
    : '- None'
}
...
${
  component.dependencies?.requiredBy && component.dependencies.requiredBy.length > 0
    ? component.dependencies.requiredBy.map((d: string) => `- ${d}`).join('\n')
    : '- None'
}
...
${
  component.dependencies?.relatedTo && component.dependencies.relatedTo.length > 0
    ? component.dependencies.relatedTo.map((d: string) => `- ${d}`).join('\n')
    : '- None'
}
