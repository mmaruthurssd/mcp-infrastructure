...
import { ProjectOverview, Component, MajorGoal, ProgressStatus } from '../types/hierarchical-entities.js';
...
export interface NextStepSuggestion {
  ...
  mcpTool?: string; // Which MCP tool to use
...
export async function suggestNextSteps(params: z.infer<typeof SuggestNextStepsSchema>) {
...
export async function getProjectPhase(params: z.infer<typeof GetProjectPhaseSchema>) {
  const startTime = Date.now();
...
export async function advanceWorkflowPhase(params: z.infer<typeof AdvanceWorkflowPhaseSchema>) {
  const startTime = Date.now();
  ...
  try {
    ...
    const transitionResult = await performPhaseTransition(
      projectPath,
...
export async function getConversationContext(params: z.infer<typeof GetConversationContextSchema>) {
  const startTime = Date.now();
...
async function analyzeProjectState(projectPath: string): Promise<ProjectState> {
...
async function generateDetailedReasoning(
  ...
  const details: string[] = [suggestion.reasoning];
...
async function determineProjectPhase(projectPath: string): Promise<ProjectPhaseInfo> {
  const state = await analyzeProjectState(projectPath);

  const phaseInfo: ProjectPhaseInfo = {
  ...
  switch (state.phase) {
    case 'planning':
      ...
      if (state.hasProjectOverview) planningChecklist += 33;
      if (state.componentCount > 0) planningChecklist += 33;
      if (state.totalGoals > 0) planningChecklist += 34;
...
function getNextPhase(currentPhase: string): 'planning' | 'execution' | 'monitoring' | 'completion' {
  const sequence: ('planning' | 'execution' | 'monitoring' | 'completion')[] = [
    'planning',
    'execution',
    'monitoring',
    'completion',
...
async function performPhaseTransition(
  projectPath: string,
  fromPhase: string,
  toPhase: string
): Promise<{ actions: string[]; recommendations: string[] }> {
...
async function buildOverviewContext(projectPath: string): Promise<ConversationContext> {
  const state = await analyzeProjectState(projectPath);
...
async function buildDetailedContext(projectPath: string): Promise<ConversationContext> {
...
async function buildSpecificEntityContext(
  projectPath: string,
  entityId: string
