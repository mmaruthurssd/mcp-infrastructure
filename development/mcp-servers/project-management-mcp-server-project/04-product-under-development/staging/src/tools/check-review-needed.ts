...
import { detectReviewsNeeded, ReviewItem } from '../utils/review-detector.js';
 ...
 */
function formatOutput(reviewItems: ReviewItem[], summary: any): string {
  let output = '═══════════════════════════════════════════════════════\n';
...
export class CheckReviewNeededTool {
  ...
  static execute(input: CheckReviewNeededInput): CheckReviewNeededOutput {
    try {
      ...
      const reviewItems = detectReviewsNeeded(
        input.projectPath,
        input.checkType || 'all',
        customThresholds
      );
