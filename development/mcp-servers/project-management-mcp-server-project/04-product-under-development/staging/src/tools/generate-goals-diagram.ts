...
function formatOutput(
  ...
  output += `${typeEmoji[diagramType as keyof typeof typeEmoji] || '📊'} Diagram Type: ${diagramType.charAt(0).toUpperCase() + diagramType.slice(1)}\n`;
 ...
 */
function countGoals(xml: string): number {
...
export class GenerateGoalsDiagramTool {
  ...
  static execute(input: GenerateGoalsDiagramInput): GenerateGoalsDiagramOutput {
    try {
      ...
      const outputPath = input.outputPath || path.join(
        input.projectPath,
        '02-goals-and-roadmap',
        'workflow-diagrams',
        `GOALS-${input.diagramType.toUpperCase()}.drawio`
      );
