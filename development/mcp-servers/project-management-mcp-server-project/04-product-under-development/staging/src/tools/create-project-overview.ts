...
export interface QuestionDefinition {
  ...
  purpose: string;
  optional: boolean;
  followUps?: string[]; // IDs of follow-up questions based on answer
  extractionHints: string[]; // What to extract from the answer
}
 ...
 */
export const QUESTION_FLOW: Record<string, QuestionDefinition> = {
  // ========== PHASE 1: ESSENTIALS ==========
  'q1-project-name': {
    id: 'q1-project-name',
 ...
 */
export function getNextQuestion(state: ConversationState): QuestionDefinition | null {
  ...
  const phaseOrder: ConversationState['phase'][] = [
    'essentials',
    'vision',
    'stakeholders',
    'resources',
 ...
 */
export function initializeConversation(projectId: string): ConversationState {
  return {
    projectId,
...
import { ProjectOverview } from '../types/hierarchical-entities.js';
...
export async function continueProjectOverviewConversation(args: {
  ...
  state.extractedData = extractFromResponse(
    currentQuestion.id,
    userResponse,
    state.extractedData
  );
...
export async function finalizeProjectOverview(args: {
  ...
  const projectOverview = extractedDataToProjectOverview(
    state.extractedData as ExtractedProjectData,
    state.projectId,
    projectPath
  );
  ...
  const content = generateProjectOverviewContent(state.extractedData as ExtractedProjectData, state.projectId);
...
export const createProjectOverviewTool = {
  ...
  description:
    'Start a guided conversation to create a comprehensive PROJECT OVERVIEW document for hierarchical planning v1.0.0',
...
export const finalizeProjectOverviewTool = {
  ...
  description:
    'Generate and save the PROJECT OVERVIEW document from conversation data',
  inputSchema: z.object({
    projectPath: z.string().describe('Absolute path to the project directory'),
    conversationId: z.string().describe('Conversation ID from create_project_overview'),
...
export async function updateProjectOverview(args: {
  ...
  try {
    ...
    const updatedData: ExtractedProjectData = {
      ...oldData,
      ...updates,
    };
    ...
    const cascadeImpacts = await analyzeCascadeImpacts(
      projectPath,
      oldEntity,
      newEntity
    );
    ...
    if (!validationResult.valid) {
      ...
      return {
        ...
        warnings: validationResult.errors.map((e: any) => e.message || String(e)),
      };
    }
 ...
 */
function validateUpdatesStructure(updates: Partial<ExtractedProjectData>): void {
 ...
 */
async function analyzeCascadeImpacts(
  projectPath: string,
  oldEntity: ProjectOverview,
  newEntity: ProjectOverview
): Promise<any[]> {
 ...
 */
function parseProjectOverviewMarkdown(content: string): ExtractedProjectData {
  const data: Partial<ExtractedProjectData> = {};
  ...
  if (purposeMatch) data.primaryPurpose = purposeMatch[1];

  const visionMatch = content.match(/### Vision Statement\n\n([^#]+)/);
  if (visionMatch) data.visionStatement = visionMatch[1].trim();
  ...
  return data as ExtractedProjectData;
}

/**
 * Extract list items from a markdown section
 */
function extractListFromSection(content: string, sectionHeader: string): string[] {
...
export const updateProjectOverviewTool = {
  ...
  description:
    'Update existing PROJECT OVERVIEW with cascade detection and version control',
  inputSchema: z.object({
    projectPath: z.string().describe('Absolute path to the project directory'),
    updates: z.object({
...
export async function identifyComponentsFromOverview(args: {
  projectPath: string;
  useAI?: boolean; // Whether to use AI-based analysis (future feature)
}): Promise<{
  success: boolean;
 ...
 */
function analyzeProjectForComponents(data: ExtractedProjectData): SuggestedComponent[] {
 ...
 */
function identifyFunctionalAreas(
  goals: string[],
  outcomes: string[]
): SuggestedComponent[] {
  const components: SuggestedComponent[] = [];
  ...
  for (const pattern of patterns) {
    ...
    if (matchCount > 0) {
      components.push({
        ...
        suggestedGoals: goals.filter(g =>
          pattern.keywords.some(kw => g.toLowerCase().includes(kw))
        ),
      });
    }
  }
 ...
 */
function identifyStakeholderComponents(
  stakeholders: Array<{ name: string; role: string; influence: string; interest: string }>
): SuggestedComponent[] {
  const components: SuggestedComponent[] = [];
 ...
 */
function identifyTechnologyComponents(
  technologies: string[],
  tools: string[]
): SuggestedComponent[] {
  const components: SuggestedComponent[] = [];
 ...
 */
function getDefaultComponentsForProjectType(
  projectType: 'software' | 'research' | 'business' | 'product' | 'other'
): SuggestedComponent[] {
  switch (projectType) {
    case 'software':
 ...
 */
function mergeAndDeduplicateComponents(
  components: SuggestedComponent[]
): SuggestedComponent[] {
  const merged: Record<string, SuggestedComponent> = {};
 ...
 */
function generateComponentReasoning(
...
export const identifyComponentsFromOverviewTool = {
  ...
  description:
    'Analyze PROJECT OVERVIEW and suggest logical components based on goals, stakeholders, and project type',
  inputSchema: z.object({
    projectPath: z.string().describe('Absolute path to the project directory'),
