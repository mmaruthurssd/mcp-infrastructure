...
export class ValidateProjectTool {
  static execute(input: ValidateProjectInput): ValidationResult {
    const checks: ValidationType[] = input.checks || ['all'];
  ...
  private static validateStructure(projectPath: string): StructureValidation {
  ...
  private static validateYamlFrontmatter(projectPath: string): YamlValidation {
    ...
    for (const file of mdFiles) {
      ...
      if (!frontmatter) {
        ...
        const missingFields = YAML_REQUIRED_FIELDS.filter(
        ...
        if (missingFields.length > 0) {
          ...
          validation.details.push(
            `⚠️  Missing fields (${missingFields.join(', ')}): ${relativePath}`
          );
          validation.passed = false;
        } else {
  ...
  private static validateResourceIndex(projectPath: string): ResourceIndexValidation {
    const validation: ResourceIndexValidation = {
      passed: true,
      untrackedFiles: [],
      missingIndex: false,
      details: [],
  ...
  private static validateGoals(projectPath: string): GoalsValidation {
    ...
    const potentialGoalsPath = path.join(
      projectPath,
      '02-goals-and-roadmap',
      'potential-goals'
    );
  ...
  private static validateGoalsInFolder(
    ...
    for (const goalFolder of goalFolders) {
      ...
      if (missingFields.length > 0) {
        validation.goalsWithInvalidMetadata.push(goalFolder);
        validation.details.push(
          `⚠️  Missing fields (${missingFields.join(', ')}): ${path.basename(goalFolder)}`
        );
        validation.passed = false;
      } else {
  ...
  private static validateWorkflow(projectPath: string): WorkflowValidation {
    ...
    const workflowSteps = [
      ...
      },
      {
        ...
        check: () => {
          ...
          if (!fs.existsSync(potentialGoalsPath)) return false;
      ...
      },
      {
        ...
        check: () => {
          ...
          if (!fs.existsSync(diagramsPath)) return false;
      ...
      },
      {
        ...
        check: () => {
          ...
          if (!fs.existsSync(selectedGoalsPath)) return false;

          const goalFolders = fs.readdirSync(selectedGoalsPath, { withFileTypes: true })
            .filter(dirent => dirent.isDirectory());

          if (goalFolders.length === 0) return true; // No selected goals yet is okay
    ...
    validation.details.unshift(
      `Workflow Completion: ${validation.completionPercentage}% (${completedCount}/${workflowSteps.length} steps)`
    );
  ...
  private static applyAutoFixes(input: ValidateProjectInput, result: ValidationResult): number {
    let fixesApplied = 0;
  ...
  private static updateSummary(result: ValidationResult, validation: any): void {
    if (!validation.passed) {
  ...
  private static findMarkdownFiles(dir: string): string[] {
    const files: string[] = [];
  ...
  private static findResourceFiles(dir: string): string[] {
    const files: string[] = [];
  ...
  private static extractFrontmatter(content: string): Record<string, any> | null {
  ...
  private static generateDefaultFrontmatter(filePath: string): string {
  ...
  private static inferFileType(fileName: string): string {
    ...
    if (fileName.includes('guide') || fileName.includes('GUIDE')) return 'guide';
    if (fileName.includes('plan') || fileName.includes('PLAN')) return 'plan';
    if (fileName.includes('spec')) return 'specification';
    if (fileName.includes('goal')) return 'goal';
    return 'reference';
  }

  private static inferTags(fileName: string): string[] {
    const tags: string[] = [];
    const lowerName = fileName.toLowerCase();

    if (lowerName.includes('goal')) tags.push('goals');
    if (lowerName.includes('plan')) tags.push('planning');
  ...
  private static getDefaultFileContent(fileName: string, projectPath: string): string {
  ...
  private static getSummaryMessage(result: ValidationResult): string {
