 ...
 */
function parseGoalFromSelected(content: string, goalId: string): any {
 ...
 */
function calculateEstimateAccuracy(
  estimatedImpact: string,
  actualImpact: string | undefined,
  estimatedEffort: string,
  actualEffort: string | undefined
): EstimateAccuracy {
  const impactLevels = { High: 3, Medium: 2, Low: 1 };

  const estImpactVal = impactLevels[estimatedImpact as keyof typeof impactLevels] || 2;
  const actImpactVal = actualImpact
    ? impactLevels[actualImpact as keyof typeof impactLevels] || 2
    : estImpactVal;

  let impactAccuracy: 'Accurate' | 'Overestimated' | 'Underestimated';
  ...
  const estEffortVal = impactLevels[estimatedEffort as keyof typeof impactLevels] || 2;
  const actEffortVal = actualEffort
    ? parseEffortToLevel(actualEffort)
    : estEffortVal;

  let effortAccuracy: 'Accurate' | 'Overestimated' | 'Underestimated';
 ...
 */
function parseEffortToLevel(effort: string): number {
 ...
 */
function removeGoalFromSelected(filePath: string, goalId: string): void {
  ...
  const finalContent = updatedContent.replace(
    ...
    `**Total Active Goals:** ${activeCount}`
  );

  // Update timestamp
  const today = new Date().toISOString().split('T')[0];
  const timestampedContent = finalContent.replace(
    /\*\*Last Updated:\*\*\s+.+/,
    `**Last Updated:** ${today}`
  );
 ...
 */
function createRetrospectiveFile(
  archivePath: string,
  goalData: any,
  retrospective: RetrospectiveData,
  archiveType: 'implemented' | 'shelved',
  ...
  static execute(input: ArchiveGoalInput): ArchiveGoalOutput {
    try {
      ...
      const estimateAccuracy = calculateEstimateAccuracy(
        goalData.impact,
        input.retrospective.actualImpact,
        goalData.effort,
        input.retrospective.actualEffort
      );

      // Step 4: Create retrospective file
      const archivePath = path.join(
        input.projectPath,
        `brainstorming/future-goals/archive/${input.archiveType}`
      );

      const archivedFilePath = createRetrospectiveFile(
        archivePath,
        goalData,
        input.retrospective,
        input.archiveType,
