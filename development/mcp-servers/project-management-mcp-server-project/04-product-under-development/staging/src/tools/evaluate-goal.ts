...
import { ImpactEstimator, ImpactEstimate } from '../evaluators/impact-estimator.js';
import { EffortEstimator, EffortEstimate } from '../evaluators/effort-estimator.js';
import { TierSuggester, GoalEvaluation } from '../evaluators/tier-suggester.js';
import { AutonomousClassifier, AutonomousClassification } from '../evaluators/autonomous-classifier.js';

export interface EvaluateGoalArgs {
  goalDescription: string;
  context?: string;
  projectType?: string;
  workspacePath?: string;  // Optional: enables autonomous classification
}

export interface EvaluateGoalResult extends GoalEvaluation {
  suggestions: string[];
  nextSteps: string[];
  autonomous?: AutonomousClassification;  // Optional: added when workspacePath provided
}
...
export class EvaluateGoalTool {
  ...
  static getToolDefinition() {
    return {
      name: 'evaluate_goal',
      description: 'Analyze a goal description to estimate Impact (High/Med/Low), Effort (High/Med/Low), and suggest a tier (Now/Next/Later/Someday). Includes autonomous deployment framework classification for confidence-based autonomy.',
      inputSchema: {
        type: 'object',
        properties: {
          goalDescription: {
          ...
          workspacePath: {
            type: 'string',
            description: 'Optional workspace path for autonomous classification (enables confidence-based autonomy analysis)'
          }
        },
   ...
   */
  static async execute(args: EvaluateGoalArgs): Promise<EvaluateGoalResult> {
    const { goalDescription, context, projectType, workspacePath } = args;
    ...
    // Autonomous classification (optional - only if workspacePath provided)
    let autonomous: AutonomousClassification | undefined;
    if (workspacePath) {
      const classifier = new AutonomousClassifier(workspacePath);
      autonomous = await classifier.classify(goalDescription, {
        component: 'goal',
        symptom: context
      });
    }
  ...
  private static generateSuggestions(
    ...
    tierSuggestion: { tier: string; confidence: string }
  ): string[] {
  ...
  static formatResult(result: EvaluateGoalResult): string {
    ...
    if (result.tier.alternativeTiers && result.tier.alternativeTiers.length > 0) {
      ...
      result.tier.alternativeTiers.forEach(alt => {
        output += `- **${alt.tier}**: ${alt.reasoning}\n`;
      });
      output += '\n';
    }

    // Autonomous classification (if available)
    if (result.autonomous) {
      output += `## Autonomous Deployment Classification\n\n`;
      output += `**Issue Type:** ${result.autonomous.issueType}`;
      if (result.autonomous.issueType !== result.autonomous.baseType) {
        output += ` → ${result.autonomous.baseType}`;
      }
      output += '\n\n';
      output += `**Confidence:** ${(result.autonomous.confidence * 100).toFixed(0)}%\n\n`;
      output += `**Recommended Action:** ${result.autonomous.recommendedAction}\n\n`;
      output += `**Reasoning:** ${result.autonomous.classificationReasoning}\n\n`;

      // Autonomy thresholds
      output += `**Thresholds:**\n`;
      output += `- Autonomous: ≥${(result.autonomous.autonomousThreshold * 100).toFixed(0)}%\n`;
      output += `- Assisted: ≥${(result.autonomous.assistedThreshold * 100).toFixed(0)}%\n`;
      output += `- Manual: <${(result.autonomous.assistedThreshold * 100).toFixed(0)}%\n\n`;
    }
