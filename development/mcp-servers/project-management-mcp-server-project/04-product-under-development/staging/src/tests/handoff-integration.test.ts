...
import {
  createGoalToSpecHandoff,
  createSpecToTasksHandoff,
  createTaskCompletionHandoff,
  createSubgoalCompletionHandoff,
  createProgressUpdateHandoff,
  sendHandoff,
  serializeHandoff,
  HandoffBuilder
} from '../utils/handoff-sender';
...
} from '../utils/handoff-error-handler';

import {
  AtomicTransaction,
  executeAtomicHandoff,
  executeAtomicBatch,
  TwoPhaseCommit,
  TwoPhaseState
} from '../utils/handoff-atomic';

import {
  HandoffAuditLogger,
  AuditLogQuery
} from '../utils/handoff-audit';

import {
  GoalToSpecPayload,
  SpecToTasksPayload,
  TaskCompletionPayload,
  SubgoalCompletionPayload,
  ProgressUpdatePayload,
  HandoffContext
} from '../types/handoff-protocol';
...
let auditLogger: HandoffAuditLogger;
...
function createTestContext(): HandoffContext {
  return {
...
function createTestGoalToSpecPayload(): GoalToSpecPayload {
  return {
    majorGoal: {
      id: '001',
...
describe('Handoff Type Creation', () => {
  ...
  it('should create valid SpecToTasksHandoff', () => {
    const context = createTestContext();
    const payload: SpecToTasksPayload = {
      subGoal: {
        id: '1.1',
        name: 'Test Sub-Goal',
  ...
  it('should create valid SubgoalCompletionHandoff', () => {
    const context = createTestContext();
    const payload: SubgoalCompletionPayload = {
      subGoal: {
        id: '1.1',
        name: 'Test Sub-Goal',
        majorGoalId: '001',
