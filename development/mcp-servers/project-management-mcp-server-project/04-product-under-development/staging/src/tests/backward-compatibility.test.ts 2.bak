...
} from '../utils/project-structure-adapter.js';
import {
...
} from '../utils/backward-compatibility-wrapper.js';
 ...
 */
async function setupV080Project(): Promise<void> {
  ...
  await fs.writeFile(
    path.join(V080_PROJECT, 'brainstorming/future-goals/SELECTED-GOALS.md'),
    `# Selected Goals
...
`
  ...
  await fs.writeFile(
    path.join(V080_PROJECT, 'brainstorming/future-goals/potential-goals/001-test-goal.md'),
    `---
id: "001"
name: Test Goal
impact: High
 ...
 */
async function setupV100Project(): Promise<void> {
  ...
  await fs.writeFile(
    path.join(V100_PROJECT, '01-planning/PROJECT-OVERVIEW.md'),
    `---
version: 1
---
...
`
  ...
  // Create ROADMAP
  await fs.writeFile(
    path.join(V100_PROJECT, '02-goals-and-roadmap/ROADMAP.md'),
    `# Roadmap
...
`
  ...
  await fs.writeFile(
    path.join(V100_PROJECT, '02-goals-and-roadmap/components/test-component/COMPONENT-OVERVIEW.md'),
    `---
id: test-component
name: Test Component
---
...
`
  ...
  // Create major goal
  await fs.writeFile(
    path.join(V100_PROJECT, '02-goals-and-roadmap/components/test-component/major-goals/001-test-goal.md'),
    `---
id: "001"
name: Test Goal
 ...
 */
async function setupUnknownProject(): Promise<void> {
...
describe('ProjectStructureAdapter - Error Handling', () => {
  ...
  test('should throw helpful error for unavailable feature', async () => {
    ...
    await expect(
      adapter.assertFeatureAvailable('createComponent', 'Create Component')
...
describe('Backward Compatibility Wrapper - withBackwardCompatibility', () => {
  ...
  test('should allow v1.0.0-only feature in v1.0.0 project', async () => {
    const result = await withBackwardCompatibility(
      V100_PROJECT,
      'createComponent',
      'Create Component',
      async () => {
  ...
  test('should provide context in error messages', async () => {
    try {
      await withBackwardCompatibility(
        V080_PROJECT,
        'createComponent',
        'Create Component',
        async () => {
...
describe('Backward Compatibility Wrapper - Helper Functions', () => {
  ...
  test('should get feature unavailable message', async () => {
    const message = await getFeatureUnavailableMessage(
      V080_PROJECT,
      'createComponent',
      'Create Component'
    );
