...
import {
  startProjectOverviewConversation,
  continueProjectOverviewConversation,
} from '../tools/start-project-overview-conversation';
import {
  generateProjectOverview,
  approveProjectOverview,
} from '../tools/generate-project-overview';
...
import { ProgressUpdateSystem } from '../services/progress-update-system';
import { loadProjectHierarchy } from '../loaders/entity-loader';
  ...
  describe('Information Extraction', () => {
    ...
    it('should extract success criteria from text', async () => {
      ...
      const criteria = extractSuccessCriteria(
        `Success means 50% reduction in time, 100 users onboarded, and positive feedback`
      );
    ...
    it('should extract technologies', async () => {
      ...
      const { technologies } = extractTechnologies(
        'Using React, Node.js, PostgreSQL, and Docker'
      );
...
describe('Goal 008: Progress Aggregation System', () => {
  let testProjectPath: string;
  let cache: ProgressCacheManager;
