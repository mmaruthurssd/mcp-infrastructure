...
} from '../tools/version-control-tools';
...
describe('Version Control Tools - Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    await mkdir(
      join(
        projectPath,
        '02-goals-and-roadmap/components/test-component/sub-area-1/major-goals/001-test-goal'
      ),
      { recursive: true }
    );
  });
...
`,
      ...
      const componentPath = join(
        projectPath,
        '02-goals-and-roadmap/components/test-component/OVERVIEW.md'
      );
      await writeFile(
        componentPath,
        `---
...
`,
      ...
      const goalPath = join(
        projectPath,
        '02-goals-and-roadmap/components/test-component/sub-area-1/major-goals/001-test-goal/GOAL-STATUS.md'
      );
      await writeFile(
        goalPath,
        `---
...
`,
      ...
      for (let i = 1; i <= 6; i++) {
        const compPath = join(
          projectPath,
          `02-goals-and-roadmap/components/component-${i}/OVERVIEW.md`
        );
...
`,
      ...
      expect(result.warnings).toEqual(
        expect.arrayContaining([expect.stringContaining('Backup created')])
      );
    });
...
`,
      ...
      expect(result.success).toBe(true);
      expect(result.warnings).toEqual(
        expect.arrayContaining([expect.stringContaining('Cascade rollback')])
      );
    });
  });
...
`,
      ...
      expect(result.warnings).toEqual(
        expect.arrayContaining([expect.stringContaining('goals depend')])
      );
    });
  ...
  describe('Performance & Concurrency', () => {
    it('should handle multiple concurrent impact analyses', async () => {
      ...
      for (const comp of components) {
        const compPath = join(
          projectPath,
          `02-goals-and-roadmap/components/${comp}/OVERVIEW.md`
        );
      ...
      const analyses = components.map(comp =>
        analyzeVersionImpact({
          projectPath,
          documentType: 'component',
