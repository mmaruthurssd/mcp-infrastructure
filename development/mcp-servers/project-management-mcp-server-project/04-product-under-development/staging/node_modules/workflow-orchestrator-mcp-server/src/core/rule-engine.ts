...
export interface RuleAction {
  ...
  reason: string | ((context: RuleContext) => string); // Why this action is suggested
  tool: string; // MCP tool to execute
  paramBuilder: (context: RuleContext) => any; // Build tool parameters
}
...
export class RuleEngine {
      ...
      conditions: [
        {
          ...
          check: ctx => {
            ...
            const archivedWorkflows = fs.readdirSync(archivePath).filter(f =>
   ...
   */
  private fileExists(projectPath: string, relativePath: string): boolean {
