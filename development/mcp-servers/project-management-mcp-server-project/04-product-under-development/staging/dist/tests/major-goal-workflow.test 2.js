...
import { createMajorGoal, promoteToMajorGoal, handoffToSpecDriven, updateMajorGoalProgress, getMajorGoalStatus, } from '../tools/major-goal-workflow';
describe('Goal 006: Major Goal Workflow Tools', () => {
    ...
    describe('create_major_goal', () => {
        it('should create major goal in component', async () => {
            ...
            expect(result.handoffContext?.projectPath).toBe(projectPath);
            expect(result.handoffContext?.componentId).toBe('test-component');
            expect(result.handoffContext?.majorGoalId).toBe('001');
            // Verify folder created
            const goalFolderPath = path.join(projectPath, '02-goals-and-roadmap', 'components', 'test-component', 'major-goals', '001-implement-authentication-system');
        ...
        it('should create major goal in sub-area', async () => {
            ...
            const goalFolderPath = path.join(projectPath, '02-goals-and-roadmap', 'components', 'test-component', 'sub-areas', 'test-sub-area', 'major-goals', '001-build-api-gateway');
            expect(fs.existsSync(goalFolderPath)).toBe(true);
        });
...
`;
            ...
            const goalFolderPath = path.join(projectPath, '02-goals-and-roadmap', 'components', 'test-component', 'major-goals', '001-advanced-analytics-dashboard');
            const statusFilePath = path.join(goalFolderPath, 'GOAL-STATUS.md');
            const statusContent = fs.readFileSync(statusFilePath, 'utf-8');
...
`;
            ...
            expect(handoffResult.handoffContext?.projectPath).toBe(projectPath);
            expect(handoffResult.handoffContext?.majorGoalId).toBe(promoteResult.goalId);
            // Step 4: Get status
...
//# sourceMappingURL=major-goal-workflow.test%202.js.map