...
import { suggestNextSteps, getProjectPhase, advanceWorkflowPhase, getConversationContext, } from '../tools/conversation-flow-tools.js';
...
describe('Conversation Flow Tools - Goal 010', () => {
    ...
    describe('suggest_next_steps', () => {
        ...
        test('should suggest creating components when PROJECT OVERVIEW exists', async () => {
            ...
            await fs.writeFile(path.join(TEST_PROJECT_PATH, '01-planning', 'PROJECT-OVERVIEW.md'), '# PROJECT OVERVIEW\n\nProject vision...', 'utf-8');
            const result = await suggestNextSteps({
                projectPath: TEST_PROJECT_PATH,
        ...
        test('should suggest defining goals when components exist', async () => {
            ...
            await fs.writeFile(path.join(TEST_PROJECT_PATH, '01-planning', 'PROJECT-OVERVIEW.md'), '# PROJECT OVERVIEW', 'utf-8');
            await fs.mkdir(path.join(TEST_PROJECT_PATH, '02-goals-and-roadmap', 'components', 'component-1'), {
                recursive: true,
            });
            await fs.mkdir(path.join(TEST_PROJECT_PATH, '02-goals-and-roadmap', 'components', 'component-1', 'major-goals'), { recursive: true });
            const result = await suggestNextSteps({
                projectPath: TEST_PROJECT_PATH,
                includeDetails: false,
                maxSuggestions: 5,
        ...
        test('should suggest execution when goals exist but none in progress', async () => {
            ...
            await fs.writeFile(path.join(TEST_PROJECT_PATH, '02-goals-and-roadmap', 'components', 'component-1', 'major-goals', '001-test-goal.md'), '# Goal 001\n\n**Status:** Planning', 'utf-8');
            const result = await suggestNextSteps({
                projectPath: TEST_PROJECT_PATH,
                includeDetails: false,
                maxSuggestions: 5,
        ...
        test('should prioritize blocked goals', async () => {
            ...
            await fs.writeFile(path.join(TEST_PROJECT_PATH, '02-goals-and-roadmap', 'components', 'component-1', 'major-goals', '001-blocked-goal.md'), '# Goal 001\n\n**Status:** Blocked', 'utf-8');
            const result = await suggestNextSteps({
                projectPath: TEST_PROJECT_PATH,
                includeDetails: false,
                maxSuggestions: 5,
    ...
    describe('get_project_phase', () => {
        ...
        test('should identify execution phase when goals in progress', async () => {
            ...
            await fs.writeFile(path.join(TEST_PROJECT_PATH, '02-goals-and-roadmap', 'components', 'component-1', 'major-goals', '001-goal.md'), '# Goal 001\n\n**Status:** In Progress', 'utf-8');
            const result = await getProjectPhase({
                projectPath: TEST_PROJECT_PATH,
            });
            expect(result.success).toBe(true);
    ...
    describe('get_conversation_context', () => {
        ...
        test('should include current blockers in context', async () => {
            ...
            await fs.writeFile(path.join(TEST_PROJECT_PATH, '02-goals-and-roadmap', 'components', 'comp-1', 'major-goals', '001-blocked.md'), '# Goal\n\n**Status:** Blocked', 'utf-8');
            const result = await getConversationContext({
                projectPath: TEST_PROJECT_PATH,
                contextType: 'overview',
            });
    ...
    describe('Edge Cases', () => {
        ...
        test('should handle malformed goal files', async () => {
            ...
            await fs.writeFile(path.join(TEST_PROJECT_PATH, '02-goals-and-roadmap', 'components', 'comp-1', 'major-goals', 'bad-goal.md'), 'Invalid content', 'utf-8');
            const result = await suggestNextSteps({
                projectPath: TEST_PROJECT_PATH,
                includeDetails: false,
                maxSuggestions: 5,
...
//# sourceMappingURL=conversation-flow-tools.test%202.js.map