...
import { detectProjectVersion, checkMigrationRecommended, getVersionInfo, setMigrationPreference } from '../tools/version-detection-tools.js';
 ...
 */
async function setupV080Project(projectPath, goalCount) {
    ...
    await fs.writeFile(path.join(projectPath, 'brainstorming/future-goals/SELECTED-GOALS.md'), selectedGoals);
    // Create potential goals
    const potentialCount = Math.ceil(goalCount / 2);
    for (let i = 1; i <= potentialCount; i++) {
        await fs.writeFile(path.join(projectPath, `brainstorming/future-goals/potential-goals/${String(i).padStart(3, '0')}-goal-${i}.md`), `---\nid: "${String(i).padStart(3, '0')}"\nname: Goal ${i}\n---\n\n# Goal ${i}\n`);
    }
}
/**
 * Setup v1.0.0 project
 */
async function setupV100Project(projectPath) {
    ...
    await fs.writeFile(path.join(projectPath, '01-planning/PROJECT-OVERVIEW.md'), '---\nversion: 1\n---\n\n# PROJECT OVERVIEW\n');
    await fs.writeFile(path.join(projectPath, '02-goals-and-roadmap/ROADMAP.md'), '# Roadmap\n');
}
/**
 * Setup new/empty project
 */
async function setupNewProject(projectPath) {
...
describe('Version Detection Tools - Edge Cases', () => {
    ...
    test('should handle non-existent project path', async () => {
        ...
        await expect(detectProjectVersion({ projectPath: nonExistentPath })).resolves.toBeDefined(); // Should not throw
    });
    ...
    test('should handle corrupt preferences file gracefully', async () => {
        await setupV080Project(V080_LARGE, 30);
        // Write invalid JSON
        await fs.writeFile(path.join(V080_LARGE, '.ai-planning-preferences.json'), '{ invalid json', 'utf-8');
        // Should not throw
        const result = await checkMigrationRecommended({ projectPath: V080_LARGE });
        expect(result).toBeDefined();
    });
});
//# sourceMappingURL=version-detection-tools.test%202.js.map