 ...
 */
export function parseSections(lines, offset = 0) {
    const sections = [];
 ...
 */
export function updateSectionContent(parsed, pattern, newContent) {
 ...
 */
export function insertAfterSection(parsed, pattern, content) {
    const sectionIndex = parsed.sections.findIndex((s) => typeof pattern === "string" ? new RegExp(pattern, "i").test(s.heading) : pattern.test(s.heading));
    if (sectionIndex < 0) {
        throw new Error(`Section not found: ${pattern}`);
    }
 ...
 * Add new section
 */
export function addSection(parsed, heading, level, content) {
    const newSection = {
        heading,
        level,
 ...
 * Remove section
 */
export function removeSection(parsed, pattern) {
    const regex = typeof pattern === "string" ? new RegExp(pattern, "i") : pattern;
 ...
 */
export function writeMarkdownFile(filePath, parsed) {
 ...
 */
export function parseComponentFromSection(content) {
    const lines = content.split("\n");
 ...
 */
export function updateFrontmatter(parsed, key, value) {
    if (!parsed.frontmatter) {
        parsed.frontmatter = {};
    }
 ...
 */
export function getFrontmatterField(parsed, key) {
