...
import { parseMarkdownFile, findSection, } from "./markdown-parser.js";
// ============================================================================
 ...
 */
export function loadProjectOverview(projectPath) {
 ...
 */
export function saveProjectOverview(projectPath, overview) {
    const overviewPath = path.join(projectPath, "01-planning/project-overview-working.md");
    // Update timestamp
 ...
 */
export function createComponent(projectPath, component) {
    ...
    const stageKey = component.stage.toLowerCase();
 ...
 */
export function getComponent(projectPath, componentName) {
    const overview = loadProjectOverview(projectPath);
    const allComponents = [
        ...overview.components.exploring,
        ...overview.components.framework,
 ...
 * Update a component
 */
export function updateComponent(projectPath, componentName, updates) {
    const overview = loadProjectOverview(projectPath);
    // Find component
 ...
 */
export function moveComponent(projectPath, componentName, toStage) {
    const overview = loadProjectOverview(projectPath);
    // Find component
    let targetComponent = null;
    let fromStage = null;
    for (const [stage, components] of Object.entries(overview.components)) {
        const index = components.findIndex((c) => c.name === componentName);
        if (index >= 0) {
            [targetComponent] = components.splice(index, 1);
            fromStage = stage.toUpperCase();
            break;
        }
    }
    ...
    // Add to new stage
    const toStageKey = toStage.toLowerCase();
 ...
 * Delete a component
 */
export function deleteComponent(projectPath, componentName) {
    const overview = loadProjectOverview(projectPath);
 ...
 */
export function logComponentChange(projectPath, entry) {
    const overview = loadProjectOverview(projectPath);
    const logEntry = {
        ...entry,
 ...
 */
export function getComponentHistory(projectPath, componentName, limit) {
    const overview = loadProjectOverview(projectPath);
 ...
 */
export function parseProjectOverviewFromMarkdown(parsed) {
    ...
    const archivedComponents = archivedSection
        ...
        : [];
    return {
        projectName: frontmatter.project_name || "Unknown Project",
        projectType: frontmatter.project_type || "software",
        description: frontmatter.description || "",
        currentPhase: frontmatter.current_phase || "initialization",
        components: {
 ...
 */
export function parseComponentsBetweenSections(sections, startSection, endSection, stage) {
    const components = [];
 ...
 */
export function parseComponentFromSection(section, stage) {
 ...
 */
export function parseComponentsFromSection(content, stage) {
    const components = [];
    const lines = content.split("\n");
    ...
    for (const line of lines) {
        ...
        if (trimmed.match(/Priority:/i)) {
            ...
            if (priorityMatch) {
                currentComponent.priority = priorityMatch[1].toLowerCase();
            }
        }
    }
    // Save last component
 ...
 */
export function parseChangeLogFromSection(content) {
    const entries = [];
    const lines = content.split("\n");
    for (const line of lines) {
        const trimmed = line.trim();
        // Match: - [timestamp] ComponentName: action (description)
        const match = trimmed.match(/^-\s*\[([^\]]+)\]\s+(.+?):\s+(\w+)(?:\s+(.+))?$/);
        if (match) {
 ...
 */
export function convertProjectOverviewToMarkdown(overview) {
    const parts = [];
    // Frontmatter
    parts.push("---");
