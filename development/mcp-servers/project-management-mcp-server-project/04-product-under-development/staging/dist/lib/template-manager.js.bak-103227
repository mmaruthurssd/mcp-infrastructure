 ...
 */
export function initializeProject(projectPath, projectName, projectType = "software") {
    const filesCreated = [];
 ...
 */
export function createPlaceholderMap(projectName, projectType = "software") {
    const now = new Date();
    return {
 ...
 */
export function updatePlaceholders(filePath, placeholders) {
 ...
 */
export function getProjectStructure(projectPath) {
    return FOLDER_STRUCTURE.map((folder) => path.join(projectPath, folder));
}
/**
 * Get key file paths
 */
export function getKeyFilePaths(projectPath) {
    const paths = {};
 ...
 */
export function copySingleTemplate(templateName, destPath, placeholders) {
    const templateBasePath = path.join(TEMPLATES_BASE, "file-templates");
