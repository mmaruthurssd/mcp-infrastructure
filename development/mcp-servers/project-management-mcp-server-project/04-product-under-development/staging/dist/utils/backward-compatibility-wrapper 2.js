 ...
 * ```
 */
import { ProjectStructureAdapter, ProjectVersion } from './project-structure-adapter.js';
/**
 ...
 * @returns Tool result
 */
export async function withBackwardCompatibility(projectPath, featureName, featureDisplayName, handler) {
 ...
 * @returns Tool result
 */
export async function withDualMode(projectPath, handler) {
    const adapter = new ProjectStructureAdapter(projectPath);
    const detection = await adapter.detectVersion();
    const paths = await adapter.getGoalPaths();
 ...
 */
export async function getFeatureUnavailableMessage(projectPath, featureName, featureDisplayName) {
    const adapter = new ProjectStructureAdapter(projectPath);
 ...
 */
export async function shouldShowUpgradePrompt(projectPath) {
    const adapter = new ProjectStructureAdapter(projectPath);
 ...
 */
export async function getUpgradePromptText(projectPath) {
    const adapter = new ProjectStructureAdapter(projectPath);
    const prompt = await adapter.getUpgradePrompt();
 ...
 */
export async function exampleDualModeTool(projectPath, goalId) {
 ...
 */
export async function exampleV100OnlyTool(projectPath, componentName) {
    return withBackwardCompatibility(projectPath, 'createComponent', // Feature key
...
//# sourceMappingURL=backward-compatibility-wrapper%202.js.map