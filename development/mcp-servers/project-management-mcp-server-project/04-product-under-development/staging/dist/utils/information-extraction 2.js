...
export function extractProjectType(text) {
    ...
    if (lowerText.includes('infrastructure') ||
 ...
 */
export function suggestComponents(projectType, description) {
    const components = [];
    switch (projectType) {
        case 'software':
            components.push({
...
export function calculateConfidence(extracted) {
    ...
    if (extracted.vision?.missionStatement)
        score += 5;
    total += 5;
    if (extracted.vision?.successCriteria && extracted.vision.successCriteria.length >= 3)
        score += 10;
    total += 10;
...
//# sourceMappingURL=information-extraction%202.js.map