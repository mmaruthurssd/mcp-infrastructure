 ...
 */
export function deserializeHandoff(json) {
    return JSON.parse(json);
}
/**
 * Load a handoff from file
 */
export function loadHandoffFromFile(filePath) {
 ...
 */
export function findPendingHandoffs(projectPath, targetMcp) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            if (handoff.metadata.targetMcp === targetMcp &&
...
export function validateHandoff(handoff) {
    ...
    if (!handoff.metadata)
        return false;
    if (!handoff.metadata.version)
        return false;
    if (!handoff.metadata.handoffId)
        return false;
    if (!handoff.metadata.handoffType)
        return false;
    if (!handoff.metadata.sourceMcp)
        return false;
    if (!handoff.metadata.targetMcp)
        return false;
    if (!handoff.metadata.sourceProjectPath)
        return false;
 ...
 */
export function createSuccessResponse(handoffId, result) {
    return {
        handoffId,
        success: true,
 ...
 */
export function createErrorResponse(handoffId, error) {
    return {
        handoffId,
        success: false,
        receivedAt: new Date().toISOString(),
 ...
 */
export function markHandoffReceived(filePath, handoff) {
 ...
 */
export function markHandoffCompleted(projectPath, handoff, response) {
 ...
 */
export function markHandoffFailed(projectPath, handoff, response) {
 ...
 */
export async function processHandoff(handoff, processor) {
    try {
        // Validate handoff
...
//# sourceMappingURL=handoff-receiver.js.map