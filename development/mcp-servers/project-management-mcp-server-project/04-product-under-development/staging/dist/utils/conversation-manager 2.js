...
export class ConversationManager {
     ...
     */
    static initConversation(projectPath, metadata) {
     ...
     */
    static loadConversation(projectPath, conversationId) {
     ...
     */
    static appendMessage(state, role, content) {
        state.messages.push({
            role,
            content,
     ...
     */
    static updateExtractedInfo(state, newInfo) {
    ...
    static calculateCompleteness(state) {
        ...
        if (info.assets.existing.length > 0 || info.assets.needed.length > 0)
            score += 10;
        if (info.resources.team.length > 0)
            score += 5;
    ...
    static getSetupDir(projectPath) {
        return path.join(projectPath, 'project-setup');
    }
    static getStatePath(projectPath, conversationId) {
        return path.join(this.getSetupDir(projectPath), 'conversation-state.json');
    }
    static getFirstQuestion(projectType) {
        const questions = {
    ...
    static generateConversationLog(state) {
...
//# sourceMappingURL=conversation-manager%202.js.map