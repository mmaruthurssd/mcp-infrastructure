 ...
 */
import { moveComponent, getComponent, logComponentChange, } from '../lib/component-manager.js';
...
export class MoveComponentTool {
    ...
    static execute(params) {
        try {
            ...
            // Move component
            const moveResult = moveComponent(params.projectPath, params.componentName, params.toStage);
            // Log change
            ...
            // Propagate changes
            propagateComponentChange(params.projectPath, moveResult.component.name, 'moved', fromStage, params.toStage);
...
function validateStageTransition(from, to) {
    ...
    if (!validTransitions[from].includes(to)) {
        throw new Error(`Invalid stage transition: ${from} → ${to}. Valid transitions from ${from}: ${validTransitions[from].join(', ')}`);
    }
}
function getNextActionsForStage(stage, componentName) {
    switch (stage) {
        case 'EXPLORING':
            return [
