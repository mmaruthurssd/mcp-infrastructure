...
export async function addBrainstormingIdea(args) {
    const { projectPath, sessionId, title, description, approach, pros = [], cons = [], research, estimatedEffort, feasibility, } = args;
    try {
        // Find session folder
 ...
 */
async function updateSessionOverviewWithIdea(sessionPath, ideaId, title) {
    const overviewPath = path.join(sessionPath, 'SESSION-OVERVIEW.md');
    let content = fs.readFileSync(overviewPath, 'utf-8');
    // Update "Ideas" section
    if (content.includes('*No ideas captured yet*')) {
        content = content.replace('*No ideas captured yet*', `- **${ideaId}:** ${title}`);
    }
    else {
        content = content.replace(/## Ideas\n\n([\s\S]*?)\n\n---/, `## Ideas\n\n$1\n- **${ideaId}:** ${title}\n\n---`);
    }
...
export async function recordBrainstormingDecision(args) {
    const { projectPath, sessionId, selectedIdeas, rejectedIdeas = [], rationale, tradeoffs, nextSteps = [], decidedBy, } = args;
    try {
        // Find session folder
        const sessionPath = path.join(projectPath, 'brainstorming', 'sessions', sessionId);
        if (!fs.existsSync(sessionPath)) {
 ...
 */
async function updateDecisionLog(sessionPath, decisionId, selectedIdeas, timestamp) {
 ...
 */
async function updateSessionStatus(sessionPath, status) {
    const overviewPath = path.join(sessionPath, 'SESSION-OVERVIEW.md');
    let content = fs.readFileSync(overviewPath, 'utf-8');
    // Update status
...
export const startBrainstormingSessionTool = {
    ...
    description: 'Start a new brainstorming session for exploring uncertain directions or multiple approaches',
...
export const addBrainstormingIdeaTool = {
    ...
    description: 'Add an idea or approach to a brainstorming session for exploration',
    inputSchema: z.object({
        projectPath: z.string().describe('Absolute path to the project directory'),
...
export const recordBrainstormingDecisionTool = {
    ...
    description: 'Record a decision about which brainstorming approaches to pursue',
    inputSchema: z.object({
        projectPath: z.string().describe('Absolute path to the project directory'),
...
export const getBrainstormingSessionTool = {
    ...
    description: 'Get detailed status of a brainstorming session including all ideas and decisions',
    inputSchema: z.object({
        projectPath: z.string().describe('Absolute path to the project directory'),
        sessionId: z.string().describe('Session ID'),
    }),
};
//# sourceMappingURL=brainstorming-workflow%202.js.map