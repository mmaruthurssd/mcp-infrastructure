...
export async function createMajorGoal(args) {
    const { projectPath, componentId, subAreaId, goalName, description, purpose, successCriteria = [], estimatedEffort, priority = 'Medium', owner = 'Unassigned', targetDate, } = args;
    try {
...
export async function promoteToMajorGoal(args) {
    const { projectPath, componentId, subAreaId, sourceType, sourcePath, goalName, description, purpose, successCriteria = [], estimatedEffort, priority = 'Medium', owner = 'Unassigned', targetDate, additionalNotes, } = args;
    try {
 ...
 */
function extractPotentialGoalData(content) {
 ...
 * Mark potential goal as promoted
 */
function markAsPromoted(content, goalId, componentId, subAreaId) {
 ...
 */
function findGoalFolder(projectPath, componentId, subAreaId, goalId) {
    const componentPath = path.join(projectPath, '02-goals-and-roadmap', 'components', componentId);
    let searchDir;
    if (subAreaId) {
 ...
 */
function parseGoalStatus(content) {
...
export const createMajorGoalTool = {
    ...
    description: 'Create a new major goal in a component or sub-area with folder structure',
...
export const promoteToMajorGoalTool = {
    ...
    description: 'Promote a brainstorming idea or potential goal to a major goal in the hierarchical planning system',
    inputSchema: z.object({
        projectPath: z.string().describe('Absolute path to the project directory'),
...
export const handoffToSpecDrivenTool = {
    ...
    description: 'Prepare handoff context for Spec-Driven MCP to decompose major goal into sub-goals',
    inputSchema: z.object({
        projectPath: z.string().describe('Absolute path to the project directory'),
...
export const updateMajorGoalProgressTool = {
    ...
    description: 'Update major goal progress by aggregating sub-goal completion',
    inputSchema: z.object({
        projectPath: z.string().describe('Absolute path to the project directory'),
        goalId: z.string().describe('Major goal ID'),
        componentId: z.string().describe('Component ID'),
...
export const getMajorGoalStatusTool = {
    ...
    description: 'Get detailed status view of a major goal including sub-goals, risks, and blockers',
    inputSchema: z.object({
        projectPath: z.string().describe('Absolute path to the project directory'),
        goalId: z.string().describe('Major goal ID'),
        componentId: z.string().describe('Component ID'),
...
//# sourceMappingURL=major-goal-workflow%202.js.map