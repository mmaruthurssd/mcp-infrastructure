 ...
 */
export async function updateProjectOverview(input) {
    const warnings = [];
    ...
    try {
        ...
        if (input.cascadeToComponents !== false) {
            const cascadeResults = await cascadeChanges(input.projectPath, changes, updatedData);
...
// VERSION MANAGEMENT
// ============================================================================
function determineVersionChangeType(updates) {
...
function mergeUpdates(current, updates) {
    ...
    if (updates.name)
        merged.projectName = updates.name;
    ...
    if (updates.constraints) {
        ...
        if (updates.constraints.resources) {
            if (!merged.constraints.resources)
                merged.constraints.resources = {};
    ...
    // Merge components
    if (updates.components) {
        merged.components = updates.components.map((name) => ({
            name,
...
// CASCADE UPDATES
// ============================================================================
async function cascadeChanges(projectPath, changes, updatedData) {
    const cascaded = [];
...
// PARSING UTILITIES
// ============================================================================
function parseProjectOverview(content) {
...
//# sourceMappingURL=update-project-overview%202.js.map