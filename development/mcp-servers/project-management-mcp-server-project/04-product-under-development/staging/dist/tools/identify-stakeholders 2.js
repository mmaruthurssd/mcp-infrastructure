...
export class IdentifyStakeholdersTool {
    static execute(input) {
        ...
        const explicitStakeholders = state.extractedInfo.stakeholders.map(s => ({
            name: s,
            type: this.inferStakeholderType(s),
        }));
        ...
        const stakeholders = uniqueStakeholders.map(mention => {
        ...
        const templatePath = path.join(path.dirname(new URL(import.meta.url).pathname), '../templates/project-setup/STAKEHOLDERS.md');
    ...
    // Helper Methods
    // ============================================================================
    static deduplicateStakeholders(mentions) {
        const seen = new Set();
    ...
    static categorizeStakeholder(name, projectType) {
    ...
    static assessInfluence(name, role) {
    ...
    static assessInterest(name, role) {
        const text = `${name} ${role || ''}`.toLowerCase();
    ...
    static identifyConcerns(name, conversationText) {
        const concerns = [];
    ...
    static determineCommunication(influence, interest) {
    ...
    static getStrategy(influence, interest) {
        if (influence === 'high' && interest === 'high')
    ...
    static inferStakeholderType(name) {
        return name.match(/\d+\s+/) ? 'group' : 'individual';
    }
    static formatOutput(projectName, stakeholders, matrix, filePath) {
...
//# sourceMappingURL=identify-stakeholders%202.js.map