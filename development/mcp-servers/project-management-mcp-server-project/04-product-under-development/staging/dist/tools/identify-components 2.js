 ...
 */
export async function identifyComponents(input) {
    const warnings = [];
    try {
...
function analyzeProjectOverview(content) {
...
function extractListItems(content, section) {
    const items = [];
...
function extractWorkAreas(content) {
    const areas = [];
    const patterns = [
...
// ============================================================================
function suggestComponentsQuick(projectType, analysis, maxComponents) {
    const suggestions = [];
    ...
    for (const base of baseSuggestions) {
        if (suggestions.length >= maxComponents)
            break;
...
// ============================================================================
function suggestComponentsThorough(projectType, analysis, maxComponents) {
    const suggestions = [];
...
function clusterFeatures(features, maxClusters) {
    ...
    const userFeatures = features.filter(f => /user|account|profile|login|signup|registration/i.test(f));
    ...
    const dataFeatures = features.filter(f => /data|content|record|document|file|report/i.test(f));
    ...
    // Business logic
    const businessFeatures = features.filter(f => /workflow|process|logic|rule|calculation|payment|billing/i.test(f) &&
    ...
    const integrationFeatures = features.filter(f => /integration|api|webhook|sync|import|export/i.test(f));
    ...
    // Remaining features
    const remainingFeatures = features.filter(f => !userFeatures.includes(f) &&
        !dataFeatures.includes(f) &&
...
// UTILITIES
// ============================================================================
function isComponentRelevant(componentName, analysis) {
...
function generateReasoning(componentName, analysis) {
    const name = componentName.toLowerCase();
...
function determinePriority(componentName, analysis) {
    const name = componentName.toLowerCase();
...
function extractExistingComponents(content) {
    const components = [];
...
//# sourceMappingURL=identify-components%202.js.map