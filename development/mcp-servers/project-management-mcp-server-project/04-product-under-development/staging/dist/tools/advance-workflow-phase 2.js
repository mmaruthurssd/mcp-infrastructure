...
export class AdvanceWorkflowPhaseTool {
     ...
     */
    static validatePhaseTransition(state, targetPhase) {
        const passed = [];
        const warnings = [];
        // Check current phase is complete
        const currentPhase = state.phases[state.currentPhase];
        const completedSteps = currentPhase.steps.filter((s) => s.status === 'complete').length;
...
//# sourceMappingURL=advance-workflow-phase%202.js.map