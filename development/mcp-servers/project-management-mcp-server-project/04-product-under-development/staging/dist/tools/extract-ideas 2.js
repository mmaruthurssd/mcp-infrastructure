 ...
 */
function scanMarkdownForIdeas(content) {
 ...
 */
function isPotentialIdea(line) {
    const lowerLine = line.toLowerCase();
    // Check for bullet points or numbered lists
    const isList = /^\s*[-*+]\s+/.test(line) || /^\s*\d+\.\s+/.test(line);
    // Check for action verbs
    const hasActionVerb = ACTION_VERBS.some(verb => new RegExp(`\\b${verb}\\b`, 'i').test(line));
    // Check for problem keywords
    const hasProblemKeyword = PROBLEM_KEYWORDS.some(keyword => lowerLine.includes(keyword));
    // Check for feature patterns
    const hasFeaturePattern = FEATURE_PATTERNS.some(pattern => lowerLine.includes(pattern));
    // Idea is likely if:
 ...
 */
function scoreIdeaConfidence(idea) {
    let score = 0.5; // Base score
    const lowerText = idea.text.toLowerCase();
    // Boost for action verbs (up to +0.2)
    const actionVerbCount = ACTION_VERBS.filter(verb => new RegExp(`\\b${verb}\\b`, 'i').test(idea.text)).length;
    ...
    if (idea.sectionHeading) {
        const isIdeaSection = IDEA_SECTION_HEADINGS.some(heading => idea.sectionHeading.toLowerCase().includes(heading));
        if (isIdeaSection) {
            score += 0.1;
        }
    }
 ...
 */
function generateIdeaName(ideaText) {
    // Strategy:
    ...
    // Find action verb
    const actionVerb = ACTION_VERBS.find(verb => new RegExp(`\\b${verb}\\b`, 'i').test(ideaText));
 ...
 */
function determineReasoning(idea, confidence) {
    const reasons = [];
    const lowerText = idea.text.toLowerCase();
    // Check what triggered detection
    const hasActionVerb = ACTION_VERBS.some(verb => new RegExp(`\\b${verb}\\b`, 'i').test(idea.text));
 ...
 */
function formatOutput(ideas, filePath) {
    let output = '═══════════════════════════════════════════════════════\n';
...
//# sourceMappingURL=extract-ideas%202.js.map