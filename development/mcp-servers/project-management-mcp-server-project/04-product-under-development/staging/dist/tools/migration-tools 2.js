 ...
 */
export async function analyzeProjectForMigration(params) {
    ...
    const potentialGoals = await extractGoalsFromFolder(join(projectPath, 'brainstorming', 'future-goals', 'potential-goals'));
 ...
 * Detect v0.8.0 structure markers
 */
async function detectV080Structure(projectPath) {
    const structure = {
 ...
 */
async function extractGoalsFromFolder(folderPath) {
    const goals = [];
    try {
 ...
 */
async function extractSelectedGoals(projectPath) {
 ...
 */
function parseGoalFile(filePath, content) {
 ...
 */
function determineConfidence(structure, goals) {
    let score = 0;
    if (structure.hasBrainstorming)
        score++;
    if (structure.hasPotentialGoals)
        score++;
    if (structure.hasSelectedGoals)
        score++;
    if (goals.length >= 3)
        score++;
    if (goals.length >= 10)
 ...
 */
export async function suggestGoalClustering(params) {
    ...
    const components = await clusterGoalsByKeywords(goals, numComponents);
 ...
 */
async function clusterGoalsByKeywords(goals, targetComponents) {
 ...
 */
export async function migrateToHierarchical(params) {
    ...
    try {
        ...
        for (const component of clustering.components) {
            ...
            const overviewContent = generateComponentOverview(component);
 ...
 */
async function createMigrationBackup(projectPath) {
 ...
 */
function generateComponentOverview(component) {
    return `---
type: component
 ...
 */
async function findGoalById(projectPath, goalId) {
 ...
 */
function convertGoalToV1(goal, newId, componentName) {
    return `---
type: major-goal
 ...
 */
function generateBasicProjectOverview(projectPath, clustering) {
...
${clustering.components.map((c, i) => `${i + 1}. **${c.name}** - ${c.description}`).join('\n')}
 ...
 */
async function checkV1Structure(projectPath) {
    try {
        const componentsPath = join(projectPath, '02-goals-and-roadmap', 'components');
 ...
 */
async function countMigratedGoals(projectPath) {
    let count = 0;
    try {
        const componentsPath = join(projectPath, '02-goals-and-roadmap', 'components');
 ...
 */
async function validateMetadata(projectPath) {
...
//# sourceMappingURL=migration-tools%202.js.map