 ...
 */
export function validateNaming(input) {
    const errors = [];
    const suggestions = [];
    switch (input.type) {
        case 'folder':
            ...
            if (!NAMING_CONVENTIONS.FOLDER_PATTERN.test(input.name)) {
                errors.push('Folder name must be kebab-case (lowercase letters, numbers, hyphens only)');
                suggestions.push(toKebabCase(input.name));
            }
            // Check length
            if (input.name.length > NAMING_CONVENTIONS.FOLDER_MAX_LENGTH) {
                errors.push(`Folder name exceeds maximum length of ${NAMING_CONVENTIONS.FOLDER_MAX_LENGTH} characters`);
            }
        ...
        case 'file':
            ...
            if (!NAMING_CONVENTIONS.FILE_PATTERN.test(input.name)) {
                errors.push('File name must be kebab-case with valid extension (.md, .json, .yaml, .yml, .ts, .js)');
            ...
            if (input.name.length > NAMING_CONVENTIONS.FILE_MAX_LENGTH) {
                errors.push(`File name exceeds maximum length of ${NAMING_CONVENTIONS.FILE_MAX_LENGTH} characters`);
            }
            break;
        case 'major-goal-id':
...
export function generatePath(input) {
    ...
    switch (entityType) {
        ...
        case 'component': {
            ...
            createDirectories.push(path.join(projectPath, '02-goals-and-roadmap'), path.join(projectPath, '02-goals-and-roadmap', 'components'), folderPath);
            break;
        }
        case 'sub-area': {
            ...
            folderPath = path.join(projectPath, '02-goals-and-roadmap', 'components', componentFolder, subAreaFolder);
            fileName = 'SUB-AREA-OVERVIEW.md';
            createDirectories.push(path.join(projectPath, '02-goals-and-roadmap', 'components', componentFolder), folderPath);
            break;
        }
        case 'major-goal': {
            if (!parentIds?.componentId) {
            ...
            if (parentIds.subAreaId) {
                ...
                folderPath = path.join(projectPath, '02-goals-and-roadmap', 'components', componentFolder, subAreaFolder, 'major-goals', goalFolder);
                createDirectories.push(path.join(projectPath, '02-goals-and-roadmap', 'components', componentFolder, subAreaFolder), path.join(projectPath, '02-goals-and-roadmap', 'components', componentFolder, subAreaFolder, 'major-goals'), folderPath);
            }
            else {
                // Goal directly under component
                folderPath = path.join(projectPath, '02-goals-and-roadmap', 'components', componentFolder, 'major-goals', goalFolder);
                createDirectories.push(path.join(projectPath, '02-goals-and-roadmap', 'components', componentFolder), path.join(projectPath, '02-goals-and-roadmap', 'components', componentFolder, 'major-goals'), folderPath);
            }
        ...
        case 'sub-goal': {
            ...
            if (parentIds.subAreaId) {
                const subAreaFolder = toKebabCase(parentIds.subAreaId);
                folderPath = path.join(projectPath, '02-goals-and-roadmap', 'components', componentFolder, subAreaFolder, 'major-goals', majorGoalFolder, 'sub-goals', subGoalFolder);
            }
            else {
                folderPath = path.join(projectPath, '02-goals-and-roadmap', 'components', componentFolder, 'major-goals', majorGoalFolder, 'sub-goals', subGoalFolder);
            }
        ...
        case 'task-workflow': {
            ...
            createDirectories.push(path.join(projectPath, 'temp'), path.join(projectPath, 'temp', 'workflows'), folderPath);
            break;
        }
    }
    return {
 ...
 */
export function createFileWithMetadata(input) {
    try {
 ...
 */
export function validateProjectStructure(input) {
...
//# sourceMappingURL=file-organization-tools%202.js.map