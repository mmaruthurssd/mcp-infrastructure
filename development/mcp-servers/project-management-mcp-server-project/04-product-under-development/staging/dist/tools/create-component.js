 ...
 */
import { createComponent, logComponentChange, } from '../lib/component-manager.js';
...
export class CreateComponentTool {
    ...
    static execute(params) {
        try {
            ...
            // Propagate if needed
            propagateComponentChange(params.projectPath, component.name, 'created', undefined, component.stage);
            // Generate next actions
            const nextActions = [];
            if (component.stage === 'EXPLORING') {
                nextActions.push('Add sub-components to break down this component further', 'Move to FRAMEWORK stage when structure is clear');
            }
            else if (component.stage === 'FRAMEWORK') {
                nextActions.push('Define detailed specifications', 'Move to FINALIZED when ready for implementation');
            }
            else if (component.stage === 'FINALIZED') {
                nextActions.push('Convert to goal for implementation tracking', 'Create implementation plan');
            }
            return {
                success: true,
                component,
