...
export class PrepareTaskExecutorHandoffTool {
    static async execute(input) {
        ...
        return {
            ...
            suggestedToolCall,
            message: `Task Executor handoff prepared for goal: ${goalFolder}${analysis.shouldParallelize ? ` (parallelization possible: ${analysis.estimatedSpeedup.toFixed(1)}x speedup)` : ''}${agentAssignments.length > 0 ? ` with ${agentAssignments.length} agent suggestions` : ''}`,
        };
    }
    /**
     * Extract tasks from specification file
     */
    static extractTasksFromSpec(specPath) {
     ...
     */
    static createGenericTasks(goalName) {
        return [
     ...
     */
    static async suggestAgentsForTasks(tasks) {
        const assignments = [];
        try {
     ...
     */
    static suggestAgentForTask(taskDescription, registry) {
        ...
        for (const [agentName, definition] of Object.entries(registry)) {
            ...
            const descWords = definition.description.toLowerCase().split(/\s+/);
            ...
            const specialization = definition.specialization;
...
//# sourceMappingURL=prepare-task-executor-handoff%202.js.map