...
import { calculateVelocity, parseProgressHistory, formatProgressHistory, addProgressUpdate, } from '../utils/velocity-calculator.js';
import { parseSpecDirectory } from '../utils/tasks-parser.js';
// ============================================================================
// Helper Functions
// ============================================================================
/**
 ...
 */
function autoUpdateStatus(currentStatus, newProgress, explicitStatus) {
 ...
 */
function updateGoalInFile(filePath, goalId, newProgress, newStatus, progressHistoryText, blockers, nextAction) {
    ...
    // Update progress
    updatedSection = updatedSection.replace(/\*\*Progress:\*\*\s*.+/, `**Progress:** ${newProgress}%`);
    // Update status
    updatedSection = updatedSection.replace(/\*\*Status:\*\*\s*.+/, `**Status:** ${newStatus}`);
    // Update or add progress history
    if (progressHistoryText) {
        const hasHistory = updatedSection.includes('**Progress History:**');
        if (hasHistory) {
            updatedSection = updatedSection.replace(/\*\*Progress History:\*\*\s*.+/, `**Progress History:** ${progressHistoryText}`);
        }
        else {
            // Add after Progress line
            updatedSection = updatedSection.replace(/(\*\*Progress:\*\*\s*.+)/, `$1\n\n**Progress History:** ${progressHistoryText}`);
        }
    }
    // Update blockers if provided
    if (blockers !== undefined) {
        updatedSection = updatedSection.replace(/\*\*Blockers:\*\*\s*.+/, `**Blockers:** ${blockers}`);
    }
    // Update next action if provided
    if (nextAction !== undefined) {
        updatedSection = updatedSection.replace(/\*\*Next Action:\*\*\s*.+/, `**Next Action:** ${nextAction}`);
    }
    // Update Last Updated
    const today = new Date().toISOString().split('T')[0];
    updatedSection = updatedSection.replace(/\*\*Last Updated:\*\*\s*.+/, `**Last Updated:** ${today}`);
 ...
 */
function formatOutput(goalId, goalName, previousProgress, newProgress, previousStatus, newStatus, velocity) {
    let output = '═══════════════════════════════════════════════════════\n';
...
export class UpdateGoalProgressTool {
    ...
    static execute(input) {
        try {
            ...
            const goalRegex = new RegExp(`###\\s+Goal\\s+${input.goalId}:[\\s\\S]+?(?=\\n---\\n|$)`, '');
            ...
            const { goalName, previousProgress, previousStatus } = updateGoalInFile(selectedGoalsPath, input.goalId, newProgress, newStatus, progressHistoryText, input.blockers, input.nextAction);
            ...
            return {
                ...
                formatted: formatOutput(input.goalId, goalName, previousProgress, newProgress, previousStatus, newStatus, velocity),
            };
        }
        catch (error) {
            return {
...
//# sourceMappingURL=update-goal-progress%202.js.map