 ...
 */
function parseSelectedGoals(content) {
 ...
 */
function validateGoalOrder(goalOrder, existingIds) {
 ...
 */
function updatePriorityInEntry(entryText, newPriority) {
    return entryText.replace(/\*\*Priority:\*\*\s+\w+/, `**Priority:** ${newPriority}`);
}
/**
 * Determine priority based on position in list
 */
function calculatePriorityFromPosition(index, total) {
 ...
 */
function rewriteActiveGoalsSection(content, goalMap, newOrder, updatePriorities) {
 ...
 */
function updateLastUpdatedTimestamp(content) {
    const today = new Date().toISOString().split('T')[0];
    return content.replace(/\*\*Last Updated:\*\*\s+.+/, `**Last Updated:** ${today}`);
}
/**
 ...
 */
function formatOutput(beforeOrder, afterOrder, prioritiesUpdated) {
    let output = '═══════════════════════════════════════════════════════\n';
...
export class ReorderSelectedGoalsTool {
    ...
    static execute(input) {
        try {
            ...
            let updatedContent = rewriteActiveGoalsSection(content, goalMap, input.goalOrder, input.updatePriorities || false);
...
//# sourceMappingURL=reorder-selected-goals%202.js.map