...
function getTelemetryFilePath(brainPath, eventType) {
...
function filterEvents(events, filters, timeRange) {
 ...
 */
export async function logEvent(args, brainPath) {
 ...
 */
export async function queryEvents(args, brainPath) {
    const startTime = Date.now();
    try {
 ...
 */
export async function getEventStats(args, brainPath) {
    try {
        const allEvents = readAllEvents(brainPath);
        const filteredEvents = filterEvents(allEvents, args.filters, args.filters?.time_range);
        let stats = {
            success: true,
            metric: args.metric
        };
...
function getGroupKey(event, groupBy) {
