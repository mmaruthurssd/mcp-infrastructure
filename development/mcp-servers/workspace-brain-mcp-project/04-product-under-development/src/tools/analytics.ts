...
function filterEventsByTimeRange(events: Event[], start: Date, end: Date): Event[] {
 ...
 */
export async function generateWeeklySummary(args: any, brainPath: string) {
  try {
 ...
 */
export async function getAutomationOpportunities(args: any, brainPath: string) {
  try {
    // Read events
    ...
    opportunities.sort((a, b) => {
      ...
      if (sortBy === "time_savings") return b.estimated_savings_hours - a.estimated_savings_hours;
 ...
 */
export async function getToolUsageStats(args: any, brainPath: string) {
  try {
    // Read events
    let events = readAllEvents(brainPath);
    ...
    let combinations: any[] | undefined;
 ...
 */
export async function analyzeWorkflowEfficiency(args: any, brainPath: string) {
  try {
    // Read events
    let events = readAllEvents(brainPath);
    ...
    workflowEvents.forEach(event => {
      ...
      // Count outcomes
      if (event.data.outcome === "completed") workflows[name].completed++;
      else if (event.data.outcome === "failed") workflows[name].failed++;
      else if (event.data.outcome === "blocked") workflows[name].blocked++;
 ...
 */
export async function identifyHighValueAutomations(args: any, brainPath: string) {
  try {
    ...
    const highValueTargets = opportunitiesData.opportunities.map((opp: any) => {
      // ROI = (time_savings * $value_per_hour) / implementation_effort_hours
      const implementationHours =
        opp.implementation.effort === "low" ? 2 :
          ...
          text: JSON.stringify(
            {
              ...
              summary: {
                very_high_value: highValueTargets.filter((t: any) => t.value_category === "very-high").length,
                high_value: highValueTargets.filter((t: any) => t.value_category === "high").length,
                medium_value: highValueTargets.filter((t: any) => t.value_category === "medium").length,
                low_value: highValueTargets.filter((t: any) => t.value_category === "low").length
              }
            },
            null,
            2
 ...
 */
export async function generateInsightsReport(args: any, brainPath: string) {
  try {
    // Read events
    let events = readAllEvents(brainPath);
    ...
    taskEvents.forEach(e => {
      (e.data.tools_used || []).forEach((tool: string) => {
    ...
    if (repeatedPatterns.length > 0) {
      insights.push({
        category: "automation",
        insight: `Found ${repeatedPatterns.length} repeated task patterns (≥3 occurrences each)`,
 ...
 */
export async function trackGoalVelocity(args: any, brainPath: string) {
  try {
    // Read events
    let events = readAllEvents(brainPath);
    ...
    const velocity = Object.entries(goals).map(([goalId, data]) => {
      ...
      return {
        ...
        recommendation:
          velocityTrend === "stalled" ? "Goal may be blocked - review and re-prioritize" :
 ...
 */
export async function analyzeMcpUsagePatterns(args: any, brainPath: string) {
  try {
    // Read events
    let events = readAllEvents(brainPath);
    ...
    mcpEvents.forEach(event => {
      ...
      tools.forEach((tool: string) => {
        ...
        if (event.data.outcome === "completed") mcpStats[tool].success_count++;
        if (event.data.outcome === "failed") mcpStats[tool].failure_count++;
    ...
    const analysis = Object.entries(mcpStats).map(([tool, stats]) => {
      ...
      return {
        ...
        recommendation:
          successRate < 75 ? "High failure rate - review error patterns" :
 ...
 */
export async function createCustomDashboard(args: any, brainPath: string) {
  try {
    ...
    switch (dashboardType) {
      case "productivity":
        ...
        dashboard = {
          ...
          top_task_types: Object.entries(
            taskEvents.reduce((acc: Record<string, number>, e) => {
              const type = e.data.task_type || "unknown";
      ...
      case "goals":
        ...
        const goalGroups = goalEvents.reduce((acc: Record<string, number>, e) => {
