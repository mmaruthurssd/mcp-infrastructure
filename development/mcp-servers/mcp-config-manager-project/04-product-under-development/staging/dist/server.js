...
import { parseConfigurationGuide, validateServerConfig, } from "./guide-parser.js";
...
function getConfigPaths(workspaceRoot) {
    ...
    if (platform === "darwin") {
        ...
        claudeCodePath = path.join(homeDir, "Library", "Application Support", "Claude", "claude_desktop_config.json");
        vsCodePath = path.join(homeDir, "Library", "Application Support", "Code", "User", "globalStorage", "saoudrizwan.claude-dev", "settings", "cline_mcp_settings.json");
    }
    else if (platform === "win32") {
        // Windows
        // Note: claude_desktop_config.json is CLAUDE CODE's global config
        claudeCodePath = path.join(homeDir, "AppData", "Roaming", "Claude", "claude_desktop_config.json");
        vsCodePath = path.join(homeDir, "AppData", "Roaming", "Code", "User", "globalStorage", "saoudrizwan.claude-dev", "settings", "cline_mcp_settings.json");
    }
    return {
...
function getLocalInstances(workspaceRoot) {
    const instancesDir = path.join(workspaceRoot, "local-instances", "mcp-servers");
...
function checkServerBuilt(workspaceRoot, serverName) {
    const serverPath = path.join(workspaceRoot, "local-instances", "mcp-servers", serverName, "dist", "server.js");
    return fs.existsSync(serverPath);
}
function detectScope(workspaceRoot, serverName) {
    const serverPath = path.join(workspaceRoot, "local-instances", "mcp-servers", serverName);
...
function getServerConfig(serverName, globalConfig, workspaceConfig, claudeCodeConfig, vsCodeConfig) {
...
async function syncMCPConfigs(workspaceRoot) {
    ...
    const globalServers = new Set(Object.keys(globalConfig?.mcpServers || {}));
    const workspaceServers = new Set(Object.keys(workspaceConfig?.mcpServers || {}));
    const claudeCodeServers = new Set(Object.keys(claudeCodeConfig?.mcpServers || {}));
    const vsCodeServers = new Set(Object.keys(vsCodeConfig?.mcpServers || {}));
    ...
    for (const instanceName of localInstances) {
        if (!allRegistered.has(instanceName)) {
            ...
            report.push(`  ⚠️  ${instanceName} (not registered, suggest: ${suggestedScope} scope, built: ${built})`);
            hasMissing = true;
        }
    }
    if (!hasMissing) {
    ...
    for (const serverName of allRegistered) {
        ...
        if (locations.length > 1) {
            ...
            const uniquePaths = new Set(configs.map((c) => c.path.replace(/\$\{workspaceFolder\}/g, "")));
            if (uniquePaths.size > 1) {
                report.push(`     ❌ PATHS DIFFER - This WILL cause loading issues!`);
                report.push(`     Claude Code may only load from one location, others will fail.`);
            }
            else {
                report.push(`     ⚠️  Paths are same but multiple registrations can cause issues.`);
            }
            report.push(`     Recommendation: Keep in ONE location only. Remove from others.`);
            if (locations.includes("claude-code")) {
                report.push(`     Suggested: Keep in claude-code (Claude Code's primary config), remove from workspace/global.`);
            }
            else if (locations.includes("vscode")) {
                report.push(`     Suggested: Keep in vscode OR claude-code depending on which you use more.`);
            }
            else {
                report.push(`     Suggested: Keep in claude-code (for Claude Code) or workspace (for project-specific).`);
            }
            hasConflicts = true;
        }
    }
    ...
    for (const serverName of allRegistered) {
        ...
        const serverConfig = getServerConfig(serverName, globalConfig, workspaceConfig, claudeCodeConfig, vsCodeConfig);
        if (serverConfig) {
            const issues = validateServerConfig(workspaceRoot, serverName, serverConfig, requirements).filter((issue) => issue.category === "env-var");
            if (issues.length > 0) {
    ...
    for (const serverName of allRegistered) {
        ...
        const serverConfig = getServerConfig(serverName, globalConfig, workspaceConfig, claudeCodeConfig, vsCodeConfig);
        if (serverConfig) {
            const issues = validateServerConfig(workspaceRoot, serverName, serverConfig, requirements).filter((issue) => issue.category === "config-dir");
            if (issues.length > 0) {
                report.push(`  ⚠️  ${serverName}:`);
                for (const issue of issues) {
                    report.push(`     ${issue.message}`);
    ...
    if (!hasDirIssues) {
        report.push("  ✓ All required configuration directories exist (or servers don't need them)");
    }
    report.push("");
    ...
    for (const serverName of allRegistered) {
        ...
        if (serverConfig) {
            const requirements = guideRequirements.get(serverName);
            if (requirements) {
                const issues = validateServerConfig(workspaceRoot, serverName, serverConfig, requirements).filter((issue) => issue.category === "path-portability");
                if (issues.length > 0) {
                    report.push(`  ⚠️  ${serverName}:`);
                    for (const issue of issues) {
                        report.push(`     ${issue.message}`);
    ...
    const warnings = [];
    if (hasConflicts) {
        warnings.push("CONFLICTS - Servers registered in multiple locations will fail to load");
    }
    if (hasEnvIssues) {
...
async function registerMCPServer(workspaceRoot, serverName, scope) {
    const configPaths = getConfigPaths(workspaceRoot);
    ...
    if (requirements) {
        ...
        if (warnings.length > 0) {
            ...
            warnings.push(`   See MCP_CONFIGURATION_GUIDE.md for the recommended configuration.\n`);
        }
        if (warnings.length > 0) {
    ...
    const configPath = targetScope === "user" ? configPaths.global : configPaths.workspace;
    // Read config
...
async function unregisterMCPServer(workspaceRoot, serverName) {
    const configPaths = getConfigPaths(workspaceRoot);
    const results = [];
    // Check global config
    const globalConfig = readConfig(configPaths.global);
...
async function listMCPServers(workspaceRoot) {
    const configPaths = getConfigPaths(workspaceRoot);
    const globalConfig = readConfig(configPaths.global);
    const workspaceConfig = readConfig(configPaths.workspace);
    const localInstances = getLocalInstances(workspaceRoot);
    ...
    // Sort servers
    const sortedServers = Array.from(serverMap.entries()).sort(([a], [b]) => a.localeCompare(b));
    // Generate report
...
async function validateMCPConfiguration(workspaceRoot, serverName) {
    const configPaths = getConfigPaths(workspaceRoot);
    const globalConfig = readConfig(configPaths.global);
    const workspaceConfig = readConfig(configPaths.workspace);
    const claudeCodeConfig = configPaths.claudeCode
    ...
    if (serverName) {
        ...
        // Run validation
        const issues = validateServerConfig(workspaceRoot, serverName, serverConfig, requirements);
        ...
        report.push(`Guide reference: MCP_CONFIGURATION_GUIDE.md:${requirements.guideSection.startLine}-${requirements.guideSection.endLine}\n`);
        // Report issues
        if (errors.length > 0) {
            report.push("** Errors **");
        ...
        for (const name of allRegistered) {
            ...
            const serverConfig = getServerConfig(name, globalConfig, workspaceConfig, claudeCodeConfig, vsCodeConfig);
            if (serverConfig) {
                const issues = validateServerConfig(workspaceRoot, name, serverConfig, requirements);
                const errors = issues.filter((i) => i.severity === "error");
                const warnings = issues.filter((i) => i.severity === "warning");
        ...
        tools: [
            {
                ...
                description: "Scan local MCP server instances and compare with config files. Reports orphaned configs, missing registrations, and build status issues.",
                inputSchema: {
                    type: "object",
                    properties: {},
                    required: [],
            ...
            },
            {
                ...
                description: "Register a local MCP server instance in the appropriate config file. Auto-detects scope (user vs project) based on server characteristics.",
                inputSchema: {
                    type: "object",
                    properties: {
                        serverName: {
                            type: "string",
                            description: "Name of the server directory in local-instances/mcp-servers/",
                        },
                        scope: {
                            type: "string",
                            enum: ["user", "project"],
                            description: "Optional: Force specific scope. If not provided, scope is auto-detected.",
                        },
                    },
                    required: ["serverName"],
                },
            },
            {
                ...
                description: "Remove an MCP server from all configuration files (both global and workspace configs).",
                inputSchema: {
                    type: "object",
                    properties: {
                        serverName: {
            ...
            },
            {
                ...
                description: "List all MCP servers with their registration scopes, build status, and existence in local instances.",
                inputSchema: {
                    type: "object",
                    properties: {},
                    required: [],
            ...
            },
            {
                ...
                description: "Validate MCP server configuration against MCP_CONFIGURATION_GUIDE.md requirements. Checks environment variables, config directories, and path portability. Can validate a specific server or all servers.",
                inputSchema: {
                    type: "object",
                    properties: {
                        serverName: {
                            type: "string",
                            description: "Optional: Name of server to validate. If not provided, validates all registered servers.",
                        },
                    },
                    required: [],
                },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (request.params.name) {
        ...
        case "register_mcp_server": {
            const { serverName, scope } = request.params.arguments;
            return {
                content: [
                    {
                        type: "text",
