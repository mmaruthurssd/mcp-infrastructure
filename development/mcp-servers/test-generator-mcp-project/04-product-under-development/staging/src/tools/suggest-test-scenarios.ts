import type { SuggestTestScenariosParams, SuggestTestScenariosResult, TestSuggestion, ScenarioType } from '../types/index.js';
 ...
 */
export async function suggestTestScenarios(params: SuggestTestScenariosParams): Promise<SuggestTestScenariosResult> {
  try {
    // Validate parameters
 ...
 */
function generateSuggestionsForFunction(
  func: FunctionInfo,
    ...
    for (const param of func.params) {
      suggestions.push({
        ...
        inputs: func.params.map(p =>
          p.name === param.name ? generateSampleInput(getOppositeType(p.type), 'valid') : generateSampleInput(p.type, 'valid')
        ),
  ...
  if (scenarioTypes.includes('security')) {
    if (context?.toLowerCase().includes('auth') || context?.toLowerCase().includes('password')) {
      suggestions.push({
        ...
        functionName: func.name,
        inputs: func.params.map(p =>
          p.type === 'string' ? "'; DROP TABLE users--" : generateSampleInput(p.type, 'valid')
        ),
 ...
 */
function generateSampleInput(type: string, validity: 'valid' | 'invalid' | 'large'): any {
  if (validity === 'valid') {
    switch (type) {
      case 'string':
        return 'test';
 ...
 */
function generateExpectedOutput(returnType: string): any {
  switch (returnType) {
    case 'string':
      return 'string value';
    case 'number':
