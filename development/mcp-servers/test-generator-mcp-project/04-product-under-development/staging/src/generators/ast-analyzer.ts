...
import type { FunctionInfo, ClassInfo, ParameterInfo, PropertyInfo } from '../types/index.js';
 ...
 */
export async function analyzeSourceFile(filePath: string): Promise<{
  ...
  try {
    ...
    return {
      functions: [],
      classes: [],
      error: `Failed to parse source file: ${(error as Error).message}`,
    };
  }
}
 ...
 */
function extractFunctionInfo(node: any, path: any, name?: string): FunctionInfo | null {
  try {
    ...
    // Extract parameters
    const params: ParameterInfo[] = (node.params || []).map((param: any) => {
 ...
 */
function extractClassInfo(node: any, path: any): ClassInfo | null {
  try {
    ...
    for (const item of node.body.body) {
      if (item.type === 'ClassMethod') {
        const methodInfo: FunctionInfo = {
          name: item.key.name,
          params: (item.params || []).map((p: any) => extractParameterInfo(p)),
...
function extractDocComment(path: any): string | undefined {
  ...
  const jsdocComment = leadingComments.find((c: any) => c.type === 'CommentBlock');
 ...
 */
export function filterFunctionsByName(functions: FunctionInfo[], names: string[]): FunctionInfo[] {
 ...
 */
export function getExportedFunctions(functions: FunctionInfo[]): FunctionInfo[] {
