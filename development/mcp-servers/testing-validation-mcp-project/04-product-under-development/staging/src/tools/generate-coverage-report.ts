...
import { CoverageReporter, ReportFormat } from '../utils/coverage-reporter.js';
...
export class GenerateCoverageReportTool {
   ...
   * Execute the tool
   */
  static async execute(input: GenerateCoverageReportInput): Promise<GenerateCoverageReportOutput> {
    try {
      // Validate input
      if (!input.mcpPath) {
        return {
      ...
      const format = (input.format as ReportFormat) || 'text';
