...
import type {
  ValidateMcpImplementationOutput,
  CategoryResults,
  ValidationIssue,
  ValidationSeverity,
} from '../types.js';
...
export class StandardsValidator {
   ...
   */
  async validate(): Promise<ValidateMcpImplementationOutput> {
    try {
   ...
   */
  private validateFileStructure(): CategoryResults {
   ...
   */
  private validateNaming(): CategoryResults {
    const issues: ValidationIssue[] = [];
    let passed = 0;
    let failed = 0;

    // Check file naming conventions
    const checkNaming = (dirPath: string, depth: number = 0): void => {
   ...
   */
  private validateDocumentation(): CategoryResults {
    const issues: ValidationIssue[] = [];
    let passed = 0;
    let failed = 0;
    ...
    const checkMarkdownFiles = (dirPath: string): void => {
      try {
        const entries = readdirSync(dirPath);
   ...
   */
  private validateCode(): CategoryResults {
    const issues: ValidationIssue[] = [];
    let passed = 0;
    let failed = 0;
   ...
   */
  private validateMcpStandards(): CategoryResults {
    const issues: ValidationIssue[] = [];
    let passed = 0;
    let failed = 0;
   ...
   */
  private generateRecommendations(results: Record<string, CategoryResults>): string[] {
  ...
  private createEmptyCompliance() {
    const emptyCategory: CategoryResults = { passed: 0, failed: 0, score: 0, issues: [] };
    return {
      overall: 0,
      categories: {
