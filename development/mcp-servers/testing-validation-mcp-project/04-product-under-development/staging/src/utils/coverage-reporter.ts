...
import type { CoverageReport, FileCoverage, GenerateCoverageReportOutput } from '../types.js';
...
export class CoverageReporter {
  ...
  constructor(mcpPath: string, format: ReportFormat = 'text') {
   ...
   */
  async generate(outputPath?: string): Promise<GenerateCoverageReportOutput> {
    try {
   ...
   */
  private parseCoverage(coveragePath: string): CoverageReport | undefined {
    try {
      ...
      for (const [filePath, data] of Object.entries(coverageData)) {
        ...
        const stmtCovered = Object.values(fileData.s || {}).filter((v) => (v as number) > 0).length;
        ...
        const branchCovered = Object.values(fileData.b || {}).filter((branches) =>
          Array.isArray(branches) ? (branches as number[]).some((count) => count > 0) : false
        ).length;
        ...
        const fnCovered = Object.values(fileData.f || {}).filter((v) => (v as number) > 0).length;
   ...
   */
  private generateTextReport(coverage: CoverageReport): string {
    const lines: string[] = [];
...
<body>
   ...
   */
  private generateJsonReport(coverage: CoverageReport): string {
   ...
   * Save report to file
   */
  private saveReport(content: string, outputPath: string, extension: string): string {
    try {
   ...
   * Create error result
   */
  private createErrorResult(error: string): GenerateCoverageReportOutput {
    return {
      success: false,
      format: this.format,
      error,
