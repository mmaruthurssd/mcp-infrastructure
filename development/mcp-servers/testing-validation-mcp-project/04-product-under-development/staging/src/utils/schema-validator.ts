...
import type { ValidateToolSchemaOutput, ToolSchemaValidation, ToolSchemaIssue, ToolParameter } from '../types.js';
...
export class SchemaValidator {
   ...
   */
  async validate(): Promise<ValidateToolSchemaOutput> {
    try {
      ...
      const tools: ToolSchemaValidation[] = [];
   ...
   */
  private extractToolDefinitions(serverContent: string): any[] {
    const tools: any[] = [];
    ...
    try {
      ...
      for (let i = 0; i < toolObjects.length; i++) {
        ...
        if (i > 0) toolStr = '{' + toolStr;
        if (i < toolObjects.length - 1) toolStr = toolStr + '}';
   ...
   */
  private extractObject(str: string): string {
    let depth = 0;
    let start = -1;
    let end = -1;
   ...
   */
  private parseSchemaString(schemaStr: string): any {
    try {
   ...
   */
  private validateToolSchema(tool: any): ToolSchemaValidation {
   ...
   * Validate schema structure
   */
  private validateSchemaStructure(schema: any, issues: ToolSchemaIssue[]): void {
   ...
   */
  private extractParameters(schema: any, parameters: ToolParameter[], issues: ToolSchemaIssue[]): void {
   ...
   * Create error result
   */
  private createErrorResult(error: string): ValidateToolSchemaOutput {
    return {
      success: false,
      tools: [],
      error,
