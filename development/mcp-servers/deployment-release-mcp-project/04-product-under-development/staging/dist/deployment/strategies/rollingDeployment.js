...
export class RollingDeploymentStrategy extends BaseDeploymentStrategy {
    async deploy() {
        ...
        try {
            ...
            logs.push(`Deploying with parallelism: ${parallelism} (updating ${parallelism} instances at a time)`);
            ...
            for (let i = 0; i < totalInstances; i += parallelism) {
                ...
                logs.push(await this.executeCommand(`echo "Updating instances ${i + 1} to ${i + batch}"`, `Rolling update batch ${Math.floor(i / parallelism) + 1}`));
...
//# sourceMappingURL=rollingDeployment.js.map