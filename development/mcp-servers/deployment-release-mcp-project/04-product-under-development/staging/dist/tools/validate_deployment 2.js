...
export async function validateDeployment(args) {
    const { environment, runSmokeTests = true, runHealthChecks = true, checkIntegrations = false } = args;
    try {
...
function formatValidationResult(result, environment) {
    const passIcon = (passed) => passed ? '✅' : '❌';
...
Checks:
${result.checks.map(check => `  ${passIcon(check.passed)} ${severityIcon(check.severity)} ${check.name}: ${check.message}`).join('\n')}
