...
import { parseCommit, categorizeCommits, extractContributors, collectBreakingChanges, } from "../release-notes/commitParser.js";
...
export async function generateReleaseNotes(params) {
    const { projectPath, fromVersion, toVersion = "HEAD", format = "markdown", includeBreakingChanges = true, includeAuthors = true, outputPath, sections, } = params;
    // Initialize git
    ...
    try {
        ...
        commits = log.all.map((commit) => {
            return parseCommit(commit.message, commit.hash, commit.author_name, commit.date);
        });
    }
    catch (error) {
    ...
    const finalOutputPath = outputPath || path.join(projectPath, `RELEASE_NOTES.${fileExtension}`);
    // Get release date
 ...
 */
async function getCurrentVersion(git) {
    try {
