export class DependencyResolver {
        ...
        for (const service of services) {
            if (service.dependencies && service.dependencies.length > 0) {
                for (const dep of service.dependencies) {
                    ...
                    if (!nodes.has(dep)) {
                        throw new Error(`Service '${service.name}' depends on '${dep}', but '${dep}' is not in the service list`);
                    }
    ...
    topologicalSort(graph) {
        ...
        let currentBatch = Array.from(graph.nodes.keys()).filter((node) => inDegree.get(node) === 0);
        ...
        if (order.length !== graph.nodes.size) {
            throw new Error("Topological sort failed: not all services were processed");
        }
    ...
    validateDependencies(services) {
            ...
            if (service.dependencies && service.dependencies.length > 0) {
                for (const dep of service.dependencies) {
                    if (!serviceNames.has(dep)) {
                        errors.push(`Service '${service.name}' depends on '${dep}', but '${dep}' is not defined`);
                    }
                }
        ...
        if (services.length > 1) {
            const hasDependencies = new Set(services
                .filter((s) => s.dependencies && s.dependencies.length > 0)
                .map((s) => s.name));
            const isDependency = new Set();
            for (const service of services) {
            ...
            for (const service of services) {
                if (!hasDependencies.has(service.name) &&
                    !isDependency.has(service.name)) {
                    warnings.push(`Service '${service.name}' has no dependencies and is not a dependency of any other service`);
                }
            }
        }
        return {
     ...
     */
    getDeploymentOrder(services, strategy) {
        switch (strategy) {
            case "sequential":
