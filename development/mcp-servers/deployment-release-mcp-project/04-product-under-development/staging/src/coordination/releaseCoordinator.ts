...
export class ReleaseCoordinator {
  ...
  private releaseRegistry: ReleaseRegistry;
  ...
  async coordinateRelease(
    params: CoordinateReleaseParams
  ): Promise<CoordinateReleaseResult> {
    const {
      projectPath,
      releaseName,
      environment,
    ...
    console.error(
      `[ReleaseCoordinator] Starting release '${releaseName}' (${releaseId}) to ${environment}`
    );
    console.error(
      `[ReleaseCoordinator] Strategy: ${strategy}, Services: ${services.length}`
    );
    ...
    try {
      ...
      if (validation.warnings.length > 0) {
        console.error(
          `[ReleaseCoordinator] Warnings:\n${validation.warnings.join("\n")}`
        );
      }

      // Step 2: Determine deployment order
      console.error(`[ReleaseCoordinator] Computing deployment order...`);
      const deploymentBatches =
        this.dependencyResolver.getDeploymentOrder(services, strategy);

      const deploymentOrder = deploymentBatches.flat();

      console.error(
        `[ReleaseCoordinator] Deployment order: ${deploymentOrder.join(" -> ")}`
      );
      console.error(
        `[ReleaseCoordinator] Batches: ${deploymentBatches.length}`
      );
      ...
      const releaseRecord = {
        ...
        releaseNotesPath: this.generateReleaseNotesPath(
          projectPath,
          releaseName,
          environment
        ),
      ...
      for (let batchIndex = 0; batchIndex < deploymentBatches.length; batchIndex++) {
        ...
        console.error(
          `[ReleaseCoordinator] Deploying batch ${batchIndex + 1}/${deploymentBatches.length}: ${batch.join(", ")}`
        );

        // Deploy services in batch (parallel or sequential based on strategy)
        const batchResults = await this.deployBatch(
          projectPath,
          environment,
          batch,
          services,
        ...
        if (batchFailed) {
          ...
          console.error(
            `[ReleaseCoordinator] Batch ${batchIndex + 1} failed, stopping deployment`
          );

          // Track which services were deployed before failure
          deployedServices.push(
            ...batchResults.filter((r) => r.status === "success").map((r) => r.service)
          );

          // Handle rollback if enabled
          if (rollbackOnFailure && deployedServices.length > 0) {
            console.error(
              `[ReleaseCoordinator] Initiating rollback for ${deployedServices.length} deployed services`
            );

            const rollbackResults = await this.rollbackServices(
              projectPath,
              environment,
              deployedServices,
              services,
              `Release ${releaseName} failed during deployment`
            );
            ...
            for (const rollbackResult of rollbackResults) {
              const serviceResult = serviceResults.find(
                (r) => r.service === rollbackResult.service
              );
              if (serviceResult) {
      ...
      if (notifyChannels.length > 0) {
        console.error(
          `[ReleaseCoordinator] Notifications would be sent to: ${notifyChannels.join(", ")}`
        );
      }

      // Step 8: Build result
      const result: CoordinateReleaseResult = {
      ...
      console.error(
        `[ReleaseCoordinator] Release ${overallSuccess ? "succeeded" : "failed"} in ${duration}ms`
      );
      ...
      try {
        ...
        console.error(
          `[ReleaseCoordinator] Failed to update registry: ${updateError}`
        );
      }
      ...
      return {
        ...
        serviceResults: [],
        overallHealth: "unhealthy",
        releaseNotes: this.generateReleaseNotesPath(
          projectPath,
          releaseName,
          environment
        ),
  ...
  private async deployBatch(
    ...
    if (parallel) {
      ...
      const promises = batch.map((serviceName) =>
        this.deployService(projectPath, environment, serviceName, allServices)
      );
  ...
  private async deployService(
    ...
    try {
      ...
      console.error(
        `[ReleaseCoordinator] Service '${serviceName}' deployment ${result.status} in ${duration}ms`
      );
      ...
      console.error(
        `[ReleaseCoordinator] Service '${serviceName}' deployment failed: ${errorMessage}`
      );
   ...
   */
  private async rollbackServices(
    projectPath: string,
    environment: "staging" | "production",
    servicesToRollback: string[],
    allServices: ServiceConfig[],
    reason: string
  ): Promise<ServiceResult[]> {
    console.error(
      `[ReleaseCoordinator] Rolling back ${servicesToRollback.length} services`
    );
    ...
    for (const serviceName of reversedServices) {
      ...
      if (!service) {
        console.error(
          `[ReleaseCoordinator] Service '${serviceName}' not found, skipping rollback`
        );
        continue;
      }
      ...
      try {
        console.error(
          `[ReleaseCoordinator] Rolling back service '${serviceName}'...`
        );
        ...
        console.error(
          `[ReleaseCoordinator] Service '${serviceName}' rollback ${rollbackResult.success ? "succeeded" : "failed"} in ${duration}ms`
        );
      } catch (error) {
        const duration = Date.now() - startTime;
        console.error(
          `[ReleaseCoordinator] Service '${serviceName}' rollback failed: ${error}`
        );
   ...
   */
  private calculateOverallHealth(serviceResults: ServiceResult[]): HealthStatus {
