 ...
 */
function getChecksToRun(requestedChecks?: string[]): Set<string> {
 ...
 */
function calculateOverallHealth(checks: ValidationCheck[]): HealthStatus {
  ...
  const criticalFailed = checks.some(
    c => c.status === 'failed' && (c.category === 'service-health' || c.category === 'data')
  );
  if (criticalFailed) {
    return 'unhealthy';
  }
...
function determineRecommendation(
  ...
  const criticalFailed = failed.filter(c =>
    c.category === 'service-health' || c.category === 'data'
  );
...
): Promise<ValidateDeploymentResult> {
  const {
    projectPath,
    environment,
    deploymentId,
