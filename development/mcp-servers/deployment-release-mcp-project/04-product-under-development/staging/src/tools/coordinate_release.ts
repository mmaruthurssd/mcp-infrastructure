...
export async function coordinateRelease(args: any) {
  const {
    systems,
    version,
    environment,
...
function formatReleaseResult(result: any): string {
  const icon = (success: boolean) => success ? '✅' : '❌';
...
${result.results.map((r: any) =>
  `  ${icon(r.success)} ${r.system}: ${r.message}`
).join('\n')}
