  ...
  try {
      ...
      try {
        ...
        if (environment === 'production') {
          if (!result.compliant) {
            ...
            throw new Error(
              `Production deployment blocked: ${result.summary.criticalViolations} critical violations`
            );
          }
          ...
          if (result.summary.complianceScore < 90) {
            ...
            throw new Error(
              `Compliance score ${result.summary.complianceScore}/100 below production threshold (90)`
            );
          }
    ...
    if (preChecks) {
      ...
      if (preChecksFailed > 0 && !dryRun) {
        ...
        if (criticalFailed) {
          const deploymentStatus: DeploymentStatus = "failed";
    ...
    const deploymentStatus: DeploymentStatus = deploymentResult.success ? "success" : "failed";
    const deploymentRecord: DeploymentRecord = {
