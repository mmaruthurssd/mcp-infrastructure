import type { HealthStatus, MonitorDeploymentHealthParams, MonitorDeploymentHealthResult } from '../types.js';
...
export class HealthMonitor {
   ...
   */
  async monitor(params: MonitorDeploymentHealthParams): Promise<MonitorDeploymentHealthResult> {
    const {
      projectPath,
      environment,
  ...
  private calculateTrend(
    ...
    const errorRateChange = this.calculatePercentChange(
      firstAvg.errorRate,
      secondAvg.errorRate
    );
    const responseTimeChange = this.calculatePercentChange(
      firstAvg.avgResponseTime,
      secondAvg.avgResponseTime
    );
    const cpuChange = this.calculatePercentChange(
      firstAvg.cpuUsage,
      secondAvg.cpuUsage
    );
    const memoryChange = this.calculatePercentChange(
      firstAvg.memoryUsage,
      secondAvg.memoryUsage
    );
   ...
   */
  private calculateAverageMetrics(snapshots: MetricsSnapshot[]) {
