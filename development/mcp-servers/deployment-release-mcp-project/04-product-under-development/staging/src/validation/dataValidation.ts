...
export async function checkMigrations(
  projectPath: string,
  migrationPaths: string[] = ["migrations", "db/migrations"]
): Promise<ValidationCheck> {
  const startTime = Date.now();
...
async function checkMigrationStatus(
  projectPath: string,
  migrationPaths: string[]
): Promise<{ allApplied: boolean; totalMigrations: number; pendingMigrations: number }> {
  ...
  for (const migrationPath of migrationPaths) {
    ...
    try {
      ...
      const migrationFiles = files.filter(f =>
        f.endsWith('.sql') || f.endsWith('.js') || f.endsWith('.ts')
      );
...
export async function runDataValidation(
  ...
  if (config.databaseConnections && config.databaseConnections.length > 0) {
    const connectionChecks = await checkDatabaseConnections(
