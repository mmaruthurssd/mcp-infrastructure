...
export class BackupEngine {
    ...
    async createBackup(params) {
        ...
        for (const source of params.sources) {
            try {
                await this.backupSource(source, backupId, metadata, stats, params.excludePatterns || [], params.compression ?? true, lastBackupTime);
            }
            catch (error) {
                BackupMetadataHelper.addError(metadata, source, `Failed to backup source: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
            }
        }
        ...
        if (params.verify) {
            ...
            if (!verified) {
                BackupMetadataHelper.addError(metadata, backupId, 'Backup verification failed', 'warning');
            }
        }
        // Add to index
     ...
     */
    async backupSource(source, backupId, metadata, stats, excludePatterns, compression, lastBackupTime) {
        const sourceStat = await stat(source);
        if (sourceStat.isFile()) {
            await this.backupFile(source, source, backupId, metadata, stats, compression, lastBackupTime);
        }
        else if (sourceStat.isDirectory()) {
            await this.backupDirectory(source, source, backupId, metadata, stats, excludePatterns, compression, lastBackupTime);
        }
    }
    /**
     * Backup a directory recursively
     */
    async backupDirectory(dirPath, basePath, backupId, metadata, stats, excludePatterns, compression, lastBackupTime) {
        ...
        for (const entry of entries) {
            ...
            try {
                if (entry.isFile()) {
                    await this.backupFile(fullPath, basePath, backupId, metadata, stats, compression, lastBackupTime);
                }
                else if (entry.isDirectory()) {
                    await this.backupDirectory(fullPath, basePath, backupId, metadata, stats, excludePatterns, compression, lastBackupTime);
                }
            }
            catch (error) {
                BackupMetadataHelper.addError(metadata, fullPath, `Failed to backup: ${error instanceof Error ? error.message : 'Unknown error'}`, 'warning');
                stats.filesSkipped++;
            }
        }
    }
     ...
     */
    async backupFile(filePath, basePath, backupId, metadata, stats, compression, lastBackupTime) {
     ...
     */
    async getLastBackupTimestamp(sources) {
     ...
     */
    async quickVerifyBackup(backupId, metadata) {
        try {
            // Load manifest
...
//# sourceMappingURL=BackupEngine.js.map