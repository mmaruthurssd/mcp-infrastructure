...
import { CreateBackupTool } from './tools/createBackup.js';
import { RestoreBackupTool } from './tools/restoreBackup.js';
import { ListBackupsTool } from './tools/listBackups.js';
import { VerifyBackupTool } from './tools/verifyBackup.js';
import { ScheduleBackupTool } from './tools/scheduleBackup.js';
import { GetBackupStatusTool } from './tools/getBackupStatus.js';
import { CleanupOldBackupsTool } from './tools/cleanupOldBackups.js';
import { ExportBackupConfigTool } from './tools/exportBackupConfig.js';
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'create_backup': {
                const tool = new CreateBackupTool(DEFAULT_CONFIG.compression.level, DEFAULT_CONFIG.backupDirectory);
            ...
            case 'schedule_backup': {
                const tool = new ScheduleBackupTool(DEFAULT_CONFIG.compression.level, DEFAULT_CONFIG.backupDirectory);
                const result = await tool.execute(args);
                return {
                    content: [{ type: 'text', text: JSON.stringify(result, null, 2) }]
                };
            ...
            case 'get_backup_status': {
                const tool = new GetBackupStatusTool(DEFAULT_CONFIG.compression.level, DEFAULT_CONFIG.backupDirectory, DEFAULT_CONFIG.retention);
                const result = await tool.execute(args);
                return {
                    content: [{ type: 'text', text: JSON.stringify(result, null, 2) }]
                };
            ...
            case 'cleanup_old_backups': {
                const tool = new CleanupOldBackupsTool(DEFAULT_CONFIG.retention, DEFAULT_CONFIG.backupDirectory);
                const result = await tool.execute(args);
                return {
                    content: [{ type: 'text', text: JSON.stringify(result, null, 2) }]
                };
            ...
            case 'export_backup_config': {
                const tool = new ExportBackupConfigTool(DEFAULT_CONFIG.compression.level, DEFAULT_CONFIG.backupDirectory);
                const result = await tool.execute(args);
                return {
                    content: [{ type: 'text', text: JSON.stringify(result, null, 2) }]
                };
