...
export class TaskAnalysisEngine {
    ...
    static analyze(input) {
        ...
        const sequentialTime = subtasks.reduce((sum, task) => sum + (task.estimatedMinutes || 10), 0);
        const parallelTime = batches.reduce((sum, batch) => sum + batch.estimatedMinutes, 0);
        ...
        const parallelizable = this.shouldParallelize(score, estimatedSpeedup, subtasks.length, risks);
        // Step 8: Generate reasoning
        const reasoning = this.generateReasoning(parallelizable, score, estimatedSpeedup, levels, risks);
        return {
            parallelizable,
     ...
     */
    static scoreParallelizability(tasks, graph) {
        const factors = {
        ...
        // Weighted score
        const weightedScore = factors.independenceFactor * 0.3 +
     ...
     */
    static calculateIndependence(tasks, graph) {
        if (tasks.length === 0)
            return 0;
     ...
     */
    static calculateDurationValue(tasks) {
        const avgMinutes = tasks.reduce((sum, t) => sum + (t.estimatedMinutes || 10), 0) / tasks.length;
     ...
     */
    static estimateConflictRisk(tasks) {
     ...
     */
    static calculateDependencyComplexity(graph) {
     ...
     */
    static estimateResourceContention(tasks) {
     ...
     */
    static generateScoreRecommendation(score, confidence) {
        if (score >= 80)
            return 'Highly recommended - excellent parallelization candidate';
        if (score >= 60)
            return 'Recommended - good parallelization potential';
        if (score >= 40)
            return 'Moderate - some parallelization benefit possible';
        if (score >= 20)
            return 'Limited - minimal parallelization benefit';
        return 'Not recommended - sequential execution preferred';
    }
    /**
     * Identify risks
     */
    static identifyRisks(tasks, graph, batches) {
        const risks = [];
     ...
     */
    static detectDuplicateTasks(tasks) {
        const duplicates = [];
        const seen = new Map(); // normalized description -> task IDs
        for (const task of tasks) {
     ...
     */
    static shouldParallelize(score, estimatedSpeedup, taskCount, risks) {
     ...
     */
    static generateReasoning(parallelizable, score, estimatedSpeedup, levels, risks) {
        if (!parallelizable) {
            if (score.score < 40) {
        ...
        const parts = [];
        // Summary
        parts.push(`Parallelization recommended with ${estimatedSpeedup.toFixed(1)}x estimated speedup`);
        // Level breakdown
        if (levels.length > 1) {
            parts.push(`Tasks can be organized into ${levels.length} parallel batches based on dependencies`);
        }
        else {
        ...
        if (topFactors.length > 0) {
            parts.push(`Strongest factors: ${topFactors.map(([name, value]) => `${name} (${value.toFixed(0)}/100)`).join(', ')}`);
        }
        // Risks
        if (risks.length > 0) {
...
//# sourceMappingURL=task-analysis-engine.js.map