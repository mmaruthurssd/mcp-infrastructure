...
export class CreateDependencyGraphTool {
  ...
  static getToolDefinition() {
    return {
      ...
      description:
        'Build a dependency graph from subtasks. Detects explicit and implicit dependencies, validates for cycles, and returns topological ordering.',
      inputSchema: {
        type: 'object',
        properties: {
          tasks: {
          ...
          detectImplicit: {
            type: 'boolean',
            description:
              'Whether to detect implicit dependencies using AI analysis (default: true)',
          },
        },
        required: ['tasks'],
      },
  ...
  static execute(
    ...
    try {
      ...
      return {
        ...result,
        graph: serializedGraph as any,
      };
    } catch (error) {
      throw new Error(
        `Graph creation failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  private static validateInput(input: CreateDependencyGraphInput): void {
    // Validate tasks
