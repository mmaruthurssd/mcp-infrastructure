...
export interface Task {
  ...
  dependsOn?: string[];
  context?: any;  // Flexible for different systems
}
...
export interface GraphEdge {
  ...
  confidence?: number;  // For implicit dependencies
}
...
export interface TaskAnalysisResponse {
  ...
  estimatedSpeedup: number;  // e.g., 3.2x
  risks: Risk[];
}
...
export interface ExecutionMetrics {
  ...
  sequentialDuration: number;  // Estimated
...
export interface OptimizeBatchDistributionOutput {
  ...
  loadBalance: number;  // 0-100, higher is more balanced
  reasoning: string;
}
