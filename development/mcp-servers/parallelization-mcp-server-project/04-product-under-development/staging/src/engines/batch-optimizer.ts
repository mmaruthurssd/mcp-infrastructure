...
export class BatchOptimizer {
  ...
  static optimize(
    ...
    // Calculate metrics
    const estimatedTotalTime = batches.reduce(
   ...
   */
  private static normalizeGraph(graph: DependencyGraph): DependencyGraph {
    ...
    const nodesObj = graph.nodes as any;
  ...
  private static optimizeForMinTime(
    ...
    for (let i = 0; i < levels.length; i++) {
      ...
      if (levelTasks.length > maxAgents) {
        const subBatches = this.splitIntoSubBatches(
          levelTasks,
          maxAgents,
          i,
          'time-based'
  ...
  private static splitIntoSubBatches(
    ...
    maxAgents: number,
    levelIndex: number,
    strategy: 'time-based' | 'count-based'
  ): Batch[] {
    const batches: Batch[] = [];
    ...
    if (strategy === 'time-based') {
      ...
      const sorted = [...tasks].sort(
        (a, b) => (b.estimatedMinutes || 10) - (a.estimatedMinutes || 10)
      );

      // Distribute using greedy algorithm
      const agentLoads: Task[][] = Array.from({ length: maxAgents }, () => []);
      const agentTimes: number[] = Array(maxAgents).fill(0);
      ...
      for (let i = 0; i < tasks.length; i += batchSize) {
        ...
        batches.push({
          ...
          tasks: batchTasks,
          estimatedMinutes: Math.max(
            ...batchTasks.map((t) => t.estimatedMinutes || 10)
          ),
          dependsOnBatches: levelIndex > 0 ? [`batch-${levelIndex}`] : [],
        });
      }
  ...
  private static balancedSplit(
    ...
    const bins: Task[][] = Array.from({ length: maxAgents }, () => []);
    const binTimes: number[] = Array(maxAgents).fill(0);
   ...
   */
  private static groupSimilarTasks(tasks: Task[], maxGroups: number): Task[][] {
   ...
   */
  private static extractKeywords(description: string): Set<string> {
   ...
   */
  private static calculateLoadBalance(batches: Batch[], maxAgents: number): number {
    ...
    // Calculate variance
    const variance =
      batchTimes.reduce((sum, t) => sum + Math.pow(t - avgTime, 2), 0) /
      batchTimes.length;
  ...
  private static generateReasoning(
    ...
    parts.push(
      `Distributed ${totalTasks} tasks into ${batches.length} batches for ${maxAgents} agents`
    );
