...
export interface GetWorkflowStatusOutput {
  ...
  tasks?: Array<{
    ...
    description: string;
    status: TaskStatus;
    complexity?: { score: number; level: string; emoji: string };
  }>;
  documentation?: WorkflowDocumentation;
  error?: string;
}
...
export interface ArchiveWorkflowOutput {
  ...
  deploymentReadiness?: {
    ready: boolean;
    checks: {
      build?: { passed: boolean; output?: string; error?: string };
      tests?: { passed: boolean; output?: string; error?: string };
      health?: { passed: boolean; details?: string[] };
    };
    recommendations: string[];
