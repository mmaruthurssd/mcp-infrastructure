...
export class WorkflowManager {
     ...
     * Complete a task
     */
    static completeTask(projectPath, workflowName, taskId, notes, skipVerification = false) {
        try {
     ...
     */
    static performBasicVerification(task, state) {
        const evidence = [];
        const concerns = [];
     ...
     * Archive workflow
     */
    static archive(projectPath, workflowName, force = false) {
        try {
            const state = this.loadState(projectPath, workflowName);
            if (!state) {
                return {
