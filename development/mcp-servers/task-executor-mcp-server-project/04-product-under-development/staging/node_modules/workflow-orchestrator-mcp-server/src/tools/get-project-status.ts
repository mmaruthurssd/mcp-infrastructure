...
import { ProjectState, PhaseInfo, WorkflowPhase } from '../types/project-state.js';
...
export class GetProjectStatusTool {
   ...
   */
  private static buildPhaseStatus(name: string, phaseInfo: PhaseInfo): PhaseStatus {
   ...
   */
  private static calculateOverallProgress(state: ProjectState): string {
   ...
   */
  private static assessHealth(state: ProjectState): {
    health: 'Good' | 'Warning' | 'Blocked';
    healthIndicators: string[];
  } {
