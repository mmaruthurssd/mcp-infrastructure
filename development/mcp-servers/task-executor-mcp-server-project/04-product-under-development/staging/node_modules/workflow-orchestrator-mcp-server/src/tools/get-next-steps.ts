...
import { RuleEngine, RuleMatch } from '../core/rule-engine.js';
...
export class GetNextStepsTool {
   ...
   */
  private static calculateProgress(state: ProjectState): string {
    const phases: WorkflowPhase[] = ['initialization', 'goal-development', 'execution', 'completion'];
   ...
   */
  private static detectBlockers(state: ProjectState, projectPath: string): string[] {
   ...
   */
  private static detectWarnings(state: ProjectState, projectPath: string): string[] {
