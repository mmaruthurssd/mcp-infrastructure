...
export interface ParallelizationConfig {
  enabled: boolean;
  minSpeedupThreshold: number; // Only parallelize if speedup >= this (e.g., 1.5x)
  maxParallelAgents: number; // Maximum concurrent agents
  executionStrategy: 'conservative' | 'aggressive';
  autoExecute: boolean; // If true, auto-execute; if false, just suggest
}
...
export class ParallelizationAnalyzer {
   ...
   */
  private fallbackHeuristic(tasks: Task[]): ParallelizationAnalysis {
   ...
   */
  private async callParallelizationMCP(
    toolName: string,
    params: any
  ): Promise<ParallelizationMCPResponse> {
    if (!this.mcpTools) {
   ...
   */
  extractDependencies(tasks: Task[]): { from: string; to: string }[] {
