...
async function getUnorganizedFiles() {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === 'analyze_file') {
            ...
            if (analysis.metadata) {
                ...
                if (analysis.metadata.type)
                    metadataText += `  Type: ${analysis.metadata.type}\n`;
                if (analysis.metadata.phase)
                    metadataText += `  Phase: ${analysis.metadata.phase}\n`;
                if (analysis.metadata.project)
                    metadataText += `  Project: ${analysis.metadata.project}\n`;
                if (analysis.metadata.tags)
                    metadataText += `  Tags: ${Array.isArray(analysis.metadata.tags) ? analysis.metadata.tags.join(', ') : analysis.metadata.tags}\n`;
                if (analysis.metadata.status)
                    metadataText += `  Status: ${analysis.metadata.status}\n`;
                if (analysis.metadata.priority)
                    metadataText += `  Priority: ${analysis.metadata.priority}\n`;
        ...
        if (name === 'create_project_folder') {
            ...
            const location = args?.location || 'projects-in-development';
            ...
            try {
                ...
                throw new McpError(ErrorCode.InvalidParams, `Project folder already exists: ${location}/${normalizedName}`);
            }
            catch (error) {
        ...
        if (name === 'add_pattern') {
            ...
            if (!pattern || !location || !reason) {
                throw new McpError(ErrorCode.InvalidParams, 'pattern, location, and reason are required');
            }
        ...
        if (name === 'record_decision') {
            ...
            const movedTo = args?.movedTo;
            const reason = args?.reason;
            if (!fileName || !movedTo || !reason) {
                throw new McpError(ErrorCode.InvalidParams, 'fileName, movedTo, and reason are required');
            }
            customRules.fileDecisions.push({
                fileName,
                movedTo,
