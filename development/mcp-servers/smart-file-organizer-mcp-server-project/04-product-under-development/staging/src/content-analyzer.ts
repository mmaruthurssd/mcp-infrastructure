...
export interface ContentAnalysis {
  ...
  metadata?: FileMetadataExtracted; // Extracted metadata from file
...
export class ContentAnalyzer {
  private maxContentSize = 100000; // 100KB max for content analysis
  ...
  async analyzeFile(filePath: string): Promise<ContentAnalysis> {
    ...
    if (this.isTextFile(metadata.extension) && metadata.size < this.maxContentSize) {
      try {
        ...
        if (!analysis.purpose) analysis.purpose = contentAnalysis.purpose;
        if (!analysis.projectAffiliation) analysis.projectAffiliation = contentAnalysis.projectAffiliation;
        analysis.reasoning.push(...contentAnalysis.reasoning);

        // Override type if content suggests something different
        if (contentAnalysis.isSecret) analysis.isSecret = true;
        if (contentAnalysis.isTemplate) analysis.isTemplate = true;
        if (contentAnalysis.isPlanningDoc) analysis.isPlanningDoc = true;
      } catch (error) {
   ...
   */
  private analyzeExtension(extension: string): { type: ContentAnalysis['fileType']; reason: string } {
    ...
    for (const [typeName, typeInfo] of Object.entries(fileTypes)) {
      ...
      if (info.extensions && info.extensions.includes(ext)) {
        return {
          type: typeName as ContentAnalysis['fileType'],
   ...
   */
  private analyzeContent(content: string, fileName: string): {
    keywords: string[];
    purpose: string;
    isSecret: boolean;
    isTemplate: boolean;
    ...
    let projectAffiliation: string | undefined;
   ...
   */
  private suggestLocation(analysis: ContentAnalysis, metadata: FileMetadata): {
    location: string;
    confidence: number;
    reason: string;
  } {
   ...
   * Format: ---\nkey: value\n---
   */
  private parseYAMLFrontmatter(content: string): FileMetadataExtracted | null {
    ...
    const metadata: FileMetadataExtracted = {};
   ...
   */
  private parseJSONMetadata(content: string): FileMetadataExtracted | null {
    ...
    for (const pattern of patterns) {
      ...
      if (match && match[1]) {
        try {
          ...
          return metadata as FileMetadataExtracted;
        } catch (error) {
   ...
   */
  private parseCommentTags(content: string): FileMetadataExtracted | null {
   ...
   */
  private extractAllMetadata(content: string, fileExtension: string): FileMetadataExtracted | null {
    let metadata: FileMetadataExtracted | null = null;
