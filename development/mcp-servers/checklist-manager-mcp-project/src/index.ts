...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'validate_checklist_compliance': {
        const result = await validateChecklistCompliance(
          args as unknown as ValidateChecklistComplianceParams
        );
        return {
          content: [
            {
      ...
      case 'create_from_template': {
        const result = await createFromTemplate(
          args as unknown as CreateFromTemplateParams
        );
        return {
          content: [
            {
      ...
      case 'update_checklist_item': {
        const result = await updateChecklistItem(
          args as unknown as UpdateChecklistItemParams
        );
        return {
          content: [
            {
