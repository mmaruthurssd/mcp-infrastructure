...
import type { PerformanceMetric, ResourceUsage } from '../types/index.js';

const dataStore = new DataStore();
const collector = new MetricsCollector();

export async function trackPerformance(args: Record<string, unknown>) {
  const startTime = Date.now();

  try {
    // Validate required parameters
    const mcpServer = args.mcpServer as string;
    ...
    const metric: PerformanceMetric = {
      ...
      resourceUsage: args.resourceUsage as ResourceUsage | undefined,
    };
