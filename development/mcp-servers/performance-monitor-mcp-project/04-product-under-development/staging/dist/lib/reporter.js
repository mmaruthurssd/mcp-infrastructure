...
export class Reporter {
     ...
     */
    async generateReport(startTime, endTime, mcpServer, format = 'markdown', includeRecommendations = true) {
        // Read metrics
    ...
    getTopErrorTools(metrics, count) {
        const toolMap = new Map();
        for (const metric of metrics) {
            if (!toolMap.has(metric.toolName)) {
