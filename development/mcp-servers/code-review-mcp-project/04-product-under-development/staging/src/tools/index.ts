...
import {
  ...
  DetectComplexityParams,
  DetectComplexityResult,
  FindCodeSmellsParams,
  FindCodeSmellsResult,
  TrackTechnicalDebtParams,
  SuggestImprovementsParams,
  SuggestImprovementsResult,
  GenerateReviewReportParams,
  GenerateReviewReportResult,
} from '../types.js';
...
export async function analyzeCodeQuality(params: AnalyzeQualityParams): Promise<AnalyzeQualityResult> {
  try {
...
export async function detectComplexity(params: DetectComplexityParams): Promise<DetectComplexityResult> {
  try {
    const files = await getFiles(params.target);
    const threshold = {
    ...
    const filesOverThreshold = new Set(
      allFunctions
    ...
    const hotspots = params.format === 'hotspots'
      ? allFunctions
        ...
        .map(f => ({
          ...
          impact: (f.cyclomatic > 20 ? 'high' : f.cyclomatic > 15 ? 'medium' : 'low') as 'high' | 'medium' | 'low',
...
export async function findCodeSmells(params: FindCodeSmellsParams): Promise<FindCodeSmellsResult> {
  try {
    const files = await getFiles(params.target);
    const allSmells = [];
...
export async function trackTechnicalDebt(params: TrackTechnicalDebtParams) {
  try {
    const manager = new DebtManager(params.projectPath);

    switch (params.action) {
      case 'add':
        if (!params.debt) throw new Error('Debt details required for add action');
      ...
      case 'update':
        if (!params.debtId) throw new Error('Debt ID required for update action');
        await manager.updateDebt(params.debtId, { status: params.status, notes: params.notes ? [params.notes] : [] });
        return { success: true };

      case 'resolve':
        if (!params.debtId) throw new Error('Debt ID required for resolve action');
...
export async function suggestImprovements(params: SuggestImprovementsParams): Promise<SuggestImprovementsResult> {
  try {
    ...
    const summary = {
      ...
      estimatedImpact: (limitedSuggestions.filter(s => s.priority === 'high').length > 3 ? 'high' : 'medium') as 'high' | 'medium' | 'low',
    };
...
export async function generateReviewReport(params: GenerateReviewReportParams): Promise<GenerateReviewReportResult> {
  try {
    // Run all analyses
    ...
    const grade: 'A' | 'B' | 'C' | 'D' | 'F' = overallScore >= 90 ? 'A' : overallScore >= 80 ? 'B' : overallScore >= 70 ? 'C' : overallScore >= 60 ? 'D' : 'F';
    ...
    const report = {
      ...
      summary: {
        ...
        technicalDebtItems: (debtResult as any).report?.totalDebt || 0,
      ...
      topIssues: Object.entries(
        qualityResult.issues.reduce((acc, issue) => {
