...
import { TemplateType, TemplateOptions, ConfigurationError } from '../types.js';
 ...
 */
function getMcpServerTemplate(options: TemplateOptions): string {
 ...
 */
function getProjectTemplate(options: TemplateOptions): string {
 ...
 */
function getEnvironmentTemplate(options: TemplateOptions): string {
  const includeComments = options.includeComments !== false;
 ...
 */
function getGitHubActionTemplate(options: TemplateOptions): string {
 ...
 */
function getDockerTemplate(options: TemplateOptions): string {
  const includeComments = options.includeComments !== false;
 ...
 */
function getTemplateContent(templateType: TemplateType, options: TemplateOptions): string {
  switch (templateType) {
    case 'mcp-server':
 ...
 */
function getOutputFilename(templateType: TemplateType): string {
  switch (templateType) {
    case 'mcp-server':
      return '.mcp.json';
    case 'project':
 ...
 */
function getInstructions(templateType: TemplateType): string[] {
  switch (templateType) {
    case 'mcp-server':
      return [
...
): Promise<{ filesCreated: string[]; content: string; instructions: string[] }> {
  try {
    ...
    if (Object.keys(customFields).length > 0) {
      content = replacePlaceholders(content, customFields as Record<string, string>);
    }
    ...
    if (exists && !options.overwrite) {
      throw new ConfigurationError(
        'FILE_EXISTS',
        `File already exists: ${finalOutputPath}. Use overwrite option to replace.`
      );
    }
 ...
 */
export async function generateEnvironmentTemplates(
  outputDir: string,
  environments: string[],
  options: TemplateOptions = {}
): Promise<string[]> {
