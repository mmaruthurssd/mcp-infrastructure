...
import { Environment, GetEnvironmentVarsResult, ConfigurationError } from '../types.js';

/**
 * Environment file hierarchy (highest to lowest priority)
 */
const ENV_FILE_HIERARCHY = [
  (env: string) => `.env.${env}.local`,  // 1. .env.production.local
  () => '.env.local',                     // 2. .env.local
  (env: string) => `.env.${env}`,        // 3. .env.production
  () => '.env',                           // 4. .env
];
...
async function loadEnvFile(filePath: string): Promise<Record<string, string>> {
  try {
    ...
    throw new ConfigurationError(
      'LOAD_FAILED',
      `Failed to load environment file: ${error instanceof Error ? error.message : 'Unknown error'}`,
      { filePath }
    );
  }
}
 ...
 */
async function loadEnvironmentHierarchy(
  projectPath: string,
  environment: Environment
): Promise<{ variables: Record<string, string>; sources: string[] }> {
...
): Promise<GetEnvironmentVarsResult> {
  try {
    // Load hierarchy of env files
    const { variables: fileVars, sources } = await loadEnvironmentHierarchy(
      projectPath,
      environment
    );
