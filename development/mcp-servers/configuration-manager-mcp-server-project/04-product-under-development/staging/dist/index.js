...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: 'manage_secrets',
                description: 'Store, retrieve, rotate, and delete secrets using OS keychain. Supports metadata tracking and rotation scheduling.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        action: {
            ...
            },
            {
                ...
                description: 'Validate configuration files against JSON schemas. Supports built-in schemas (mcp-config, project-config, environment-config) or custom schemas.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        configPath: {
            ...
            },
            {
                ...
                description: 'Retrieve environment-specific configuration with cascading hierarchy (.env.production.local > .env.local > .env.production > .env).',
                inputSchema: {
                    type: 'object',
                    properties: {
                        environment: {
            ...
            },
            {
                ...
                description: 'Generate configuration file templates (mcp-server, project, environment, github-action, docker) with placeholder replacement.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        templateType: {
            ...
            },
            {
                ...
                description: 'Detect configuration differences across environments. Classifies drift by severity (critical, warning, info) and provides recommendations.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        projectPath: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        switch (name) {
            case 'manage_secrets':
                result = await manageSecrets(args);
                break;
            case 'validate_config':
                result = await validateConfig(args);
                break;
            case 'get_environment_vars':
                result = await getEnvironmentVars(args);
                break;
            case 'template_config':
                result = await templateConfig(args);
                break;
            case 'check_config_drift':
                result = await checkConfigDrift(args);
                break;
            default:
