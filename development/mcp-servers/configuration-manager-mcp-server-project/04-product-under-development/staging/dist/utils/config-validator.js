...
async function loadJson(filePath) {
    try {
        ...
        throw new ConfigurationError('INVALID_JSON', `Failed to parse JSON: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
...
export async function validateConfig(params) {
    ...
    try {
        ...
        if (schemaPath) {
            ...
            throw new ConfigurationError('NO_SCHEMA', 'Either schemaPath or schemaType must be provided');
        }
        ...
        throw new ConfigurationError('VALIDATION_FAILED', `Validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 * Check if schema file exists
 */
export async function schemaExists(schemaType) {
    try {
