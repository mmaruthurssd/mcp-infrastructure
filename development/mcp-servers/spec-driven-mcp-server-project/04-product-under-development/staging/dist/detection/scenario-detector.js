...
export class ScenarioDetector {
     ...
     */
    static hasConstitution(projectPath) {
        const possiblePaths = [
    ...
    static hasSpecs(projectPath) {
        ...
        try {
            ...
            const hasNumberedSpecs = entries.some(entry => entry.isDirectory() && /^\d{3}-/.test(entry.name));
    ...
    static hasCode(projectPath) {
        ...
        try {
            ...
            const hasCodeDirs = entries.some(entry => ['src', 'lib', 'app', 'components', 'services'].includes(entry) &&
                fs.statSync(path.join(projectPath, entry)).isDirectory());
            // Check for common code files
            const hasCodeFiles = entries.some(entry => /\.(ts|js|py|java|go|rs|rb|php|gs)$/.test(entry));
            // Check for Apps Script files specifically
            const hasAppsScriptFiles = entries.some(entry => entry === 'Code.gs' || entry === 'appsscript.json');
     ...
     */
    static createFeatureDirectoryName(featureName, featureNumber) {
