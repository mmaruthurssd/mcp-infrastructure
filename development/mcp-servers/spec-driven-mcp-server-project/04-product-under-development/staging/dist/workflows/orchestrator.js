...
export class WorkflowOrchestrator {
     ...
     */
    start(projectPath, description, scenario, goalContext) {
     ...
     */
    async answer(projectPath, answer) {
     ...
     * Handle setup confirmation
     */
    handleSetupConfirmation(state, confirmed) {
        if (!confirmed) {
            return {
                step: 'setup',
                scenario: state.scenario,
     ...
     */
    getCurrentQuestion(state, questionSet) {
     ...
     */
    generateConstitution(state, context) {
     ...
     */
    generateSpecification(state, context) {
     ...
     */
    generatePlan(state, context) {
        const featureDirName = ScenarioDetector.createFeatureDirectoryName(state.featureName || 'feature', state.featureId);
    ...
    extractTasksFromContext(context) {
        ...
        if (context.PREREQUISITE_TASKS && Array.isArray(context.PREREQUISITE_TASKS)) {
            context.PREREQUISITE_TASKS.forEach((task) => {
                tasks.push({
                    ...
                    dependsOn: task.dependencies && task.dependencies !== 'None' ? task.dependencies.split(',').map((d) => d.trim()) : []
                });
            });
        }
        // Extract from phases
        if (context.PHASES && Array.isArray(context.PHASES)) {
            context.PHASES.forEach((phase) => {
                if (phase.task_groups && Array.isArray(phase.task_groups)) {
                    phase.task_groups.forEach((group) => {
        ...
        if (context.INTEGRATION_TASKS && Array.isArray(context.INTEGRATION_TASKS)) {
            context.INTEGRATION_TASKS.forEach((task) => {
                tasks.push({
        ...
        if (context.E2E_TASKS && Array.isArray(context.E2E_TASKS)) {
            context.E2E_TASKS.forEach((task) => {
                tasks.push({
        ...
        if (context.DOCUMENTATION_TASKS && Array.isArray(context.DOCUMENTATION_TASKS)) {
            context.DOCUMENTATION_TASKS.forEach((task) => {
                tasks.push({
    ...
    getNextStep(currentStep) {
        const steps = ['setup', 'constitution', 'specification', 'planning', 'tasks', 'complete'];
