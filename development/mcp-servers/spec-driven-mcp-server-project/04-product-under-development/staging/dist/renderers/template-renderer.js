...
export class TemplateRenderer {
        ...
        return template.replace(eachRegex, (match, variable, content) => {
            ...
            return array.map((item, index) => {
                ...
                if (typeof item === 'object') {
                    itemContent = itemContent.replace(/\{\{this\.([a-zA-Z_]+)\}\}/g, (_m, prop) => {
                        return item[prop] !== undefined ? String(item[prop]) : _m;
                    });
                    // Handle nested {{#each this.property}}
                    itemContent = itemContent.replace(/\{\{#each\s+this\.([a-zA-Z_]+)\}\}([\s\S]*?)\{\{\/each\}\}/g, (_m, prop, nestedContent) => {
                        ...
                        return nestedArray.map(nestedItem => {
                            ...
                            if (typeof nestedItem === 'string') {
                                ...
                                nc = nc.replace(/\{\{this\.([a-zA-Z_]+)\}\}/g, (_mm, p) => {
                    ...
                    itemContent = itemContent.replace(/\{\{#if\s+this\.([a-zA-Z_]+)\}\}([\s\S]*?)\{\{\/if\}\}/g, (_m, prop, conditionalContent) => {
    ...
    buildTestingRequirements(answers) {
    ...
    buildCodeQualityDescription(answers) {
