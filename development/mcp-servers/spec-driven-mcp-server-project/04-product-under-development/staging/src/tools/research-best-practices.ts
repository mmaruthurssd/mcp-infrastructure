...
export class ResearchBestPracticesTool {
   ...
   * Identify key topics to investigate
   */
  private static identifyKeyTopics(topic: string): string[] {
    return [
   ...
   * Define evaluation criteria
   */
  private static defineEvaluationCriteria(topic: string, constraints: string[]): string[] {
    const criteria = [
   ...
   * Recommend resource types
   */
  private static recommendResourceTypes(topic: string): string[] {
    return [
   ...
   */
  private static getCautionaryNotes(topic: string, context?: string): string[] {
    const notes = [
