...
export class SDDGuideTool {
   ...
   */
  private handleStart(projectPath: string, description: string, scenario?: Scenario, goalContext?: any): any {
    if (!projectPath) {
      return {
        success: false,
   ...
   * Handle user answer
   */
  private handleAnswer(projectPath: string, response: any): any {
    if (!projectPath) {
      return {
        success: false,
        error: 'project_path is required'
