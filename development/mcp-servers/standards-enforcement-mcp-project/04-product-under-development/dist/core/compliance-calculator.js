...
export class ComplianceCalculator {
     ...
     */
    calculateSummary(totalRules, issues) {
        ...
        const complianceScore = this.calculateComplianceScore(totalRules, passedRules, criticalViolations, warningViolations, infoViolations);
        return {
            totalRules,
            passedRules,
            failedRules,
     ...
     * - Minimum score: 0
     */
    calculateComplianceScore(totalRules, passedRules, criticalViolations, warningViolations, infoViolations) {
        if (totalRules === 0) {
            return 100;
        }
     ...
     */
    calculateImprovementNeeded(currentScore, targetScore, summary) {
        ...
        const criticalToFix = Math.min(summary.criticalViolations, Math.ceil(pointsNeeded / 10));
        const remainingPoints = pointsNeeded - criticalToFix * 10;
        const warningsToFix = Math.min(summary.warningViolations, Math.ceil(remainingPoints / 2));
        return {
            criticalToFix,
            warningsToFix,
            pointsNeeded,
...
//# sourceMappingURL=compliance-calculator.js.map