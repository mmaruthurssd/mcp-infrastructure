 ...
 */
export async function validateDualEnvironment(context) {
    const issues = [];
    ...
    // Expected paths
    const devPath = path.join(workspacePath, 'development', 'mcp-servers', `${mcpName}-project`);
    const prodPath = path.join(workspacePath, 'local-instances', 'mcp-servers', mcpName);
 ...
 */
async function validateDevelopmentStructure(devPath, _mcpName) {
    const issues = [];
 ...
 */
async function validateProductionStructure(prodPath, mcpName) {
    const issues = [];
    // Check for package.json
 ...
 */
export async function validateDevProdSync(context) {
    const issues = [];
    if (context.targetType !== 'mcp' || !context.mcpName) {
        return issues;
    }
    const { workspacePath, mcpName } = context;
    const devPackageJsonPath = path.join(workspacePath, 'development', 'mcp-servers', `${mcpName}-project`, '04-product-under-development', 'package.json');
    const prodPackageJsonPath = path.join(workspacePath, 'local-instances', 'mcp-servers', mcpName, 'package.json');
...
//# sourceMappingURL=dual-environment.js.map