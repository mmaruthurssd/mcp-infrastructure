 ...
 */
export async function validateProjectStructure(context) {
    const issues = [];
    ...
    const structureIssues = await validateFolderStructure(targetPath, expectedStructure);
 ...
 */
async function detectExpectedStructure(projectPath) {
 ...
 */
async function validateFolderStructure(projectPath, expectedStructure) {
    const issues = [];
    const requiredFolders = expectedStructure === '8-folder'
    ...
    for (const folderName of requiredFolders) {
        ...
        if (!exists) {
            const severity = folderName === '01-planning' || folderName === '04-product-under-development'
                ? 'critical'
                : 'warning';
            issues.push({
    ...
    const unexpectedIssues = await validateNoUnexpectedFolders(projectPath, requiredFolders);
 ...
 */
async function validateNoUnexpectedFolders(projectPath, expectedFolders) {
    const issues = [];
    try {
 ...
 */
async function validatePlanningFolder(projectPath) {
    const issues = [];
 ...
 */
async function validateSpecificationContent(specPath) {
    const issues = [];
    try {
 ...
 */
async function validateProductFolder(projectPath) {
    const issues = [];
    ...
    const hasBuildConfig = (await fileExists(tsconfigPath)) ||
 ...
 */
async function validateNoMisplacedFiles(projectPath) {
    const issues = [];
    try {
        const entries = await fs.readdir(projectPath, { withFileTypes: true });
 ...
 */
export async function validateEightFolderStructureStrict(context) {
    const issues = [];
    const { targetPath } = context;
 ...
 */
export async function validateFourFolderStructure(context) {
    const issues = [];
    const { targetPath } = context;
