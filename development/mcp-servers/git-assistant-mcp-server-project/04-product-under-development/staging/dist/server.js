    ...
    try {
        switch (name) {
            case 'check_commit_readiness': {
                ...
                const result = heuristics.analyzeCommitReadiness(diffSummary, timeSinceLastCommit, status.staged.length > 0);
                ...
                if (securityIntegration.isEnabled()) {
                    ...
                    if (!securityScan.passed) {
                        ...
                        if (securityScan.issues.length > 0) {
                            ...
                            result.suggested_next_steps.unshift('🔒 Resolve security issues before committing', '📖 Review SECURITY_BEST_PRACTICES.md for guidance');
                        }
                        ...
                        for (const issue of securityScan.issues) {
                            ...
                            if (!hasSeen) {
                                await learningEngine.addSecurityPattern(issue.pattern, issue.severity, issue.file, issue.remediation);
                            }
                        }
                    }
                }
                ...
                try {
                    ...
                    if (mcpNameMatch) {
                        ...
                        if (summary.criticalViolations > 0) {
                            ...
                            result.warnings.unshift(`⚠️  ${summary.criticalViolations} critical standards violation(s) detected (Score: ${summary.complianceScore}/100)`);
                            result.suggested_next_steps.push('📋 Fix critical standards violations before committing', `💡 Run: validate_mcp_compliance({mcpName: "${mcpName}"}) for details`);
                        }
                        else if (!validation.compliant) {
                            // Has warnings but no critical violations
                            result.warnings.push(`ℹ️  Standards compliance score: ${summary.complianceScore}/100 (${summary.warningViolations} warnings)`);
                            result.suggested_next_steps.push(`📋 Consider fixing standards warnings (Score: ${summary.complianceScore}/100)`);
                        }
                        else {
                ...
                const learningSuggestion = await learningEngine.suggestPattern('check_commit_readiness', { file_count: diffSummary.total_files_changed }, result);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'suggest_commit_message': {
                ...
                const recentCommits = await git.getRecentCommits(10);
                const message = await messageGenerator.generateCommitMessage(diff, diffStat, recentCommits, style);
                // Suggest pattern learning
                const learningSuggestion = await learningEngine.suggestPattern('suggest_commit_message', { style }, message);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'add_learned_pattern': {
                const { tool, condition, action, reason } = args;
...
// === HELPER FUNCTIONS ===
function getGuidanceContent(topic) {
    const guidance = {
        'commit-messages': {
...
function generateInsights(commits) {
    ...
    const conventionalCount = commits.filter((c) => /^(feat|fix|refactor|docs|test|chore|style|perf):/.test(c.message)).length;
...
function generateRecommendations(commits) {
