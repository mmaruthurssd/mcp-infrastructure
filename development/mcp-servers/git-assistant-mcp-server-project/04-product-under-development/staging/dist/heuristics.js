...
export class HeuristicsEngine {
    ...
    analyzeCommitReadiness(diffSummary, timeSinceLastCommit, hasStagedFiles) {
        const scores = [];
        const reasons = [];
        const warnings = [];
    ...
    evaluateLineChanges(totalLines) {
    ...
    evaluateTimeSinceLastCommit(minutes) {
    ...
    generateRecommendation(confidence, diffSummary, timeSinceLastCommit) {
        if (confidence >= 0.8) {
    ...
    generateNextSteps(readyToCommit, hasStagedFiles) {
        const steps = [];
        if (readyToCommit) {
