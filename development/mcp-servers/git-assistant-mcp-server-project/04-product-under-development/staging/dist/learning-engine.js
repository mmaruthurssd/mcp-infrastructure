...
export class LearningEngine {
    ...
    async addPattern(tool, condition, action, reason, createdBy = 'user') {
    ...
    async checkPatterns(tool, params) {
        await this.loadPatterns();
    ...
    async suggestPattern(tool, params, result) {
     ...
     */
    async addSecurityPattern(pattern, severity, file, remediation) {
        return this.addPattern('security_scan', `Pattern detected: ${pattern} in ${file}`, `Block commit and warn user`, `${remediation} (Severity: ${severity})`, 'security-system');
    }
    /**
     * Check if a security issue has been seen before
     */
    async hasSeenSecurityIssue(pattern, file) {
        await this.loadPatterns();
        return this.patterns.some(p => p.tool === 'security_scan' &&
    ...
    async analyzeCommitHistory(commits) {
        if (commits.length === 0) {
            return {
    ...
    getPatternUsageStats(patternId) {
        if (patternId) {
