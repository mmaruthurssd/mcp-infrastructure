...
import { TemplateConfigParams, TemplateConfigResult, ConfigurationError } from '../types.js';
import * as templateGenerator from '../utils/template-generator.js';

export async function templateConfig(params: TemplateConfigParams): Promise<TemplateConfigResult> {
  ...
  try {
    ...
    if (templateType === 'environment' && options.environments && options.environments.length > 1) {
      const filesCreated = await templateGenerator.generateEnvironmentTemplates(
        outputPath,
