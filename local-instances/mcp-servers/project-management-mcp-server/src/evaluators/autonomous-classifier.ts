...
export interface AutonomousClassification {
  ...
  issueType: string; // Specific issue classification
  confidence: number;
  autonomyLevel: 'autonomous' | 'assisted' | 'manual';
  autonomousEligible: boolean; // Whether eligible for autonomous execution
  recommendedAction: string; // What action to take
  reasoning: string;
  classificationReasoning: string; // Why this classification was chosen
  autonomousThreshold: number; // 0.90
  assistedThreshold: number; // 0.70
  safetyChecks?: {
...
export class AutonomousClassifier {
  ...
  async classify(
    ...
    let baseType: 'broken' | 'missing' | 'improvement';
    ...
    let autonomyLevel: 'autonomous' | 'assisted' | 'manual';
    ...
    const recommendedAction =
      baseType === 'broken' ? 'Fix and validate' :
    ...
    const classificationReasoning = `Classified as "${baseType}" (${issueType}) because: ${
      baseType === 'broken' ? 'detected error/failure keywords' :
  ...
  private detectPHI(description: string, symptom: string): boolean {
    const phiKeywords = ['patient', 'phi', 'hipaa', 'medical', 'health'];
    return phiKeywords.some(kw => description.includes(kw) || symptom.includes(kw));
  }

  private detectSecurity(description: string, symptom: string): boolean {
  ...
  private buildReasoning(
    ...
    safetyChecks: { isPhiRelated: boolean; isSecurityRelated: boolean; isFirstTime: boolean }
  ): string {
