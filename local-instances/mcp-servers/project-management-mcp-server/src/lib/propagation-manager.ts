 ...
 */
export function propagateBatchChanges(
  projectPath: string,
  changes: Array<{
    componentName: string;
    action: ComponentAction;
  ...
  const allFilesUpdated = new Set<string>();

  for (const change of changes) {
    const { filesUpdated } = propagateComponentChange(
      projectPath,
 ...
 */
export function updateReadme(projectPath: string): void {
 ...
 */
export function generateProjectComponentsContent(components: Component[]): string {
 ...
 */
export function shouldLogToEventLog(action: ComponentAction, toStage?: ComponentStage): boolean {
  // Log major decisions and state changes
  const significantActions: ComponentAction[] = [
    "created",
    "moved",
    "split",
    "merged",
...
export function updateRoadmap(
  ...
  const entry = `- **${componentName}** - [View Goal](${path.relative(
    path.dirname(roadmapPath),
    goalPath
  )})`;
 ...
 */
export function validateReadmeSync(projectPath: string): {
  inSync: boolean;
  ...
  const readmeComponentNames: string[] = [];
