...
export interface GoalSummary {
  ...
  createdDate?: string;     // File creation date
}
 ...
 */
export function scanAllGoals(projectPath: string): ScannedGoals {
 ...
 */
export function scanPotentialGoals(baseDir: string): GoalSummary[] {
 ...
 */
export function scanSelectedGoals(baseDir: string): GoalSummary[] {
 ...
 */
export function scanArchivedGoals(baseDir: string, type: 'implemented' | 'shelved'): GoalSummary[] {
 ...
 */
function parsePotentialGoalFile(filePath: string): GoalSummary {
  const content = fs.readFileSync(filePath, 'utf-8');
 ...
 */
function parseSelectedGoalsFile(content: string, filePath: string): GoalSummary[] {
  ...
  while ((match = goalRegex.exec(content)) !== null) {
    ...
    const goalSectionRegex = new RegExp(
      `###\\s+(?:✅\\s+)?(?:❌\\s+)?Goal\\s+${goalId}:[\\s\\S]+?(?=\\n###|\\n##|$)`,
      ''
    );
 ...
 */
function parseArchivedGoalFile(filePath: string, type: 'implemented' | 'shelved'): GoalSummary {
  const content = fs.readFileSync(filePath, 'utf-8');
  const stats = fs.statSync(filePath);
 ...
 */
export function filterGoals(goals: GoalSummary[], filters: FilterOptions): GoalSummary[] {
  return goals.filter(goal => {
    if (filters.tier && goal.tier !== filters.tier) return false;
    if (filters.priority && goal.priority !== filters.priority) return false;
    if (filters.status && goal.status !== filters.status) return false;
    if (filters.owner && goal.owner !== filters.owner) return false;
    return true;
  });
}
 ...
 */
export function sortGoals(goals: GoalSummary[], sortBy: string): GoalSummary[] {
  const sorted = [...goals];
  ...
  switch (sortBy) {
    case 'impact':
      return sorted.sort((a, b) => {
        ...
        return (order[b.impactScore as keyof typeof order] || 0) -
               (order[a.impactScore as keyof typeof order] || 0);
      });

    case 'effort':
      return sorted.sort((a, b) => {
        const order = { Low: 3, Medium: 2, High: 1, Unknown: 0 };
        return (order[b.effortScore as keyof typeof order] || 0) -
               (order[a.effortScore as keyof typeof order] || 0);
      });

    case 'priority':
      return sorted.sort((a, b) => {
        const order = { High: 3, Medium: 2, Low: 1 };
        return (order[(b.priority || 'Medium') as keyof typeof order] || 0) -
               (order[(a.priority || 'Medium') as keyof typeof order] || 0);
      });
 ...
 */
export function calculateStatistics(scanned: ScannedGoals): GoalStatistics {
  const allGoals = [
