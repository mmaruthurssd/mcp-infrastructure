...
import { HandoffAuditEntry, Handoff, HandoffMetadata } from '../types/handoff.js';
...
export class AuditTrail {
  ...
  query(filter: AuditQueryFilter = {}): HandoffAuditEntry[] {
    ...
    for (const file of logFiles) {
      ...
      for (const line of lines) {
        ...
        try {
          const entry = JSON.parse(line) as HandoffAuditEntry;

          // Apply filters
          if (filter.handoffId && entry.handoffId !== filter.handoffId) continue;
          if (filter.handoffType && entry.metadata.handoffType !== filter.handoffType) continue;
          if (filter.sourceMcp && entry.metadata.sourceMcp !== filter.sourceMcp) continue;
          if (filter.targetMcp && entry.metadata.targetMcp !== filter.targetMcp) continue;
   ...
   */
  getRecent(limit: number = 50): HandoffAuditEntry[] {
   ...
   */
  private updateIndex(entry: HandoffAuditEntry): void {
    let index: any = {};
