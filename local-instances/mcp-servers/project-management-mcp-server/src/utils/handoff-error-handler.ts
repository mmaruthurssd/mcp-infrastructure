 ...
 */
export async function retryHandoffSend(
  handoff: Handoff,
  config: RetryConfig = {}
): Promise<string> {
 ...
 */
export function determineRecoveryStrategy(error: any): RecoveryStrategy {
...
export function createHandoffError(
  ...
  const error = new Error(message) as HandoffError;
  error.code = code;
 ...
 */
export async function handleHandoffError(
  error: any,
  handoff: Handoff,
