...
export interface ReviewThresholds {
  stalePotentialDays: number;      // Default: 90
  staleSelectedDays: number;        // Default: 30
 ...
 */
function parsePotentialGoals(projectPath: string): any[] {
  const potentialGoalsDir = path.join(
    projectPath,
 ...
 */
function parseSelectedGoals(projectPath: string): any[] {
 ...
 */
function detectStalePotentialGoals(
  potentialGoals: any[],
  ...
  for (const goal of potentialGoals) {
    if (!goal.dateCreated) continue;

    const daysSinceCreated = daysBetween(goal.dateCreated);

    if (daysSinceCreated >= thresholds.stalePotentialDays) {
      let urgency: 'high' | 'medium' | 'low' = 'low';
      if (daysSinceCreated >= 180) urgency = 'high';
      else if (daysSinceCreated >= 120) urgency = 'medium';
 ...
 */
function detectLongRunningGoals(
  selectedGoals: any[],
  thresholds: ReviewThresholds
): ReviewItem[] {
  const items: ReviewItem[] = [];
  ...
  for (const goal of selectedGoals) {
    if (!goal.datePromoted) continue;
    if (goal.status !== 'In Progress' && goal.status !== 'Active') continue;

    const daysSincePromoted = daysBetween(goal.datePromoted);

    if (daysSincePromoted >= thresholds.longRunningDays) {
      let urgency: 'high' | 'medium' | 'low' = 'medium';
      if (daysSincePromoted >= 120) urgency = 'high';
 ...
 */
function detectNoProgressGoals(
  selectedGoals: any[],
  thresholds: ReviewThresholds
): ReviewItem[] {
  const items: ReviewItem[] = [];
  ...
  for (const goal of selectedGoals) {
    ...
    if (daysSinceUpdate >= thresholds.noProgressDays) {
      ...
      else if (daysSinceUpdate >= 21) urgency = 'medium';
...
function detectBlockedGoals(
  ...
  for (const goal of selectedGoals) {
    ...
    if (daysSinceBlocked >= thresholds.blockedDays) {
      let urgency: 'high' | 'medium' | 'low' = 'medium';
      if (daysSinceBlocked >= 60) urgency = 'high';
 ...
 */
function detectCompletedNotArchived(selectedGoals: any[]): ReviewItem[] {
  const items: ReviewItem[] = [];
 ...
 */
export function detectReviewsNeeded(
  projectPath: string,
  checkType: 'all' | 'selected' | 'potential' = 'all',
  customThresholds?: Partial<ReviewThresholds>
): ReviewItem[] {
