...
export class NLPExtractor {
  ...
  static extractGoals(text: string): GoalMention[] {
    ...
    const verbPattern = new RegExp(
      `(?:need to|want to|should|must|plan to|going to|will) (${actionVerbs.join('|')}) ([^.!?]+)[.!?]`,
      'gi'
    );
  ...
  static extractStakeholders(text: string): StakeholderMention[] {
    const stakeholders: StakeholderMention[] = [];
  ...
  static extractResources(text: string): ResourceMention[] {
    const resources: ResourceMention[] = [];
  ...
  static extractConstraints(text: string): ConstraintMention[] {
    const constraints: ConstraintMention[] = [];
  ...
  static extractSuccessCriteria(text: string): CriteriaMention[] {
    const criteria: CriteriaMention[] = [];
  ...
  private static isMeasurable(criterion: string): boolean {
