...
describe('Visualization Tools - Goal 011', () => {
  ...
  describe('generate_roadmap_timeline', () => {
    ...
    test('should handle different time ranges', async () => {
      ...
      const ranges = ['all', 'current-quarter', 'current-year'] as const;
    ...
    test('should calculate goal metrics', async () => {
      ...
      if (result.success) {
        ...
        const totalAccounted =
          result.metrics!.goalsCompleted + result.metrics!.goalsInProgress + result.metrics!.goalsBlocked;
  ...
  describe('generate_dependency_graph', () => {
    ...
    test('should support different scopes', async () => {
      ...
      const scopes = ['all', 'component', 'major-goal'] as const;
  ...
  describe('Edge Cases', () => {
    ...
    test('should handle invalid goal files gracefully', async () => {
      ...
      await fs.writeFile(
        path.join(TEST_PROJECT_PATH, '02-goals-and-roadmap', 'components', 'test-component', 'major-goals', 'bad-goal.md'),
        'Invalid content with no structure',
        'utf-8'
      );
...
async function setupTestProject() {
  ...
  // Create test goals
  await fs.writeFile(
    path.join(componentPath, 'major-goals', '001-test-goal-active.md'),
    `# Major Goal 001: Test Goal Active
...
`,
  ...
  await fs.writeFile(
    path.join(componentPath, 'major-goals', '002-test-goal-complete.md'),
    `# Major Goal 002: Test Goal Complete
...
`,
  ...
  await fs.writeFile(
    path.join(componentPath, 'major-goals', '003-test-goal-blocked.md'),
    `# Major Goal 003: Test Goal Blocked
...
async function setupLargeProject(goalCount: number) {
  ...
  for (let i = 1; i <= goalCount; i++) {
    ...
    await fs.writeFile(
      path.join(componentPath, 'major-goals', `${goalId}-goal-${i}.md`),
      `# Goal ${goalId}\n\n**Status:** Planning\n**Progress:** 0%`,
      'utf-8'
    );
  }
