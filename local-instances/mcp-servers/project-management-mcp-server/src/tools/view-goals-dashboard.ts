...
import {
  ...
  ScannedGoals,
  GoalStatistics,
} from '../utils/goal-scanner.js';
import {
  detectAlerts,
  Alert,
} from '../utils/alert-detector.js';
...
function formatDashboard(
  scanned: ScannedGoals,
  stats: GoalStatistics | undefined,
  alerts: Alert[] | undefined,
  input: ViewGoalsDashboardInput
): string {
  let output = '═══════════════════════════════════════════════════════\n';
  ...
  if (scanned.potentialGoals.length > 0) {
    ...
    scanned.potentialGoals.slice(0, 10).forEach((goal) => { // Show top 10
  ...
  if (scanned.completedGoals.length > 0) {
    ...
    scanned.completedGoals.slice(0, 5).forEach((goal) => { // Show top 5
      output += `   ✓ ${goal.name} [${goal.tier}]\n`;
  ...
  if (scanned.shelvedGoals.length > 0) {
    ...
    scanned.shelvedGoals.slice(0, 5).forEach((goal) => { // Show top 5
      output += `   ✗ ${goal.name} [${goal.tier}]\n`;
 ...
 */
function formatGoalSummary(goal: GoalSummary, includeProgress: boolean = false): string {
