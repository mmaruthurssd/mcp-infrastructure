...
export interface CreatePotentialGoalArgs {
  ...
  // Evaluation results (from evaluate_goal or manual)
  impactScore: string;
  ...
  // Optional answers to 7 evaluation questions
  problem?: string;
  ...
  // Optional suggestions and next steps
  suggestions?: string[];
  nextSteps?: string[];

  // Optional autonomous classification (from framework)
...
export class CreatePotentialGoalTool {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          autonomousConfidence: {
            type: 'number',
            description: 'Optional: Autonomous deployment confidence score (0-1)'
          },
          autonomousEligible: {
            type: 'boolean',
            description: 'Optional: Whether goal is eligible for autonomous deployment'
          },
          recommendedAction: {
            type: 'string',
            enum: ['autonomous', 'assisted', 'manual'],
            description: 'Optional: Recommended deployment action level'
          },
          issueType: {
            type: 'string',
            description: 'Optional: Classified issue type (broken/missing/improvement or custom)'
          },
          baseType: {
            type: 'string',
            enum: ['broken', 'missing', 'improvement'],
            description: 'Optional: Mapped base issue type'
          },
          classificationReasoning: {
            type: 'string',
            description: 'Optional: Reasoning for autonomous classification'
          }
        },
        required: [
          'projectPath',
  ...
  static execute(args: CreatePotentialGoalArgs): CreatePotentialGoalResult {
    try {
      ...
      const context = GoalTemplateRenderer.buildPotentialGoalContext({
        ...
        autonomousConfidence: args.autonomousConfidence,
      ...
      const content = GoalTemplateRenderer.render('potential-goal', context);
   ...
   */
  static createFromEvaluation(
    projectPath: string,
    goalName: string,
    goalDescription: string,
    evaluationResult: EvaluateGoalResult,
    context?: string,
    userOverride?: string,
    evaluationAnswers?: {
      problem?: string;
    ...
    return this.execute({
      ...
      goalDescription,
      context,
      impactScore: evaluationResult.impact.score.toString(),
      ...
      effortScore: evaluationResult.effort.score.toString(),
      ...
      // Extract autonomous classification if available
      autonomousConfidence: evaluationResult.autonomous?.confidence,
      autonomousEligible: evaluationResult.autonomous?.autonomousEligible,
      recommendedAction: evaluationResult.autonomous?.recommendedAction,
      issueType: evaluationResult.autonomous?.issueType,
      baseType: evaluationResult.autonomous?.baseType,
      classificationReasoning: evaluationResult.autonomous?.classificationReasoning,
      ...evaluationAnswers
    });
  }
   ...
   */
  private static getPotentialGoalsDir(projectPath: string): string {
   ...
   * Generate filename from goal name
   */
  private static generateFileName(goalName: string): string {
