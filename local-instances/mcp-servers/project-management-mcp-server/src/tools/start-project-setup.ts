...
import { ConversationManager, type ProjectType, type ConstitutionMode } from '../utils/conversation-manager.js';
...
export class StartProjectSetupTool {
  static execute(input: StartProjectSetupInput): StartProjectSetupOutput {
    const projectType: ProjectType = input.projectType || 'software';
    const constitutionMode: ConstitutionMode = input.constitutionMode || 'quick';
