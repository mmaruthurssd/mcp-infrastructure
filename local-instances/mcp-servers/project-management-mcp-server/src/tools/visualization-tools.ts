...
export interface DashboardMetrics {
  ...
  goalsBlocked: number;
  velocity: number; // goals completed per week
...
export async function generateHierarchyTree(params: z.infer<typeof GenerateHierarchyTreeSchema>) {
  const startTime = Date.now();

  try {
    const { projectPath, outputFormat, maxDepth, showProgress, filterStatus, outputPath } =
      GenerateHierarchyTreeSchema.parse(params);
...
export async function generateRoadmapTimeline(params: z.infer<typeof GenerateRoadmapTimelineSchema>) {
  const startTime = Date.now();

  try {
    const { projectPath, outputFormat, groupBy, showMilestones, timeRange, outputPath } =
      GenerateRoadmapTimelineSchema.parse(params);
...
export async function generateProgressDashboard(params: z.infer<typeof GenerateProgressDashboardSchema>) {
  const startTime = Date.now();

  try {
    const { projectPath, outputFormat, includeVelocity, includeHealth, outputPath } =
      GenerateProgressDashboardSchema.parse(params);
...
export async function generateDependencyGraph(params: z.infer<typeof GenerateDependencyGraphSchema>) {
  const startTime = Date.now();

  try {
    const { projectPath, outputFormat, scope, entityId, showCriticalPath, outputPath } =
      GenerateDependencyGraphSchema.parse(params);
...
async function buildHierarchyTree(
  ...
  if (await fileExists(componentsDir)) {
    ...
    for (const componentFolder of componentFolders) {
      if (componentFolder.startsWith('.')) continue;

      const componentPath = path.join(componentsDir, componentFolder);
      const stats = await fs.stat(componentPath);
      if (!stats.isDirectory()) continue;

      const componentNode: HierarchyNode = {
        id: componentFolder,
      ...
      if (maxDepth >= 3) {
        ...
        if (await fileExists(goalsDir)) {
          ...
          for (const goalFile of goalFiles) {
            if (!goalFile.endsWith('.md') || goalFile.includes('COMPONENT')) continue;
            ...
            // Apply filter
            if (filterStatus !== 'all') {
              if (filterStatus === 'active' && status === 'Complete') continue;
              if (filterStatus === 'completed' && status !== 'Complete') continue;
              if (filterStatus === 'blocked' && status !== 'Blocked') continue;
            }
...
function generateASCIIHierarchy(tree: HierarchyNode, showProgress: boolean): string {
  const lines: string[] = [];

  function renderNode(node: HierarchyNode, prefix: string, isLast: boolean) {
...
function generateMermaidHierarchy(tree: HierarchyNode, showProgress: boolean): string {
  const lines: string[] = ['```mermaid', 'graph TD'];

  function addNode(node: HierarchyNode, parentId?: string) {
...
function generateDrawIOHierarchy(tree: HierarchyNode, showProgress: boolean): string {
...
function generateMermaidTimeline(items: TimelineItem[], showMilestones: boolean): string {
...
function generateASCIITimeline(items: TimelineItem[], showMilestones: boolean): string {
  const lines: string[] = ['PROJECT ROADMAP TIMELINE', '='.repeat(80), ''];
...
function generateDrawIOTimeline(items: TimelineItem[], showMilestones: boolean): string {
  return `<?xml version="1.0" encoding="UTF-8"?>
<mxfile>
...
function generateASCIIDashboard(metrics: DashboardMetrics): string {
  const lines: string[] = [];
...
function generateMermaidDashboard(metrics: DashboardMetrics): string {
  const lines: string[] = ['```mermaid', 'pie title Project Progress'];
  lines.push(`  "Completed" : ${metrics.goalsCompleted}`);
  lines.push(`  "In Progress" : ${metrics.goalsInProgress}`);
...
function generateDrawIODashboard(metrics: DashboardMetrics): string {
  return `<?xml version="1.0" encoding="UTF-8"?>
<mxfile>
...
async function buildDependencyData(
  projectPath: string,
  scope: string,
  entityId?: string
): Promise<any[]> {
...
function generateMermaidDependencies(dependencies: any[], showCriticalPath: boolean): string {
  return '```mermaid\ngraph LR\n  A[Goal A] --> B[Goal B]\n  B --> C[Goal C]\n```';
}

function generateASCIIDependencies(dependencies: any[], showCriticalPath: boolean): string {
  return 'DEPENDENCY GRAPH\n\nGoal A → Goal B → Goal C';
}

function generateDrawIODependencies(dependencies: any[], showCriticalPath: boolean): string {
...
function formatName(kebabCase: string): string {
  return kebabCase
    .split('-')
...
function extractGoalName(content: string): string | null {
  const match = content.match(/# (?:Major Goal \d+:\s*)?(.+)/);
  return match ? match[1].trim() : null;
}

function extractProgress(content: string): number {
...
function extractDate(content: string, type: 'start' | 'end'): string | null {
...
function countNodes(tree: HierarchyNode): number {
  let count = 1;
