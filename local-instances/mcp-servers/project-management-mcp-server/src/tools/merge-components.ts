...
import {
  MergeComponentsParams,
  MergeComponentsResult,
  ComponentStage,
    ...
    try {
      ...
      for (const component of originalComponents) {
        ...
        if (component.subComponents && component.subComponents.length > 0) {
          allSubComponents.push(
            ...component.subComponents.map((sc: any) => ({
              ...sc,
      ...
      if (!params.keepOriginals) {
        for (const componentName of params.componentNames) {
          propagateComponentChange(
            ...
            componentName,
            'merged',
            'FINALIZED' as ComponentStage,
            'ARCHIVED' as ComponentStage
          );
        }
      }
