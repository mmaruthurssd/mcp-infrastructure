...
import { FileAnalyzer, type FileAnalysisResult } from '../utils/file-analyzer.js';
...
export class MigrateExistingProjectTool {
  ...
  private static scanProject(projectPath: string): {
    files: string[];
    directories: string[];
  } {
    const files: string[] = [];
  ...
  private static detectConflicts(
    projectPath: string,
    suggestions: FileSuggestion[]
  ): Array<{ type: 'readme' | 'folder' | 'file'; description: string; options: string[] }> {
    ...
    const readmeFiles = suggestions.filter(s =>
      path.basename(s.sourceFile).toLowerCase().startsWith('readme')
    );
  ...
  private static percentage(value: number, total: number): number {
