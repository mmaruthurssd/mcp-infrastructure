...
export interface PromoteToSelectedInput {
  ...
  // Workflow control
  generateSpec?: boolean;       // If true, return goal context for sdd_guide
 ...
 */
function parsePotentialGoal(filePath: string): ParsedPotentialGoal {
 ...
 */
function assignGoalId(selectedGoalsPath: string): string {
...
function updateSelectedGoals(
  ...
  if (!fs.existsSync(selectedGoalsPath)) {
    const templatePath = path.join(
      ...
      '../templates/goal-workflow/selected-goals-index.md'
    );
 ...
 */
function updateStatistics(content: string, newGoalPriority: string): string {
...
function buildGoalContext(
  goalId: string,
  parsed: ParsedPotentialGoal,
  input: PromoteToSelectedInput
): GoalContext {
  return {
    goalId,
...
export class PromoteToSelectedTool {
  ...
  static execute(input: PromoteToSelectedInput): PromoteToSelectedOutput {
    try {
      ...
      const selectedGoalsDir = path.join(
        input.projectPath,
      ...
      return {
        ...
        formatted: formatOutput(
          ...
          input.targetDate || 'TBD',
          status,
          input.generateSpec || false
        ),
      };
    } catch (error) {
      return {
