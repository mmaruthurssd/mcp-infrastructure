...
export class IdentifyResourcesAndAssetsTool {
  static execute(input: IdentifyResourcesAndAssetsInput): IdentifyResourcesAndAssetsOutput {
    ...
    const resourcesTemplatePath = path.join(
      ...
      '../templates/project-setup/RESOURCES.md'
    );
    ...
    const assetsTemplatePath = path.join(
      path.dirname(new URL(import.meta.url).pathname),
      '../templates/project-setup/ASSETS.md'
    );
...
`;
  ...
  private static extractName(teamMention: string): string | null {
    // Try to extract a name (capitalized words)
    const nameMatch = teamMention.match(/([A-Z][a-z]+ [A-Z][a-z]+)/);
    return nameMatch ? nameMatch[1] : null;
  }

  private static extractRole(teamMention: string): string {
  ...
  private static extractAllocation(teamMention: string): string {
  ...
  private static extractSkills(teamMention: string): string {
    // Try to extract skills if mentioned
    const skillsMatch = teamMention.match(/skills?:?\s*([^,\.]+)/i);
    return skillsMatch ? skillsMatch[1].trim() : '';
  }

  private static categorizeTools(tools: string[]): Array<{ category: string; tools: string[] }> {
  ...
  private static categorizeTechnologies(techs: string[]): Array<{ category: string; technologies: string[] }> {
  ...
  private static parseBudgetBreakdown(budgetMentions: string[]): Array<{ category: string; amount: string; notes: string }> {
  ...
  private static inferAssetType(assetName: string): string {
    const lower = assetName.toLowerCase();

    if (/database|db|sql|data/i.test(lower)) return 'data';
    if (/api|integration|interface/i.test(lower)) return 'integration';
    if (/design|ui|ux|mockup/i.test(lower)) return 'design';
    if (/doc|documentation|guide/i.test(lower)) return 'documentation';

    return 'other';
  }

  private static inferPriority(assetName: string): string {
    const lower = assetName.toLowerCase();

    if (/critical|required|must|essential/i.test(lower)) return 'critical';
    if (/important|high|priority/i.test(lower)) return 'high';
    if (/nice|optional|would be/i.test(lower)) return 'low';

    return 'medium';
  }

  private static extractProvider(externalAsset: string): string {
