...
export async function analyzeVersionImpact(
  input: AnalyzeVersionImpactInput
): Promise<AnalyzeVersionImpactOutput> {
  ...
  try {
    ...
    switch (input.documentType) {
      case 'project-overview':
        await analyzeProjectOverviewImpact(
          input.projectPath,
    ...
    if (impactSummary.highImpactCount > 0) {
      recommendations.push(
        `${impactSummary.highImpactCount} high-impact documents will be affected. Consider creating a backup before proceeding.`
      );
    }

    if (input.changeType === 'major' && impactSummary.totalDocumentsAffected > 5) {
      recommendations.push(
        'Major version change affecting many documents. Consider breaking into smaller updates.'
      );
    }

    if (impactSummary.requiresReviewCount > 0) {
      recommendations.push(
        `${impactSummary.requiresReviewCount} documents require manual review after update.`
      );
    }
...
): Promise<GetVersionHistoryOutput> {
  try {
    const fullPath = join(input.projectPath, input.documentPath);
    if (!existsSync(fullPath)) {
      return {
...
export interface RollbackVersionInput {
  ...
  cascadeRollback?: boolean; // Rollback dependent documents too
  createBackup?: boolean; // Create backup before rollback (default: true)
...
export async function rollbackVersion(
...
): Promise<RollbackVersionOutput> {
  const warnings: string[] = [];
  const cascadedRollbacks: Array<{
    documentPath: string;
    previousVersion: number;
    restoredVersion: number;
  }> = [];
  ...
  try {
    ...
    if (input.cascadeRollback) {
      warnings.push(
        'Cascade rollback requested but not fully implemented. Manual review of dependent documents recommended.'
      );
    }
...
export interface UpdateComponentVersionInput {
  ...
  cascadeToGoals?: boolean; // Update dependent goals (default: true)
  dryRun?: boolean;
}
...
export async function updateComponentVersion(
...
): Promise<UpdateComponentVersionOutput> {
  const warnings: string[] = [];
  const changes: Array<{
    field: string;
    previousValue: any;
  ...
  try {
    ...
    const updatedContent = applyUpdatesToComponent(
      content,
      input.updates,
      newVersion,
      changes
    ...
    if (input.cascadeToGoals !== false) {
      const goalPattern = join(
        input.projectPath,
        '02-goals-and-roadmap/components',
        input.componentId,
        '*/major-goals/*/GOAL-STATUS.md'
      );
      ...
      if (cascadedUpdates.length > 0) {
        warnings.push(
          `${cascadedUpdates.length} goals depend on this component. Review recommended.`
        );
      }
    }
...
async function analyzeProjectOverviewImpact(
  projectPath: string,
  proposedChanges: Record<string, any>,
  impactedDocuments: ImpactedDocument[],
  recommendations: string[],
  ...
  const componentsPattern = join(
    projectPath,
    '02-goals-and-roadmap/components/*/OVERVIEW.md'
  );
  ...
  recommendations.push(
    'PROJECT OVERVIEW changes are high-impact. Schedule stakeholder review.'
  );
}
...
async function analyzeMajorGoalImpact(
  projectPath: string,
  documentPath: string,
  proposedChanges: Record<string, any>,
  impactedDocuments: ImpactedDocument[],
...
async function analyzeRoadmapImpact(
  projectPath: string,
  proposedChanges: Record<string, any>,
  impactedDocuments: ImpactedDocument[],
  recommendations: string[],
...
function calculateNewVersion(currentVersion: number, changeType: 'major' | 'minor' | 'patch'): number {
  switch (changeType) {
    case 'major':
...
function updateVersionInContent(content: string, newVersion: number, changeNote: string): string {
...
function parseComponentDocument(content: string): Record<string, any> {
...
function determineChangeType(changes: Array<{ field: string; previousValue: any; newValue: any }>): 'major' | 'minor' | 'patch' {
...
function applyUpdatesToComponent(
  content: string,
  updates: Record<string, any>,
  newVersion: number,
  changes: Array<{ field: string; previousValue: any; newValue: any }>
): string {
  let updated = content;
  const date = new Date().toISOString().split('T')[0];
...
export const analyzeVersionImpactTool = {
  ...
  description:
    'Analyze the impact of proposed version changes before committing. Shows affected documents, impact levels, and recommendations.',
...
export const getVersionHistoryTool = {
  ...
  description:
    'Get version history for a document. Returns list of versions with dates, changes, and authors.',
  inputSchema: z.object({
    projectPath: z.string().describe('Absolute path to project directory'),
...
export const rollbackVersionTool = {
  ...
  description:
    'Rollback a document to a previous version. Creates backup and optionally cascades rollback to dependent documents.',
  inputSchema: z.object({
    projectPath: z.string().describe('Absolute path to project directory'),
    documentType: DocumentTypeSchema.describe('Type of document'),
    documentPath: z.string().describe('Relative path to document from project root'),
...
export const updateComponentVersionTool = {
  ...
  description:
    'Update a component with version tracking and cascade to dependent goals. Supports dry-run mode for preview.',
  inputSchema: z.object({
    projectPath: z.string().describe('Absolute path to project directory'),
