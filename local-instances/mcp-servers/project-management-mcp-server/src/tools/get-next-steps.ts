...
import { RuleEngine, RuleMatch } from 'workflow-orchestrator-mcp-server/dist/core/rule-engine.js';
import { StateDetector } from 'workflow-orchestrator-mcp-server/dist/core/state-detector.js';

export interface GetNextStepsInput {
  projectPath: string;
  maxSuggestions?: number; // Limit number of suggestions (default: 5)
  skipSync?: boolean; // Skip auto-sync with file system (default: false)
}
...
export interface GetNextStepsResult {
  ...
  blockers: string[];
  warnings: string[];
  syncedChanges?: string[]; // Changes detected and synced
  message?: string;
}
...
export class GetNextStepsTool {
  static execute(input: GetNextStepsInput): GetNextStepsResult {
    ...
    const nextActions: NextStepSuggestion[] = matches
   ...
   */
  private static matchToSuggestion(match: RuleMatch): NextStepSuggestion {
   ...
   */
  private static calculateProgress(state: any): string {
    ...
    if (currentPhaseInfo && currentPhaseInfo.status === 'in-progress') {
      ...
      const completedSteps = currentPhaseInfo.steps.filter((s: any) => s.status === 'complete').length;
   ...
   */
  private static detectBlockers(state: any, projectPath: string): string[] {
   ...
   * Detect warnings
   */
  private static detectWarnings(state: any, projectPath: string): string[] {
