...
import { TrackedIssue, IssueContext, ReviewDecision } from './types.js';
    ...
    switch (name) {
      case 'track_issue': {
        ...
        const issue = await issueTracker.trackIssue(config.knowledgeBaseFile, {
          ...
          rootCause: args.root_cause as string | undefined,
      ...
      case 'check_optimization_triggers': {
        ...
        const triggers = await optimizer.checkOptimizationTriggers(
      ...
      case 'optimize_knowledge_base': {
        ...
        if (promotedIssues.length > 0 && config.preventiveCheckFile) {
          await preventiveGenerator.addPreventiveChecks(
            config.preventiveCheckFile,
            promotedIssues
          );
        }
      ...
      case 'get_domain_stats': {
        ...
        const highestFrequency = issues.reduce(
          (max, issue) => (issue.frequency > max.frequency ? issue : max),
          { issueNumber: 0, frequency: 0 } as any
        );
      ...
      case 'get_issues': {
        ...
        if (filter === 'promoted') {
          ...
          const duplicateIssueNumbers = new Set(
            duplicateGroups.flatMap(g => g.duplicates.map(d => d.issueNumber))
          );
      ...
      case 'review_promotion': {
        ...
        const action = args.action as 'approve' | 'reject' | 'defer';
        const reviewedBy = args.reviewed_by as string;
        const reviewNotes = args.review_notes as string | undefined;
        ...
        const kb = await issueTracker.parseKnowledgeBase(
          `${PROJECT_ROOT}/${config.knowledgeBaseFile}`
        );
        kb.issues = issues;
