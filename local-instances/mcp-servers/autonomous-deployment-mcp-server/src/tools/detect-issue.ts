...
interface DetectedIssue {
  ...
  suggestedApproaches: Approach[];
  requiresApproval: boolean;
  canResolveAutonomously: boolean;
}
 ...
 */
export async function detectIssue(args: DetectIssueArgs) {
  try {
    ...
    const highConfidenceCount = detectedIssues.filter(
      (issue) => issue.canResolveAutonomously
    ).length;
    ...
    const summary = {
      ...
      assistedEligible: detectedIssues.filter(
        (issue) => issue.confidence >= 0.7 && issue.confidence < 0.95
      ).length,
      lowConfidence: detectedIssues.filter(
        (issue) => issue.confidence < 0.7
      ).length,
    };
