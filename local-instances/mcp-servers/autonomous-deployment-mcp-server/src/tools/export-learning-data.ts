...
interface PatternPerformanceRecord {
  totalUsage: number;
  successful: number;
  failed: number;
  lastUsed: string;
...
interface PatternPerformanceData {
  patterns: Record<string, PatternPerformanceRecord>;
  metadata: {
    version: string;
    createdAt: string;
    lastUpdated: string;
 ...
 * Load patterns data
 */
async function loadPatternsData(): Promise<PatternsData> {
 ...
 */
async function loadPerformanceData(): Promise<PatternPerformanceData> {
 ...
 */
function generateMetadata(params: ExportLearningDataParams): ExportMetadata {
  return {
 ...
 * Main tool handler
 */
export async function exportLearningData(params: ExportLearningDataParams) {
  try {
    // Ensure exports directory exists
