 ...
 */
async function savePatterns(data: PatternsData): Promise<void> {
 ...
 */
async function updatePattern(patternId: string, updates: Partial<Pattern>): Promise<string> {
  const data = await loadPatterns();
 ...
 * Main tool handler
 */
export async function managePatterns(params: ManagePatternsParams) {
  try {
    let result: string;
