...
interface GetStatsArgs {
  ...
  groupBy?: "type" | "severity" | "pattern" | "none";
}
 ...
 */
function filterByTimeRange(outcomes: PatternOutcome[], timeRange: string): PatternOutcome[] {
  const now = new Date();
  let cutoffTime: Date;
 ...
 */
function calculateConfidence(successRate: number, totalUses: number, baseConfidence: number): number {
 ...
 */
export async function getStats(args: any) {
  const { timeRange = "week", groupBy = "none" } = args as GetStatsArgs;
  ...
  try {
    ...
    for (const [patternId, perf] of Object.entries(performance.patterns)) {
      ...
      if (filteredOutcomes.length === 0) continue;
    ...
    if (groupBy !== "none" && patternStats.length > 0) {
      ...
      if (groupBy === "pattern") {
        ...
        const groups = new Map<string, { uses: number; successes: number }>();
