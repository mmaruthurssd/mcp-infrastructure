 ...
 */
async function loadThresholds(): Promise<ThresholdsData> {
 ...
 */
async function saveThresholds(data: ThresholdsData): Promise<void> {
 ...
 */
function applyChanges(current: ThresholdsData, params: AdjustThresholdsParams): ThresholdsData {
 ...
 * Main tool handler
 */
export async function adjustThresholds(params: AdjustThresholdsParams) {
  try {
    ...
    if (!params.dryRun) {
      ...
      await logAuditTrail(
        changes,
        params.reason || 'Manual threshold adjustment',
        params.reviewedBy || 'system'
      );
    }
