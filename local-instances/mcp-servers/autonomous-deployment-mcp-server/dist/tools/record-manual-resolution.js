 ...
 */
function calculatePatternSimilarity(errorMessage, pattern) {
    try {
        ...
        const commonWords = errorWords.filter(word => patternWords.some(pw => pw.includes(word) || word.includes(pw)));
 ...
 */
export async function recordManualResolution(args) {
    const { issueId, errorMessage, solution, solutionSteps, duration, outcome } = args;
    try {
...
//# sourceMappingURL=record-manual-resolution.js.map