 ...
 */
export async function resolveWithApproval(args) {
    const { issueId, approachId, approvedBy } = args;
    try {
        ...
        const thresholds = JSON.parse(await fs.readFile(thresholdsPath, "utf-8"));
        ...
        for (const pattern of patternsData.patterns) {
            const approach = pattern.suggestedApproaches?.find((a) => a.id === approachId);
            if (approach) {
        ...
        if (confidence < thresholds.confidenceThresholds.assisted) {
            throw new Error(`Confidence ${confidence.toFixed(2)} is below assisted threshold ${thresholds.confidenceThresholds.assisted}. ` +
        ...
        executionResult.steps.forEach((step, idx) => {
...
//# sourceMappingURL=resolve-with-approval.js.map