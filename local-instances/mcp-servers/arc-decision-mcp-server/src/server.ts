...
interface DecisionTree {
  ...
  fallbacks: Record<string, Outcome>;
}
...
// Load decision tree
async function loadDecisionTree(): Promise<void> {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === 'analyze_requirements') {
    const { description, externalSystems, complexity, stateNeeded } = args as {
      description: string;
    ...
    const hasWorkflowPatterns = workflowPatternKeywords.some(keyword =>
  ...
  if (name === 'suggest_architecture') {
    const { hasExternalSystems, isComplex, needsState, needsTeaching } = args as {
  ...
  if (name === 'record_decision') {
    const { toolDescription, decision, reasoning, externalSystems, complexity, stateManagement } = args as {
  ...
  if (name === 'update_decision_outcome') {
    const { patternId, outcome } = args as {
      patternId: string;
