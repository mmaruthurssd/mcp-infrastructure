...
import { ValidateTemplateExistsParams } from '../types/tool-params.js';
 ...
 */
export async function validateTemplateExistsTool(
  params: ValidateTemplateExistsParams
  ...
  const context: ValidationContext = {
    workspacePath,
    targetPath: path.join(workspacePath, 'templates', 'drop-in-templates', `${mcpName}-template`),
    targetType: 'mcp',
    mcpName,
  };
  ...
  if (!checkMetadata) {
    result.violations = result.violations.filter(
      (v) => !v.ruleId.includes('metadata') && !v.ruleId.includes('template-json')
    );
  }

  if (!checkInstallable) {
    result.violations = result.violations.filter(
      (v) => !v.ruleId.includes('installable') && !v.ruleId.includes('install-script')
    );
  }
