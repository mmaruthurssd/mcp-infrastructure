...
async function validateEnvironmentFiles(
  ...
  const hasEnvFiles = (await Promise.all(
    envFiles.map((f) => fileExists(path.join(projectPath, f)))
  )).some(Boolean);
 ...
 */
async function validateEnvFile(envPath: string): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
 ...
 */
async function validateMcpRegistration(
  mcpName: string
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
 ...
 */
async function checkDeprecatedDependencies(
  packageJson: any
): Promise<string[]> {
