...
import {
  ValidateMcpComplianceParamsSchema,
  ValidateProjectStructureParamsSchema,
  ValidateTemplateExistsParamsSchema,
} from './types/tool-params.js';

import {
  validateMcpCompliance,
  validateProjectStructureTool,
  validateTemplateExistsTool,
} from './tools/index.js';
...
class StandardsEnforcementServer {
    ...
    return [
      {
        ...
        description:
          'Validate MCP against workspace standards. Checks dual-environment, template-first, project structure, configuration, documentation, and security standards.',
        inputSchema: {
          type: 'object',
          properties: {
            mcpName: {
            ...
            categories: {
              ...
              items: { type: 'string' },
              description:
                'Optional: Specific categories to validate (dual-environment, template-first, project-structure, configuration, documentation, security)',
            },
            failFast: {
              type: 'boolean',
      ...
      },
      {
        ...
        description:
          'Validate project folder structure. Checks for 8-folder or 4-folder standard structure compliance.',
        inputSchema: {
          type: 'object',
          properties: {
            projectPath: {
      ...
      },
      {
        ...
        description:
          'Validate that MCP has a corresponding drop-in template. Checks template existence, metadata, and installability.',
        inputSchema: {
          type: 'object',
          properties: {
            mcpName: {
   ...
   */
  private async handleValidateMcpCompliance(args: unknown) {
   ...
   */
  private async handleValidateProjectStructure(args: unknown) {
   ...
   */
  private async handleValidateTemplateExists(args: unknown) {
    ...
    if (result.violations.length > 0) {
      ...
      for (const [category, violations] of Object.entries(groupedByCategory)) {
        ...
        for (const v of violations) {
          const icon =
            v.severity === 'critical' ? '🔴' : v.severity === 'warning' ? '🟡' : 'ℹ️';
