...
export interface ProjectInfo {
  ...
  confidence: number; // 0-1 confidence this is a real project
...
export class ProjectDetector {
  ...
  private minimumProjectFiles: number;
   ...
   */
  async analyzeDirectory(dirPath: string): Promise<ProjectDetectionResult> {
   ...
   */
  private async checkProjectIndicators(dirPath: string, files: string[]): Promise<{
    hasPackageJson: boolean;
    hasReadme: boolean;
    hasSrcFolder: boolean;
    hasConfigFiles: boolean;
    relatedCodeFiles: number;
  }> {
    const result = {
   ...
   */
  private suggestProjectLocation(indicators: any, confidence: number): string {
   ...
   */
  async detectRelatedFiles(files: string[], rootPath: string): Promise<Map<string, string[]>> {
   ...
   */
  async shouldGraduateToProjects(activeWorkPath: string): Promise<{
   ...
   */
  async shouldArchive(projectPath: string): Promise<{
   ...
   */
  private async listFilesRecursive(dirPath: string, maxDepth: number, currentDepth = 0): Promise<string[]> {
    const files: string[] = [];
