...
export class GetBackupStatusTool {
    ...
    scheduler;
    retentionEngine;
    constructor(compressionLevel = 6, backupDirectory = '~/.backup-dr/backups', retentionPolicy = {
        dailyRetention: 7,
        weeklyRetention: 4,
        monthlyRetention: 12
    }) {
    ...
    async execute(params = {}) {
        ...
        if (params.includeRetention) {
            ...
            const estimatedSpaceReclaimed = retentionPreview.toDelete.reduce((sum, b) => sum + b.compressedSize, 0);
            result.retention = {
            ...
            if (retentionPreview.expired.length > 0) {
                result.health.issues.push(`${retentionPreview.expired.length} backup(s) exceeded retention policy`);
