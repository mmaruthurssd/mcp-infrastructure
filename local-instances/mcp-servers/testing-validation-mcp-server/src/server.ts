  ...
  private setupHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          case 'run_mcp_tests':
            result = await RunMcpTestsTool.execute(args as any);
            break;

          case 'validate_mcp_implementation':
            result = await ValidateMcpImplementationTool.execute(args as any);
            break;

          case 'check_quality_gates':
            result = await CheckQualityGatesTool.execute(args as any);
            break;

          case 'generate_coverage_report':
            result = await GenerateCoverageReportTool.execute(args as any);
            break;

          case 'run_smoke_tests':
            result = await RunSmokeTestsTool.execute(args as any);
            break;

          case 'validate_tool_schema':
            result = await ValidateToolSchemaTool.execute(args as any);
            break;
