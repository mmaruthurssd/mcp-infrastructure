...
export class TestRunner {
  ...
  constructor(mcpPath: string, verbose: boolean = false) {
   ...
   */
  async run(testType: TestType = 'all', coverage: boolean = false): Promise<RunMcpTestsOutput> {
    try {
      // Validate MCP path
      ...
      let testResults: RunMcpTestsOutput;
   ...
   */
  private async executeTests(testType: 'unit' | 'integration', coverage: boolean): Promise<RunMcpTestsOutput> {
   ...
   */
  private parseTestResults(jsonOutput: string, testType: 'unit' | 'integration'): RunMcpTestsOutput {
    try {
      ...
      const results: TestRunResults = {
        ...
        executionTime: jestResult.testResults?.reduce((sum: number, r: any) => sum + (r.perfStats?.runtime || 0), 0) || 0,
      };
      ...
      const output: RunMcpTestsOutput = {
  ...
  private parseCoverage(): CoverageReport | undefined {
    try {
      ...
      for (const [filePath, fileData] of Object.entries(coverageData)) {
        ...
        const stmtCovered = Object.values(data.s || {}).filter((v) => (v as number) > 0).length;
        const branchTotal = Object.keys(data.b || {}).length;
        const branchCovered = Object.values(data.b || {}).filter((arr: any) => arr.some((v: number) => v > 0)).length;
        const funcTotal = Object.keys(data.f || {}).length;
        const funcCovered = Object.values(data.f || {}).filter((v) => (v as number) > 0).length;
        const lineTotal = Object.keys(data.statementMap || {}).length;
        const lineCovered = Object.values(data.s || {}).filter((v) => (v as number) > 0).length;
   ...
   */
  private combineResults(unitResults: RunMcpTestsOutput, integrationResults: RunMcpTestsOutput): RunMcpTestsOutput {
    const combined: RunMcpTestsOutput = {
   ...
   */
  private validateMcpPath(): string | null {
    if (!this.mcpPath) {
