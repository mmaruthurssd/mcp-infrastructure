...
import type { RunSmokeTestsOutput, SmokeTestResult } from '../types.js';
...
export class SmokeTester {
   ...
   * Run smoke tests
   */
  async run(): Promise<RunSmokeTestsOutput> {
    try {
      ...
      const passed = results.filter(
        (r) => r.available && r.schemaValid && r.basicInvocation === 'pass' && r.responseValid
      ).length;
   ...
   */
  private extractTools(serverContent: string): string[] {
    const tools: string[] = [];
   ...
   */
  private checkToolSchema(toolName: string, serverContent: string): { valid: boolean; issues: string[] } {
    const issues: string[] = [];
    ...
    const typePattern = new RegExp(
      `name:\\s*['"]${toolName}['"][\\s\\S]*?inputSchema:\\s*\\{[\\s\\S]*?type:\\s*['"]object['"]`,
      'm'
    );
    ...
    const propsPattern = new RegExp(
      `name:\\s*['"]${toolName}['"][\\s\\S]*?inputSchema:\\s*\\{[\\s\\S]*?properties:\\s*\\{`,
      'm'
    );
   ...
   */
  private checkToolHandler(toolName: string, serverContent: string): { implemented: boolean } {
    ...
    const notImplementedPattern = new RegExp(
      `case\\s+['"]${toolName}['"]:[\\s\\S]*?['"]Tool not yet implemented['"]`,
      'm'
    );
   ...
   * Check if response structure is valid
   */
  private checkResponseStructure(toolName: string, serverContent: string): { valid: boolean; issues: string[] } {
    const issues: string[] = [];
   ...
   */
  private checkErrorHandling(toolName: string, serverContent: string): { hasErrorHandling: boolean } {
    // Check if there's a try-catch or error handling in the tool implementation
    const errorHandlingPattern = new RegExp(
      `case\\s+['"]${toolName}['"]:[\\s\\S]{0,1000}(try|catch|error|Error)`,
      'm'
    );
   ...
   * Create error result
   */
  private createErrorResult(error: string): RunSmokeTestsOutput {
    return {
      success: false,
      results: [],
      summary: {
