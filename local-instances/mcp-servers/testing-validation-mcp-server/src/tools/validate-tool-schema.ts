...
export class ValidateToolSchemaTool {
   ...
   * Execute the tool
   */
  static async execute(input: ValidateToolSchemaInput): Promise<ValidateToolSchemaOutput> {
    try {
      // Validate input
      if (!input.mcpPath) {
        return {
   ...
   */
  private static formatResults(result: ValidateToolSchemaOutput): string {
    const lines: string[] = [];
