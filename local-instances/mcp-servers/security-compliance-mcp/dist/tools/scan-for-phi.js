...
import { PHIScanner } from '../scanners/phi-scanner.js';
import { auditPHIScan, auditPHIViolation, generateCorrelationId, } from '../audit/audit-helper.js';
/**
 * Scan for PHI based on mode
 */
export async function scanForPHI(args) {
    ...
    for (const finding of allFindings) {
        await auditPHIViolation(finding.file, finding.category, finding.pattern, correlationId);
    }
    return {
 ...
 */
async function scanStagedFiles(scanner) {
    try {
 ...
 */
async function scanCommit(scanner, commitHash) {
    try {
 ...
 */
async function findFilesInDirectory(dirPath, exclude) {
    const files = [];
 ...
 */
function determineComplianceImpact(findings, riskLevel) {
 ...
 */
export function formatPHIScanResults(result) {
    const lines = [];
...
//# sourceMappingURL=scan-for-phi.js.map