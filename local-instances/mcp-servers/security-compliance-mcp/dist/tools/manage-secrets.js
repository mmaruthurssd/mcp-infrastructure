...
import { auditSecretEncrypted, auditSecretDecrypted, auditSecretRotated, auditSecretRotationWarning, generateCorrelationId, } from '../audit/audit-helper.js';
/**
 * Manage secrets
 */
export async function manageSecrets(args) {
    ...
    switch (args.action) {
        ...
        case 'status': {
            ...
            const needingRotation = secrets.filter((s) => s.rotationStatus === 'expired' || s.rotationStatus === 'expiring');
            // Audit warnings for secrets needing rotation
            for (const secret of needingRotation) {
                const daysUntil = Math.floor((new Date(secret.nextRotation).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
export function formatSecretsResult(result) {
    const lines = [];
    ...
    if (result.secrets && result.secrets.length > 0) {
        ...
        result.secrets.forEach((secret) => {
            ...
            const daysSince = Math.floor((new Date().getTime() - new Date(secret.lastRotated).getTime()) / (1000 * 60 * 60 * 24));
            const daysUntil = Math.floor((new Date(secret.nextRotation).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
            lines.push('');
