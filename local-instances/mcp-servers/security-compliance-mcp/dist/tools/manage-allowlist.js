...
import { auditAllowListAdded, auditAllowListRemoved, generateCorrelationId, } from '../audit/audit-helper.js';
/**
 * Manage security allow-list
 */
export async function manageAllowList(args, configPath) {
 ...
 */
async function loadAllowListFromFile(configFile) {
    try {
 ...
 */
async function addToAllowList(currentList, entry, configFile) {
    ...
    const isDuplicate = currentList.some((existing) => {
        return (existing.filePath === newEntry.filePath &&
 ...
 */
async function removeFromAllowList(currentList, index, configFile) {
 ...
 */
function listAllowList(currentList) {
    return {
        success: true,
        action: 'list',
        allowList: currentList,
 ...
 */
async function saveAllowList(allowList, configFile) {
    try {
        // Read current config
        const content = await fs.readFile(configFile, 'utf-8');
        const config = JSON.parse(content);
...
//# sourceMappingURL=manage-allowlist.js.map