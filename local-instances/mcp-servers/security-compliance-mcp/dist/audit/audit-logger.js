...
export class AuditLogger {
    ...
    currentLogFile;
    metadataFile;
    previousChecksum = GENESIS_CHECKSUM;
    metadata = null;
    constructor(logDir) {
     ...
     */
    calculateChecksum(event, previousChecksum) {
     ...
     * Log an audit event
     */
    async logEvent(eventType, severity, outcome, actor, details, options) {
        if (!this.metadata) {
    ...
    async verifyChain() {
        ...
        for (let i = 0; i < events.length; i++) {
            ...
            if (event.checksum !== expectedChecksum) {
                errors.push(`Checksum mismatch at event ${i} (ID: ${event.id}). Expected: ${expectedChecksum}, Got: ${event.checksum}`);
            }
     ...
     * Export as JSON
     */
    async exportJSON(outputPath, queryResult, options) {
        const exportData = {
     ...
     * Export as CSV
     */
    async exportCSV(outputPath, queryResult, _options) {
        const lines = [];
        // Header
     ...
     */
    async exportPDF(_outputPath, _queryResult, _options) {
    ...
    async pruneOldEvents() {
        ...
        if (pruned > 0) {
            ...
            await this.logEvent('audit_log_pruned', 'info', 'success', 'system', {
                eventsPruned: pruned,
