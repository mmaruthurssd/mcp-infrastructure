...
export class CredentialScanner {
   ...
   */
  scanText(content: string, filePath: string = '<text>'): Violation[] {
   ...
   */
  async scanDirectory(dirPath: string, options: { exclude?: string[] } = {}): Promise<ScanResult[]> {
   ...
   */
  private generateSuggestion(pattern: CredentialPattern): string {
   ...
   */
  private async findFiles(dirPath: string, exclude: string[]): Promise<string[]> {
    const files: string[] = [];
 ...
 */
export function scanText(content: string, filePath?: string, options?: ScanOptions): Violation[] {
  const scanner = new CredentialScanner(options);
...
export async function scanDirectory(
  dirPath: string,
  options?: ScanOptions & { exclude?: string[] }
): Promise<ScanResult[]> {
  const scanner = new CredentialScanner(options);
