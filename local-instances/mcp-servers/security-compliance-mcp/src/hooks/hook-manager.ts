 ...
 */
export async function installPreCommitHook(gitDir?: string): Promise<HookInstallResult> {
  try {
    // Find git directory
 ...
 */
export async function uninstallPreCommitHook(gitDir?: string): Promise<HookInstallResult> {
  try {
    const gitRoot = gitDir || findGitRoot();
    if (!gitRoot) {
      return {
 ...
 */
export async function checkHookStatus(gitDir?: string): Promise<{
  installed: boolean;
  hookPath?: string;
  hasBackup: boolean;
  message: string;
