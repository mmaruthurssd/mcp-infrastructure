...
export class MessageGenerator {
  async generateCommitMessage(
    ...
    style: 'conventional' | 'simple' | 'detailed' = 'conventional'
  ...
  private detectChangeType(diff: string, diffStat: string): CommitMessageResult['change_type'] {
  ...
  private extractKeyChanges(diff: string, diffStat: string): string[] {
  ...
  private learnCommitStyle(recentCommits: Commit[]): 'conventional' | 'simple' | 'detailed' {
    if (recentCommits.length === 0) {
      return 'conventional';
    }

    // Check if user uses conventional commits
    const conventionalCount = recentCommits.filter(commit =>
      /^(feat|fix|refactor|docs|test|chore|style|perf):/.test(commit.message)
    ).length;
    ...
    const detailedCount = recentCommits.filter(commit =>
  ...
  private generateSubject(
    changeType: CommitMessageResult['change_type'],
    keyChanges: string[],
  ...
  private getActionVerb(changeType: CommitMessageResult['change_type']): string {
    const verbs: Record<CommitMessageResult['change_type'], string> = {
      feat: 'add',
      fix: 'fix',
      refactor: 'refactor',
      docs: 'update',
  ...
  private summarizeChanges(keyChanges: string[]): string {
  ...
  private generateBody(keyChanges: string[], style: 'conventional' | 'simple' | 'detailed'): string {
    if (style === 'simple') {
  ...
  private truncateSubject(subject: string, maxLength: number = 50): string {
  ...
  private calculateConfidence(keyChanges: string[], recentCommits: Commit[]): number {
  ...
  private getTypeDescription(changeType: CommitMessageResult['change_type']): string {
    const descriptions: Record<CommitMessageResult['change_type'], string> = {
