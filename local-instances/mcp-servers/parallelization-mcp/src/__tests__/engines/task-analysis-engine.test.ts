...
describe('TaskAnalysisEngine', () => {
  describe('analyze()', () => {
    ...
    it('should create multiple batches for dependent tasks', () => {
      ...
      expect(result.suggestedBatches[0].tasks).toContainEqual(
    ...
    it('should identify unbalanced batch risk', () => {
      ...
      const unbalancedRisk = result.risks.find(r =>
        r.description.includes('Unbalanced')
      );
      expect(unbalancedRisk).toBeDefined();
    });

    it('should identify large batch risk', () => {
      const subtasks: Task[] = Array.from({ length: 15 }, (_, i) => ({
        id: `${i + 1}`,
      ...
      const largeBatchRisk = result.risks.find(r =>
        r.description.includes('>10 tasks')
      );
