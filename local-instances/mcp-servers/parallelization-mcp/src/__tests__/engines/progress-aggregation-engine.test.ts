...
import { AgentProgress } from '../../types.js';
...
describe('ProgressAggregationEngine', () => {
  describe('aggregate()', () => {
    ...
    it('should detect bottlenecks - slow progress', () => {
      ...
      const slowBottleneck = result.bottlenecks.find(b =>
        b.reason.includes('significantly behind') || b.reason.includes('slow')
      );
    ...
    it('should detect failed agents', () => {
      ...
      const failedBottleneck = result.bottlenecks.find(b =>
        b.reason.includes('failed')
      );
