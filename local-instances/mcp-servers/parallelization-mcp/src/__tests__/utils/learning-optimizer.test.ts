...
import { ExecutionRecord, ExecutionResult, Task, DependencyGraph } from '../../types.js';

describe('LearningOptimizer', () => {
  describe('recordExecution()', () => {
    const createTestRecord = (): ExecutionRecord => ({
      executionId: 'exec-1',
      timestamp: new Date(),
    ...
    it('should generate benchmark from history', () => {
      const record = {
        ...
        outcome: {
          ...
          results: [],
          conflicts: [],
          aggregatedProgress: { percentComplete: 100, method: 'simple-average' as const },
          metrics: {
            totalTasks: 5,
            parallelTasks: 4,
            sequentialTasks: 1,
