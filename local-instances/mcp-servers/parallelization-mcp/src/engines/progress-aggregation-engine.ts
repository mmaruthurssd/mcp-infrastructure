...
export class ProgressAggregationEngine {
   ...
   */
  private static simpleAverage(progresses: AgentProgress[]): number {
   ...
   */
  private static weightedAverage(progresses: AgentProgress[]): number {
    if (progresses.length === 0) return 0;

    const totalWeight = progresses.reduce(
      (sum, p) => sum + (p.taskWeight || 1),
      0
    );

    if (totalWeight === 0) return 0;

    const weightedSum = progresses.reduce(
      (sum, p) => sum + p.percentComplete * (p.taskWeight || 1),
      0
    );
  ...
  private static criticalPathProgress(
  ...
  ): { progress: number; path: string[] } {
    if (progresses.length === 0) {
      return { progress: 0, path: [] };
    }

    // Find agents on critical path (those with longest estimated time remaining)
    const sortedByTimeRemaining = [...progresses].sort(
      (a, b) =>
        (b.estimatedTimeRemaining || 0) - (a.estimatedTimeRemaining || 0)
    );

    // Critical path = agents that will finish last
    const criticalAgents = sortedByTimeRemaining.slice(
      0,
      Math.ceil(progresses.length * 0.3)
    ); // Top 30%
    ...
    const criticalProgress =
      criticalAgents.reduce((sum, a) => sum + a.percentComplete, 0) /
  ...
  private static detectBottlenecks(
    ...
    if (progresses.length === 0) return bottlenecks;

    // Calculate average progress
    const avgProgress =
      progresses.reduce((sum, p) => sum + p.percentComplete, 0) /
      progresses.length;
    ...
    for (const progress of progresses) {
      if (progress.percentComplete < avgProgress * 0.5 && avgProgress > 20) {
        bottlenecks.push({
          ...
          impact: this.assessBottleneckImpact(
    ...
    const workingCount = progresses.filter(
      (p) => p.status === 'working'
    ).length;
  ...
  private static estimateCompletion(
    ...
    const withTimeEstimates = progresses.filter(
      (p) => p.estimatedTimeRemaining && p.estimatedTimeRemaining > 0
    );
    ...
    if (withTimeEstimates.length > 0) {
      ...
      estimatedMinutes = Math.max(
        ...withTimeEstimates.map((p) => p.estimatedTimeRemaining || 0)
      );
    } else {
  ...
  static formatProgressReport(output: AggregateProgressOutput): string {
    ...
    lines.push(
      `Estimated Completion: ${output.estimatedCompletion.toLocaleTimeString()}`
    );

    if (output.criticalPath && output.criticalPath.length > 0) {
      lines.push(
        `Critical Path: ${output.criticalPath.slice(0, 3).join(' → ')}${output.criticalPath.length > 3 ? '...' : ''}`
      );
    }
    ...
    if (output.bottlenecks.length > 0) {
      ...
      for (const bottleneck of output.bottlenecks) {
        lines.push(
          `  - ${bottleneck.agentId}: ${bottleneck.reason} (${bottleneck.impact} impact)`
        );
      }
    }
    ...
    for (const [agentId, progress] of output.agentStatuses) {
      lines.push(
        `  - ${agentId}: ${progress.percentComplete.toFixed(0)}% (${progress.status})`
      );
    }
