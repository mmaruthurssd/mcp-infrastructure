...
export class ExecuteParallelWorkflowTool {
  ...
  static getToolDefinition() {
    return {
      ...
      description:
        'Execute tasks in parallel with sub-agent coordination and conflict detection. Spawns multiple Claude sub-agents, distributes work, monitors progress, and aggregates results.',
      inputSchema: {
        type: 'object',
        properties: {
          analysisResult: {
            type: 'object',
            description:
              'Analysis result from analyze_task_parallelizability containing dependency graph and suggested batches',
          },
          executionStrategy: {
            type: 'string',
            enum: ['conservative', 'aggressive'],
            description:
              'Execution strategy: conservative (fewer agents, safer) or aggressive (more agents, faster)',
          },
          maxParallelAgents: {
            type: 'number',
            description:
              'Maximum number of parallel agents to spawn (default: 5)',
          },
          constraints: {
            type: 'object',
            properties: {
            ...
            description:
              'Optional execution constraints (rate limits, resource limits)',
          },
        },
   ...
   */
  static execute(
    input: ExecuteParallelWorkflowInput
  ): ExecuteParallelWorkflowOutput {
    // Validation
   ...
   */
  private static validateInput(input: ExecuteParallelWorkflowInput): void {
