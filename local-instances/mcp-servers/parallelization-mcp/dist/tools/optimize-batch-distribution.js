...
export class OptimizeBatchDistributionTool {
    ...
    static getToolDefinition() {
        return {
            ...
            description: 'Optimize task distribution across parallel agents. Uses critical path method and load balancing to minimize total execution time. Returns optimized batch assignments.',
            inputSchema: {
                type: 'object',
                properties: {
                    tasks: {
                    ...
                    optimizationGoal: {
                        ...
                        description: 'Optimization goal: minimize-time (fastest completion), balance-load (even distribution), minimize-conflicts (reduce conflict risk)',
                    },
                },
    ...
    static validateInput(input) {
        ...
        if (!validGoals.includes(input.optimizationGoal)) {
            throw new Error(`optimizationGoal must be one of: ${validGoals.join(', ')}`);
        }
        ...
        for (let i = 0; i < input.tasks.length; i++) {
            ...
            if (task.estimatedMinutes !== undefined &&
                ...
                throw new Error(`Task ${task.id}: estimatedMinutes must be between 1 and 1440`);
            }
        }
    }
}
//# sourceMappingURL=optimize-batch-distribution.js.map