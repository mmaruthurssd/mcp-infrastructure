...
export class AggregateProgressTool {
    ...
    static getToolDefinition() {
        return {
            ...
            description: 'Aggregate progress from multiple parallel agents into a unified view. Supports simple average, weighted, and critical path strategies. Detects bottlenecks and estimates completion time.',
            inputSchema: {
                type: 'object',
                properties: {
                    agentProgresses: {
                        ...
                        items: {
                            ...
                            properties: {
                                ...
                                taskWeight: {
                                    type: 'number',
                                    description: 'Weight/importance of current task (for weighted strategy)',
                                },
                                status: {
                                    type: 'string',
                    ...
                    aggregationStrategy: {
                        ...
                        description: 'Strategy for aggregating progress: simple-average (equal weight), weighted (by task importance), critical-path (based on longest dependency chain)',
                    },
                },
    ...
    static execute(input) {
        ...
        try {
            ...
            throw new Error(`Progress aggregation failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    static validateInput(input) {
        ...
        if (input.agentProgresses.length > 50) {
            throw new Error('Maximum 50 agents allowed (current: ' + input.agentProgresses.length + ')');
        }
        ...
        if (!validStrategies.includes(input.aggregationStrategy)) {
            throw new Error(`aggregationStrategy must be one of: ${validStrategies.join(', ')}`);
        }
        ...
        for (let i = 0; i < input.agentProgresses.length; i++) {
            ...
            if (progress.percentComplete < 0 || progress.percentComplete > 100) {
                throw new Error(`Agent ${progress.agentId}: percentComplete must be between 0 and 100`);
            }
            // Validate status
            const validStatuses = ['idle', 'working', 'blocked', 'complete', 'failed'];
            if (!validStatuses.includes(progress.status)) {
                throw new Error(`Agent ${progress.agentId}: status must be one of: ${validStatuses.join(', ')}`);
            }
            // Validate taskWeight (if provided and using weighted strategy)
            if (progress.taskWeight !== undefined &&
                (progress.taskWeight < 0 || progress.taskWeight > 100)) {
                throw new Error(`Agent ${progress.agentId}: taskWeight must be between 0 and 100`);
            }
            // Validate estimatedTimeRemaining (if provided)
            if (progress.estimatedTimeRemaining !== undefined &&
                progress.estimatedTimeRemaining < 0) {
                throw new Error(`Agent ${progress.agentId}: estimatedTimeRemaining cannot be negative`);
            }
        }
        // Weighted strategy requires taskWeight
        if (input.aggregationStrategy === 'weighted') {
            const missingWeights = input.agentProgresses.filter(p => p.taskWeight === undefined);
            if (missingWeights.length > 0) {
                throw new Error('Weighted strategy requires taskWeight for all agents. ' +
                    `Missing for: ${missingWeights.map(p => p.agentId).join(', ')}`);
            }
        }
    }
}
//# sourceMappingURL=aggregate-progress.js.map