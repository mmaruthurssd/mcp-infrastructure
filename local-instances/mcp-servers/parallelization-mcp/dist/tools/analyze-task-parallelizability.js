...
export class AnalyzeTaskParallelizabilityTool {
    ...
    static getToolDefinition() {
        return {
            ...
            description: 'Analyze if a task can benefit from parallel sub-agent execution. Returns parallelization feasibility, dependency graph, suggested batches, and estimated speedup.',
            inputSchema: {
                type: 'object',
                properties: {
                    taskDescription: {
                    ...
                    subtasks: {
                        ...
                        description: 'Array of subtasks to analyze for parallelization opportunities',
                    },
                    context: {
                        type: 'object',
                        description: 'Optional context (flexible for different systems - Project Management MCP goals, Task Executor workflows, etc.)',
                    },
                },
    ...
    static validateInput(input) {
        ...
        if (input.taskDescription.length > 5000) {
            throw new Error('taskDescription exceeds maximum length of 5000 characters');
        }
        if (input.subtasks && input.subtasks.length > 0) {
            ...
            for (let i = 0; i < input.subtasks.length; i++) {
                ...
                if (!task.description || task.description.trim().length === 0) {
                    throw new Error(`Subtask at index ${i} is missing required field: description`);
                }
            ...
            if (input.subtasks.length > 100) {
                throw new Error('Maximum 100 subtasks allowed (current: ' +
...
//# sourceMappingURL=analyze-task-parallelizability.js.map