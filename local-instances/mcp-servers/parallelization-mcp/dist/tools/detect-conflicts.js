...
export class DetectConflictsTool {
    ...
    static getToolDefinition() {
        return {
            ...
            description: 'Detect potential conflicts from parallel agent execution. Identifies file-level conflicts, semantic conflicts, and dependency violations. Returns resolution options for each conflict.',
            inputSchema: {
                type: 'object',
                properties: {
                    agentResults: {
                    ...
                    dependencyGraph: {
                        type: 'object',
                        description: 'Optional dependency graph for detecting dependency violations',
                    },
                },
    ...
    static validateInput(input) {
        ...
        if (input.agentResults.length > 100) {
            throw new Error('Maximum 100 agent results allowed (current: ' + input.agentResults.length + ')');
        }
            ...
            if (result.changes) {
                ...
                for (let j = 0; j < result.changes.length; j++) {
                    ...
                    if (!change.file || change.file.trim().length === 0) {
                        throw new Error(`Result ${result.agentId}, change ${j}: file is required`);
                    }
                    if (!change.type) {
                        throw new Error(`Result ${result.agentId}, change ${j}: type is required`);
                    }
                    const validTypes = ['create', 'modify', 'delete'];
                    if (!validTypes.includes(change.type)) {
                        throw new Error(`Result ${result.agentId}, change ${j}: type must be one of: ${validTypes.join(', ')}`);
                    }
                }
            }
        }
...
//# sourceMappingURL=detect-conflicts.js.map