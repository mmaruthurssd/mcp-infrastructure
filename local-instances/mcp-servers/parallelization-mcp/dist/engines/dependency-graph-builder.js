...
export class DependencyGraphBuilder {
            ...
            if (task.dependsOn && task.dependsOn.length > 0) {
                for (const depId of task.dependsOn) {
                    ...
                    if (!graph.nodes.has(depId)) {
                        console.warn(`Warning: Task ${task.id} depends on non-existent task ${depId}`);
                        continue;
                    }
                    graph.edges.push({
                        from: depId,
     ...
     */
    static detectImplicitDependencies(tasks) {
        const implicit = [];
     ...
     */
    static deduplicateImplicitDeps(deps) {
        const seen = new Set();
        const unique = [];
     ...
     */
    static computeTopologicalLevels(graph) {
     ...
     */
    static getTopologicalLevels(graph) {
        const levels = [];
