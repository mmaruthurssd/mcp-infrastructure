 ...
 */
export async function findCoverageFile(projectPath) {
    const possiblePaths = [
 ...
 */
export async function parseCoverageFile(coverageFilePath) {
    try {
        const content = await readFile(coverageFilePath, 'utf-8');
        return JSON.parse(content);
    }
    catch (error) {
        throw new Error(`Failed to parse coverage file: ${error.message}`);
    }
}
/**
 * Calculate overall coverage metrics
 */
export function calculateOverallCoverage(coverageData) {
    ...
    for (const file of files) {
        ...
        coveredFunctions += Object.values(funcs).filter((v) => v > 0).length;
        // Lines (use statement map as approximation)
        const statementMap = fileCoverage.statementMap || {};
        totalLines += Object.keys(statementMap).length;
        coveredLines += Object.keys(statementMap).filter((key) => fileCoverage.s[key] > 0).length;
    }
    return {
 ...
 */
export function findUncoveredFiles(coverageData, threshold, excludePatterns) {
    ...
    for (const [filePath, fileCoverage] of Object.entries(coverageData)) {
        ...
        const coveredStatements = Object.values(stmts).filter((v) => v > 0).length;
 ...
 */
export function generateRecommendations(uncoveredFiles) {
 ...
 */
function determinePriority(coverage, uncoveredFunctionCount) {
 ...
 */
export function formatCoverageMetrics(metrics) {
    return `
