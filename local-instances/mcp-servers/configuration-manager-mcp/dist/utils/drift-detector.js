 ...
 */
function classifyDriftSeverity(key, values, environments) {
 ...
 */
function generateRecommendation(key, values, severity) {
    switch (severity) {
        case 'critical':
 ...
 */
export async function detectEnvironmentDrift(projectPath, environments, ignoreKeys = []) {
    // Load all environment configurations
    const envConfigs = {};
    for (const env of environments) {
        try {
            const result = await environmentLoader.loadEnvironmentVars(env, projectPath, undefined, false);
            envConfigs[env] = result.variables;
        }
        catch (error) {
            throw new ConfigurationError('LOAD_ENV_FAILED', `Failed to load environment: ${env}`, { environment: env });
        }
    }
    ...
    const summary = {
        ...
        environmentsParity: Math.round(((keysToCheck.length - drifts.length) / keysToCheck.length) * 100),
    };
 ...
 */
export function generateDriftReport(environments, drifts, summary) {
 ...
 */
export function generateDriftReportHTML(environments, drifts, summary) {
