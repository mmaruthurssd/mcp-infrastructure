...
export class StatePreservationManager {
  ...
  private async snapshotConfiguration(snapshotDir: string, environment: string): Promise<void> {
  ...
  private async snapshotStateFiles(snapshotDir: string, environment: string): Promise<void> {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(stateDir, `${environment}-state.json`),
        JSON.stringify(minimalState, null, 2)
      );
    }
  }

  private async snapshotDatabaseState(snapshotDir: string, environment: string): Promise<void> {
    ...
    await fs.writeFile(
      path.join(dbDir, 'preservation-note.json'),
      JSON.stringify(preservationNote, null, 2)
    );
  }
  ...
  async restoreSnapshot(snapshotDir: string): Promise<void> {
