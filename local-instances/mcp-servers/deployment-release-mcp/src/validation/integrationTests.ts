...
export interface IntegrationTestConfig {
  ...
  apiIntegrations?: Array<{ name: string; endpoint: string }>;
  timeout?: number;
}
...
export async function testApiIntegrations(
  integrations: Array<{ name: string; endpoint: string }>,
  timeout: number = 5000
): Promise<ValidationCheck[]> {
  const checks: ValidationCheck[] = [];
 ...
 */
function pingService(url: string, timeout: number): Promise<{ accessible: boolean; error?: string }> {
