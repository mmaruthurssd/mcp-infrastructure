 ...
 */
export async function checkResponseTimes(
  endpoints: string[],
 ...
 * Check CPU usage
 */
async function checkCpuUsage(maxCpuUsage: number): Promise<ValidationCheck> {
  const startTime = Date.now();
 ...
 * Check memory usage
 */
async function checkMemoryUsage(maxMemoryUsage: number): Promise<ValidationCheck> {
  const startTime = Date.now();
 ...
 */
function parseMemoryUsage(output: string): number {
...
function measureResponseTime(
  url: string,
  timeout: number
): Promise<{ ok: boolean; responseTime?: number; error?: string }> {
...
export async function runPerformanceValidation(
  ...
  if (config.endpoints && config.endpoints.length > 0) {
    const responseChecks = await checkResponseTimes(
      config.endpoints,
  ...
  const resourceChecks = await checkResourceUsage(
    thresholds.maxCpuUsage || 80,
    thresholds.maxMemoryUsage || 85
  );
