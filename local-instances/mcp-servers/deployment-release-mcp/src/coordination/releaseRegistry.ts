...
export class ReleaseRegistry {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      try {
        ...
        const initialData: ReleaseRegistryData = {
          version: "1.0.0",
      ...
      console.error(
        `[ReleaseRegistry] Failed to initialize registry: ${error}`
      );
      throw error;
    }
  }
   ...
   * Load registry data
   */
  async load(): Promise<ReleaseRegistryData> {
    try {
      const content = await fs.readFile(this.registryPath, "utf-8");
      return JSON.parse(content) as ReleaseRegistryData;
    } catch (error) {
      console.error(`[ReleaseRegistry] Failed to load registry: ${error}`);
      throw new Error(
        `Failed to load release registry from ${this.registryPath}`
      );
    }
  }
   ...
   * Save registry data
   */
  async save(data: ReleaseRegistryData): Promise<void> {
    try {
   ...
   */
  async addRelease(record: ReleaseRecord): Promise<void> {
   ...
   */
  async updateRelease(releaseId: string, updates: Partial<ReleaseRecord>): Promise<void> {
    const data = await this.load();
   ...
   */
  async getReleasesByEnvironment(environment: string): Promise<ReleaseRecord[]> {
    const data = await this.load();
   ...
   */
  async getLatestRelease(environment: string): Promise<ReleaseRecord | null> {
   ...
   */
  async getSuccessfulReleases(environment?: string): Promise<ReleaseRecord[]> {
    const data = await this.load();
   ...
   */
  async getStatistics(environment?: string): Promise<{
    total: number;
    successful: number;
    failed: number;
    rolledBack: number;
