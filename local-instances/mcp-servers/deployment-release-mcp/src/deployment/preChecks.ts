...
export class PreChecksManager {
  ...
  async runAllChecks(options: PreCheckOptions = {}): Promise<PreCheckResult[]> {
  ...
  private async runCodeQualityCheck(): Promise<PreCheckResult> {
    try {
  ...
  private async runTestsCheck(): Promise<PreCheckResult> {
    try {
  ...
  private async runSecurityCheck(): Promise<PreCheckResult> {
    try {
        ...
        if (stdout.trim() === "exists") {
          try {
            const { stdout: auditOutput } = await execAsync(
              `cd "${this.projectPath}" && npm audit --audit-level=moderate --json`,
              { timeout: 30000 }
            );
