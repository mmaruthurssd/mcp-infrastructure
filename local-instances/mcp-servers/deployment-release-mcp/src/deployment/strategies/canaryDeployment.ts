...
export class CanaryDeploymentStrategy extends BaseDeploymentStrategy {
  async deploy(): Promise<DeploymentResult> {
    ...
    try {
      ...
      logs.push(
        await this.executeCommand(
          "echo 'Deploying canary instance (10% traffic)'",
          "Canary deployment"
        )
      );
      ...
      for (const percentage of trafficSteps) {
        logs.push(
          await this.executeCommand(
            ...
            `Traffic increase to ${percentage}%`
          )
        );
      ...
      logs.push(
        await this.executeCommand(
          "echo 'Canary deployment successful, replacing all instances'",
          "Finalizing canary deployment"
        )
      );
