...
export async function rollbackDeployment(params) {
    const { projectPath, environment, deploymentId, target, preserveData = true, reason, force = false, } = params;
    ...
    try {
        ...
        const safetyChecks = await validator.validateRollbackTarget(targetDeployment, currentDeployment, force);
        const { canProceed, blockers } = await validator.validateSafetyChecks(safetyChecks, force);
        if (!canProceed) {
            throw new Error(`Rollback safety checks failed:\n${blockers.join('\n')}\n\nUse force=true to override safety checks.`);
        }
        ...
        const snapshotPath = await stateManager.createSnapshot(rollbackId, environment, currentDeployment?.id || 'unknown', targetDeployment.id, preserveData);
        ...
        const rollbackResult = await rollbackManager.executeRollback(projectPath, environment, targetDeployment, preserveData);
        ...
        const healthResult = await healthChecker.runHealthChecks(projectPath, environment, targetDeployment.servicesDeployed);
        ...
        await registryManager.addRollbackRecord(rollbackId, currentDeployment?.id || 'unknown', targetDeployment.id, reason);
...
//# sourceMappingURL=rollbackDeployment.js.map