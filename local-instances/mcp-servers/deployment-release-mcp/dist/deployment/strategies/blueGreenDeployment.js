...
export class BlueGreenDeploymentStrategy extends BaseDeploymentStrategy {
    async deploy() {
        ...
        try {
            ...
            logs.push(await this.executeCommand(`echo "Deploying to green environment (${this.context.environment}-green)"`, "Deploying to green environment"));
            logs.push(await this.executeCommand("echo 'Green environment deployment complete'", "Green environment setup"));
            ...
            logs.push(await this.executeCommand("echo 'Switching traffic from blue to green environment'", "Traffic switch"));
            // Step 5: Keep blue environment for rollback
            logs.push(await this.executeCommand("echo 'Blue environment kept for potential rollback'", "Preserving blue environment"));
...
//# sourceMappingURL=blueGreenDeployment.js.map