...
export class StateDetector {
  ...
  static compareWithState(
    ...
    const potentialMismatch = this.arrayDifference(
      detected.goalsDetected.potential,
      state.goals.potential
    );
    ...
    const selectedMismatch = this.arrayDifference(
      detected.goalsDetected.selected,
    ...
    const workflowsMismatch = this.arrayDifference(
      detected.workflowsDetected.active,
      state.integrations.taskExecutor.activeWorkflows
    );
   ...
   */
  private static detectGoals(projectPath: string): {
    potential: string[];
    selected: string[];
    completed: string[];
  } {
   ...
   */
  private static detectWorkflows(projectPath: string): {
    active: string[];
    archived: string[];
  } {
    const active: string[] = [];
   ...
   */
  private static detectKeyFiles(projectPath: string): {
    constitution: boolean;
    stakeholders: boolean;
    roadmap: boolean;
  } {
   ...
   */
  private static detectSpecifications(projectPath: string): string[] {
    const goalsWithSpecs: string[] = [];
    const selectedGoalsPath = path.join(projectPath, '02-goals-and-roadmap/selected-goals');
   ...
   */
  private static arrayDifference(detected: string[], stored: string[]): {
    added: string[];
    removed: string[];
  } {
