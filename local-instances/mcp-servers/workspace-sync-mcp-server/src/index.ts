  ...
  try {
    switch (name) {
      ...
      case 'apply_platform_update': {
        ...
        const autoConfirm = (args as any)?.autoConfirm ? 'yes' : '';
      ...
      case 'list_projects': {
        ...
        if (filter) {
          projects = projects.filter((p: any) => p.sync?.method === filter);
        }
      ...
      case 'update_manifest': {
        ...
        const { section, action, data } = args as any;
        ...
        switch (section) {
          case 'projects':
            ...
            if (action === 'add') {
              ...
              const index = manifest.projects.findIndex((p: any) => p.name === data.name);
              if (index !== -1) {
              ...
              manifest.projects = manifest.projects.filter((p: any) => p.name !== data.name);
            }
            break;
          ...
          case 'mcps':
            ...
            if (action === 'add') {
              ...
              const index = manifest.mcps.enabled.findIndex((m: any) => m.name === data.name);
              if (index !== -1) {
              ...
              manifest.mcps.enabled = manifest.mcps.enabled.filter((m: any) => m.name !== data.name);
            }
            break;
...
For now, use git sync method for projects.`,
      ...
      case 'post_team_message': {
        const { message, status } = args as any;
        ...
        await execAsync(
          `cd "${workspaceRoot}" && git commit -m "team: [${cleanUsername}] posted activity update"`
        );
      ...
      case 'get_team_activity': {
        const { since, limit = 10 } = args as any;
        const activityLog = join(workspaceRoot, 'team', 'activity.log');
      ...
      case 'update_team_status': {
        ...
        await execAsync(
          `cd "${workspaceRoot}" && git commit -m "team: [${cleanUsername}] updated status to ${status}"`
        );
        await execAsync(`cd "${workspaceRoot}" && git push origin main`);
      ...
      case 'get_team_status': {
        ...
        for (const [member, info] of Object.entries(state.team_members as any)) {
          const memberInfo = info as any;
      ...
      case 'daemon_control': {
        ...
        const { stdout, stderr } = await execAsync(
          `cd "${workspaceRoot}" && bash "${controlScript}" ${action}`
        );
      ...
      case 'rollback_team_activity': {
        ...
        const { stdout: logOutput } = await execAsync(
          `cd "${workspaceRoot}" && git log --oneline -${count} team/activity.log`
        );
