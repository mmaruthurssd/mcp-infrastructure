...
export class WorkflowManager {
     ...
     */
    static getWorkflowPath(projectPath, workflowName, archived = false) {
     ...
     * Get state file path
     */
    static getStateFilePath(projectPath, workflowName, archived = false) {
     ...
     */
    static fromLegacyState(legacyState, name) {
        const now = new Date();
    ...
    static create(input) {
        try {
            ...
            const tasks = taskInputs.map((taskInput, index) => {
     ...
     * Load workflow state
     */
    static loadState(projectPath, workflowName, archived = false) {
        try {
            ...
            if (data.customData && data.customData.tasks) {
                data.customData.tasks.forEach((task) => {
                    if (task.startedAt)
                        task.startedAt = new Date(task.startedAt);
                    if (task.completedAt)
                        task.completedAt = new Date(task.completedAt);
                    if (task.verifiedAt)
                        task.verifiedAt = new Date(task.verifiedAt);
                });
            }
            // Parse metadata date
     ...
     * Save workflow state
     */
    static saveState(state, projectPath, workflowName, archived = false) {
     ...
     */
    static async completeTask(projectPath, workflowName, taskId, notes, skipVerification = false, runValidation = false) {
        try {
     ...
     */
    static async performBasicVerification(task, state, projectPath, runValidation = false) {
        const evidence = [];
        const concerns = [];
     ...
     * Get workflow status
     */
    static getStatus(projectPath, workflowName) {
        try {
            const state = this.loadState(projectPath, workflowName);
            if (!state) {
                return {
     ...
     */
    static async archive(projectPath, workflowName, force = false, checkDeploymentReadiness = false) {
        try {
            const state = this.loadState(projectPath, workflowName);
            if (!state) {
                return {
            ...
            try {
                ...
                if (mcpNameMatch) {
                    ...
                    if (summary.criticalViolations > 0) {
                        warnings.push(`⚠️  ${summary.criticalViolations} critical standards violation(s) detected (Score: ${summary.complianceScore}/100)`);
                        warnings.push(`💡 Fix violations with: validate_mcp_compliance({mcpName: "${mcpName}"})`);
                    }
                    else if (!validationResult.compliant) {
                        // Has warnings but no critical violations
                        warnings.push(`ℹ️  Standards compliance score: ${summary.complianceScore}/100 (${summary.warningViolations} warnings)`);
                    }
                    else {
     ...
     */
    static detectDocumentation(projectPath) {
        const existing = [];
     ...
     */
    static getConstraintsStatus(state) {
     ...
     */
    static generatePlan(state, workflowPath) {
...
//# sourceMappingURL=workflow-manager-v2.js.map