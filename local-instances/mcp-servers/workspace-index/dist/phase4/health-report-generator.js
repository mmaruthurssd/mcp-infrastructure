...
export class HealthReportGenerator {
     ...
     */
    async generateReport(reportType = 'quarterly') {
        ...
        const recommendations = this.generateRecommendations(metrics, trends, opportunities, analysisResult);
     ...
     */
    async generateMarkdownReport(reportType = 'quarterly') {
     ...
     */
    async loadPreviousMetrics(reportType) {
        try {
            const metricsPath = path.join(this.adapter.getWorkspaceRoot(), '.doc-health-metrics', `${reportType}-latest.json`);
     ...
     */
    async saveMetrics(metrics, reportType) {
        try {
     ...
     */
    identifyConsolidationOpportunities(analysis) {
        const opportunities = [];
        ...
        for (const issue of redundancyIssues) {
            const estimatedReduction = issue.analysis.overlaps
                ...
                : 50; // Default estimate
            const priority = issue.confidence >= 0.85 ? 'high' :
     ...
     */
    suggestStrategy(fileCount, confidence) {
     ...
     */
    calculateHealthScore(metrics, analysis) {
        let score = 100;
     ...
     * Generate actionable recommendations
     */
    generateRecommendations(metrics, trends, opportunities, analysis) {
        ...
        if (highPriorityOpps.length > 0) {
            recommendations.push(`Address ${highPriorityOpps.length} high-priority consolidation opportunity(ies) to reduce ~${highPriorityOpps.reduce((sum, o) => sum + o.estimatedLineReduction, 0)} lines`);
        }
        ...
        if (filesGrowth && filesGrowth.trend === 'increasing' && filesGrowth.changePercent > 10) {
            recommendations.push(`Documentation files growing rapidly (${filesGrowth.changePercent.toFixed(1)}%) - consider consolidation to maintain manageability`);
        }
        // Recommendation based on redundancy
        if (metrics.redundancyClusters > 5) {
            recommendations.push(`${metrics.redundancyClusters} redundancy clusters detected - schedule consolidation sprint to reduce maintenance burden`);
        }
        // Recommendation based on stale docs
        if (metrics.staleDocs > 10) {
            recommendations.push(`${metrics.staleDocs} stale documents identified - review for archival or updates`);
        }
     ...
     */
    identifyKeyFindings(metrics, trends, analysis) {
        const findings = [];
        // Significant growth
        const linesGrowth = trends.find(t => t.metric === 'Total Lines');
        if (linesGrowth && Math.abs(linesGrowth.changePercent) > 15) {
            findings.push(`Documentation ${linesGrowth.trend === 'increasing' ? 'grew' : 'reduced'} by ${Math.abs(linesGrowth.changePercent).toFixed(1)}% (${Math.abs(linesGrowth.change).toLocaleString()} lines)`);
        }
        // High confidence issues
        if (analysis.summary.highConfidence > 0) {
            findings.push(`${analysis.summary.highConfidence} issue(s) detected with high confidence (>85%) - ready for autonomous resolution`);
        }
        // Redundancy concerns
        const redundancyRatio = metrics.totalFiles > 0 ? (metrics.redundancyClusters / metrics.totalFiles) * 100 : 0;
        if (redundancyRatio > 10) {
            findings.push(`${redundancyRatio.toFixed(1)}% of files are in redundancy clusters - consolidation recommended`);
        }
        return findings;
    }
}
