...
export class ReferenceValidator {
     ...
     */
    async updateReferences(oldPath, newPath, dryRun = false) {
        const workspaceRoot = this.adapter.getWorkspaceRoot();
        const mdFiles = await this.findMarkdownFiles(workspaceRoot);
     ...
     */
    async batchUpdateReferences(mappings, dryRun = false) {
        const workspaceRoot = this.adapter.getWorkspaceRoot();
        const mdFiles = await this.findMarkdownFiles(workspaceRoot);
     ...
     */
    async validateLink(sourceFile, targetFile, section) {
        try {
     ...
     */
    async findReferencesTo(sourceFile, oldPath, newPath) {
        ...
        for (const [relativeFile, fileUpdates] of updatesByFile.entries()) {
            try {
                ...
                for (const update of fileUpdates) {
                    newContent = newContent.replace(`](${update.oldLink})`, `](${update.newLink})`);
                }
