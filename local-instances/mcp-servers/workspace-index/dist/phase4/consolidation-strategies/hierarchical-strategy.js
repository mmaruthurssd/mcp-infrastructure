...
export class HierarchicalStrategy {
        ...
        try {
            for (const modification of plan.filesToModify) {
                ...
                if (modification.action === 'remove-sections' && modification.sections) {
                    ...
                    const result = await this.removeSectionsFromFile(filePath, modification.sections, dryRun);
                    if (result.modified) {
                ...
                if (modification.action === 'add-references' && modification.referenceTarget) {
                    ...
                    const result = await this.addReferenceToFile(filePath, modification.referenceTarget, dryRun);
     ...
     */
    async identifyPrimaryDocument(files) {
     ...
     */
    async identifyDuplicateSections(file, primaryFile, overlaps) {
        const duplicateSections = [];
        // Find overlap entry for this file pair
        const overlap = overlaps.find(o => (o.file1 === file && o.file2 === primaryFile) ||
     ...
     */
    async removeSectionsFromFile(filePath, sections, dryRun) {
        try {
            ...
            for (const line of lines) {
                ...
                if (headerMatch) {
                    ...
                    inRemovalSection = sections.some(s => s.toLowerCase().includes(currentHeader.toLowerCase()) ||
     ...
     */
    async addReferenceToFile(filePath, primaryFile, dryRun) {
        try {
            const content = await fs.readFile(filePath, 'utf-8');
