...
export class EnhancedRedundancyDetector {
     ...
     */
    async detectCrossDirectoryRedundancy(files) {
        const issues = [];
        ...
        const filesInClusters = new Set(clusterIssues.flatMap(issue => issue.files));
        const remainingFiles = files.filter(f => !filesInClusters.has(this.adapter.getRelativePath(f)) &&
     ...
     */
    async detectPatternBasedClusters(allFiles, threshold) {
        const issues = [];
            ...
            if (overlaps.length > 0) {
                ...
                issues.push({
                    ...
                    recommendedAction: {
                        ...
                        steps: this.generateConsolidationSteps(patternConfig.suggested_strategy || 'hierarchical', primaryDoc, relativePaths),
     ...
     */
    async detectPairwiseRedundancy(files, threshold) {
        const issues = [];
     ...
     */
    async calculateDetailedOverlap(file1, file2) {
        try {
     ...
     */
    findMatchingFiles(allFiles, patterns) {
        const matching = [];
     ...
     */
    identifyPrimaryDocument(overlapMatrix, allFiles) {
        let maxScore = -1;
     ...
     */
    generateConsolidationSteps(strategy, primaryDoc, allDocs) {
    ...
    calculateConfidence(type, evidence, complexityPenalty, reversibility, contextClarity) {
    ...
    aggregateConfidence(factors) {
        ...
        return (factors.patternMatch * 0.40 +
            factors.historicalSuccess * 0.30 +
            factors.reversibility * 0.15 +
            factors.contextClarity * 0.15) * factors.complexityPenalty; // Apply complexity penalty
    }
    generateIssueId(type, identifier) {
        return `${type}-${identifier.replace(/[^a-zA-Z0-9]/g, '-')}`;
    }
    async isFirstTimePattern(pattern) {
