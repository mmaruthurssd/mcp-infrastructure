...
export class DocumentationValidator {
    ...
    async validate(options = {}) {
        ...
        const shouldRunCheck = (check) => checks.includes('all') || checks.includes(check);
        const issues = [];
        let totalChecks = 0;
        let passed = 0;
     ...
     */
    async validateTemplateInventory() {
        const issues = [];
        try {
            // Scan actual template directories
            const templatesPath = path.join(this.projectRoot, 'templates-and-patterns', 'mcp-server-templates', 'templates');
