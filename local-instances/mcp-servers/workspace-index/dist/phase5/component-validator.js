...
export class ComponentValidator {
    ...
    async validateComponentDocumentation(componentName, location, type, isCritical = false, isPermanent = true) {
        const violations = [];
        const warnings = [];
        const suggestions = [];
        ...
        for (const requirement of DOCUMENTATION_REQUIREMENTS) {
            ...
            const violation = await this.checkRequirement(requirement, componentName, location, type);
            if (violation) {
    ...
    requirementApplies(requirement, type, isCritical, isPermanent) {
    ...
    async checkRequirement(requirement, componentName, location, type) {
        const filePath = path.join(this.projectRoot, requirement.file);
        try {
            const content = await fs.readFile(filePath, 'utf-8');
            // Replace placeholders in search patterns
            const patterns = requirement.searchPatterns.map(pattern => pattern
    ...
    async validateMultipleComponents(components) {
        ...
        for (const component of components) {
            const result = await this.validateComponentDocumentation(component.name, component.location, component.type, component.isCritical || false, component.isPermanent !== false);
            results.push(result);
        }
        return results;
    }
