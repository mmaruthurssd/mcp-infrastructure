...
export class DocumentationSuggester {
    ...
    async suggestDocumentation(componentPath, analyzeContent = true) {
    ...
    async analyzeComponent(absolutePath, relativePath) {
        const metadata = {
            estimatedPurpose: '',
    ...
    async generateBasicMetadata(componentPath) {
        return {
    ...
    async detectComponentDetails(componentPath, metadata) {
    ...
    inferComponentType(componentPath) {
    ...
    generateComponentName(componentPath, type) {
    ...
    generateDocumentationEntries(component, metadata) {
    ...
    generateSystemComponentsEntry(component, metadata) {
    ...
    generateWorkspaceArchitectureEntry(component, metadata) {
        let entry = `## ${component.name}\n\n`;
    ...
    extractPurposeFromContent(content, path) {
    ...
    inferPurposeFromPath(componentPath) {
    ...
    extractQuickStart(readme, componentPath) {
        const lines = readme.split('\n');
    ...
    extractWorkflowCommand(workflowPath) {
        const filename = path.basename(workflowPath);
        return `gh run list --workflow=${filename} --limit 5`;
    }
    calculateSuggestionConfidence(metadata, analyzeContent) {
