...
export interface ConfidencePrediction {
  ...
  timestamp: string;
  baseType?: string; // Optional: mapped base type
...
export interface CalibrationData {
  ...
  calibration_error: number; // predicted - actual (positive = overconfident)
  success: boolean; // Did prediction match outcome?
  bucket: string; // e.g., "0.90-1.00"
}

export interface ConfidenceBucket {
  range: string; // e.g., "0.90-1.00"
  min: number;
  max: number;
  predictions_count: number;
  actual_success_rate: number;
  predicted_confidence_avg: number;
  calibration_error: number; // predicted - actual
  recommendation: string;
}

export interface CalibrationReport {
  period: string; // e.g., "week", "month", "all"
...
export interface ThresholdAdjustment {
  current_thresholds: {
    autonomous: number; // 0.90
    assisted: number; // 0.70
  };
  ...
  expected_improvement: string; // e.g., "95% success rate for autonomous"
...
async function ensureCalibrationDirectories(workspaceBrainPath: string): Promise<void> {
  const dirs = [
...
function getConfidenceBucket(confidence: number): { range: string; min: number; max: number } {
  if (confidence >= 0.90) return { range: '0.90-1.00', min: 0.90, max: 1.00 };
  if (confidence >= 0.80) return { range: '0.80-0.89', min: 0.80, max: 0.89 };
  if (confidence >= 0.70) return { range: '0.70-0.79', min: 0.70, max: 0.79 };
  if (confidence >= 0.60) return { range: '0.60-0.69', min: 0.60, max: 0.69 };
  if (confidence >= 0.50) return { range: '0.50-0.59', min: 0.50, max: 0.59 };
...
async function loadAllPredictions(
  ...
  try {
    ...
    for (const file of files) {
      ...
      const prediction: ConfidencePrediction = JSON.parse(content);
...
export async function validateConfidencePrediction(
  ...
  const sameBucketPredictions = allPredictions.filter(
    p => getConfidenceBucket(p.predicted_confidence).range === bucket.range
  );
  ...
  const feedback = {
    ...
    recommendation:
      Math.abs(calibration_error) < 0.05
        ? 'Well calibrated'
...
export async function getCalibrationReport(
  ...
  const bucketMap = new Map<string, ConfidencePrediction[]>();
  ...
  const by_bucket: ConfidenceBucket[] = [];
  bucketMap.forEach((preds, bucketRange) => {
    ...
    const predicted_confidence_avg =
      preds.reduce((sum, p) => sum + p.predicted_confidence, 0) / preds.length;
  ...
  // Group by issue type
  const issueTypeMap = new Map<string, ConfidencePrediction[]>();
  for (const pred of predictions) {
  ...
  const by_issue_type: CalibrationReport['by_issue_type'] = {};
  ...
  if (highConfidenceBucket) {
    if (highConfidenceBucket.actual_success_rate < 0.90) {
      recommendations.push(
        `Autonomous threshold (0.90) is too low. Actual success rate: ${(highConfidenceBucket.actual_success_rate * 100).toFixed(0)}%. Increase to 0.95+`
      );
  ...
  Object.entries(by_issue_type).forEach(([type, data]) => {
    if (data.calibration_error > 0.10) {
      recommendations.push(
        `Issue type "${type}" is overconfident by ${(data.calibration_error * 100).toFixed(0)}%. Apply 0.${Math.round((1 - data.calibration_error) * 100)} multiplier`
      );
    }
  });
  ...
  const avgCalibrationError =
    by_bucket.reduce((sum, b) => sum + Math.abs(b.calibration_error), 0) / by_bucket.length;

  let calibration_quality: CalibrationReport['calibration_quality'];
  if (avgCalibrationError < 0.05) calibration_quality = 'excellent';
  else if (avgCalibrationError < 0.10) calibration_quality = 'good';
  else if (avgCalibrationError < 0.20) calibration_quality = 'needs-improvement';
...
export async function getConfidenceTrends(
  args: {
  ...
  // Group by ISO week
  const weekMap = new Map<string, ConfidencePrediction[]>();
  for (const pred of predictions) {
  ...
  const firstHalfError =
    firstHalf.reduce((sum, t) => sum + Math.abs(t.calibration_error), 0) / firstHalf.length;
  const secondHalfError =
    secondHalf.reduce((sum, t) => sum + Math.abs(t.calibration_error), 0) / secondHalf.length;
  ...
  const issueTypeAnalysis = new Map<string, { error: number; count: number }>();
  ...
  const poorlyCalibrated: Array<{ type: string; avg_error: number }> = [];
...
export async function adjustConfidenceThresholds(
  args: {
  ...
  for (let threshold = 0.99; threshold >= 0.80; threshold -= 0.01) {
    ...
    if (candidates.length >= minSampleSize) {
      ...
      if (successRate >= targetSuccessRate) {
        ...
        justification.push(
          `Autonomous threshold set to ${autonomousThreshold} based on ${candidates.length} predictions with ${(successRate * 100).toFixed(1)}% success rate`
        );
        break;
      }
    }
  ...
  for (let threshold = 0.85; threshold >= 0.50; threshold -= 0.01) {
    const candidates = sorted.filter(
      p => p.predicted_confidence >= threshold && p.predicted_confidence < autonomousThreshold
    );
    ...
    if (candidates.length >= minSampleSize) {
      ...
      if (successRate >= 0.85) {
        ...
        justification.push(
          `Assisted threshold set to ${assistedThreshold} based on ${candidates.length} predictions with ${(successRate * 100).toFixed(1)}% success rate`
        );
        break;
      }
    }
  ...
  const adjustment_needed =
    Math.abs(autonomousThreshold - currentThresholds.autonomous) > 0.02 ||
