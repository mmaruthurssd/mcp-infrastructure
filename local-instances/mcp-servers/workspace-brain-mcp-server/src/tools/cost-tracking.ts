...
import { readFileSync, appendFileSync, existsSync, mkdirSync, writeFileSync, readdirSync } from "fs";
 ...
 */
function calculateAPICost(inputTokens: number, outputTokens: number): number {
 ...
 */
function calculateHumanCostSaved(timeHours: number): number {
 ...
 */
function calculateROI(apiCost: number, humanCostSaved: number) {
 ...
 */
function loadWorkflowCosts(workspacePath: string, filters?: {
  startDate?: Date;
  endDate?: Date;
  workflowName?: string;
}): WorkflowCost[] {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const cost: WorkflowCost = JSON.parse(content);
      ...
      if (filters?.startDate && costDate < filters.startDate) continue;
      if (filters?.endDate && costDate > filters.endDate) continue;
 ...
 */
function saveMonthlySum(workspacePath: string, yearMonth: string, summary: any) {
...
${costs
  ...
  .map(
    (c) =>
      `| ${c.workflow_name} | $${c.api_cost.toFixed(4)} | ${c.time_saved_hours.toFixed(1)}h | $${c.net_roi.toFixed(2)} | ${c.roi_ratio.toFixed(2)}x |`
  )
  .join("\n")}
...
${mostExpensive
  .map(
    (w) =>
      `| ${w.name} | ${w.count} | $${w.total_cost.toFixed(2)} | $${(w.total_cost / w.count).toFixed(4)} | ${(w.success_rate * 100).toFixed(0)}% |`
  )
  .join("\n")}
...
${highestROI
  .map(
    (w) =>
      `| ${w.name} | ${w.count} | $${w.total_roi.toFixed(2)} | ${w.avg_roi_ratio.toFixed(2)}x | Scale up |`
  )
  .join("\n")}
...
${lowestROI
  .map(
    (w) =>
      `| ${w.name} | ${w.count} | $${w.total_roi.toFixed(2)} | ${w.avg_roi_ratio.toFixed(2)}x | ${w.total_roi < 0 ? "Optimize or disable" : "Monitor"} |`
  )
  .join("\n")}
 ...
 */
export async function createROIDashboard(
  args: CreateROIDashboardArgs,
  workspacePath: string
): Promise<{ content: Array<{ type: string; text: string }> }> {
  try {
    // Current month
