 ...
 */
export async function archiveOldData(args, brainPath) {
    try {
 ...
 */
export async function exportData(args, brainPath) {
    try {
            ...
            case "learning": {
                ...
                for (const filename of learningFiles) {
                    ...
                    if (existsSync(filePath)) {
                        ...
                        if (data.patterns)
                            exportData.push(...data.patterns);
                        if (data.solutions)
                            exportData.push(...data.solutions);
                        if (data.checks)
                            exportData.push(...data.checks);
                    }
                }
                break;
            }
 ...
 */
export async function getStorageStats(args, brainPath) {
    try {
        ...
        if (oldestEvent)
            stats.oldest_event = oldestEvent;
        if (newestEvent)
            stats.newest_event = newestEvent;
        return {
            content: [
                {
                    type: "text",
