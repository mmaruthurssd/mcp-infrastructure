 ...
 */
export async function recordPattern(args, brainPath) {
    try {
 ...
 */
export async function getSimilarPatterns(args, brainPath) {
    try {
        const patternsFile = join(brainPath, "learning", "issue-patterns.json");
        // Read patterns
        let patternsData = {
        ...
        const scoredPatterns = patterns.map(pattern => {
            ...
            queryWords.forEach((qWord) => {
                if (searchWords.some((sWord) => sWord.includes(qWord) || qWord.includes(sWord))) {
                    matches++;
                }
            });
 ...
 */
export async function getPreventiveChecks(args, brainPath) {
    try {
 ...
 */
function getDefaultPreventiveChecks(context) {
    const allChecks = [
        {
            id: "pre-commit-creds",
