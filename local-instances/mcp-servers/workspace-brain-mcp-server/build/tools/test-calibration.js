...
import { validateConfidencePrediction, getCalibrationReport, getConfidenceTrends, adjustConfidenceThresholds } from './calibration.js';
 ...
 */
function generateSyntheticPredictions(count) {
    const predictions = [];
    const issueTypes = ['broken', 'missing', 'improvement'];
    ...
    for (let i = 0; i < count; i++) {
        ...
        const resolution_time_minutes = actual_outcome === 'success' ? 10 + Math.random() * 30 :
 ...
 */
function addTimestampsToPredictions(predictions) {
    const now = new Date();
...
async function testCalibrationSystem() {
    ...
    const reportResult = await getCalibrationReport({ time_range: 'all' }, WORKSPACE_BRAIN_PATH);
    ...
    report.by_bucket.forEach((bucket) => {
    ...
    const trendsResult = await getConfidenceTrends({ weeks_back: 12 }, WORKSPACE_BRAIN_PATH);
    ...
    if (trends.poorly_calibrated_types.length > 0) {
        ...
        trends.poorly_calibrated_types.forEach((type) => {
    ...
    const thresholdResult = await adjustConfidenceThresholds({ target_success_rate: 0.95, min_sample_size: 10 }, WORKSPACE_BRAIN_PATH);
    ...
    thresholds.justification.forEach((reason) => {
        console.log(`    - ${reason}`);
    });
    console.log();
...
//# sourceMappingURL=test-calibration.js.map