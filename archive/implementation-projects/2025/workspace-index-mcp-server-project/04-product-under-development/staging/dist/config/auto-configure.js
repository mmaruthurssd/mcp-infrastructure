 ...
 */
export async function detectWorkspaceStructure(workspaceRoot) {
    const result = {
        documentationRoots: [],
 ...
 */
export async function autoConfigureWorkspace(workspaceRoot, templateConfigPath) {
    ...
    configContent = configContent.replace(/"{{AUTO_DETECT:workspace_name}}"/g, JSON.stringify(detection.workspaceName));
    configContent = configContent.replace(/"{{AUTO_DETECT:workspace_root}}"/g, JSON.stringify(workspaceRoot));
    configContent = configContent.replace(/"{{AUTO_DETECT:workspace_type}}"/g, JSON.stringify(detection.workspaceType));
    // Replace workspace structure paths
    configContent = configContent.replace(/"{{AUTO_DETECT:development\/}}"/g, JSON.stringify(detection.developmentDir || 'development'));
    configContent = configContent.replace(/"{{AUTO_DETECT:local-instances\/}}"/g, JSON.stringify(detection.productionDir || 'local-instances'));
    configContent = configContent.replace(/"{{AUTO_DETECT:archive\/}}"/g, JSON.stringify(detection.archiveDir || 'archive'));
    configContent = configContent.replace(/"{{AUTO_DETECT:development\/frameworks\/}}"/g, JSON.stringify(detection.frameworksDir || 'development/frameworks'));
    configContent = configContent.replace(/"{{AUTO_DETECT:templates-and-patterns\/}}"/g, JSON.stringify(detection.templatesDir || 'templates-and-patterns'));
    ...
    configContent = configContent.replace(/"{{AUTO_DETECT:documentation_files}}"/g, docRootsJson);
    // Replace maturity threshold
    configContent = configContent.replace(/"{{AUTO_DETECT:maturity_threshold}}"/g, detection.maturityThreshold.toString());
    // Replace timestamp
    const timestamp = new Date().toISOString();
    configContent = configContent.replace(/"{{AUTO_DETECT:timestamp}}"/g, JSON.stringify(timestamp));
    // Replace maturity level
    configContent = configContent.replace(/"{{AUTO_DETECT:maturity}}"/g, JSON.stringify(detection.maturityLevel));
 ...
 */
export async function saveCustomizedConfig(config, outputPath) {
