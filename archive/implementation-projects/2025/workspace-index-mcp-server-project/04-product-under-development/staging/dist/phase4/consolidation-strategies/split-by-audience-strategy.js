 ...
 */
export class SplitByAudienceStrategy {
    adapter;
    constructor(adapter) {
    ...
    async analyze(files, overlaps) {
        ...
        const audiences = await Promise.all(files.map(async (file) => ({
            file,
        ...
        try {
            for (const modification of plan.filesToModify) {
                ...
                if (modification.action === 'add-references' && modification.referenceTarget) {
                    ...
                    const result = await this.addCrossReferences(filePath, modification.referenceTarget.split(', '), dryRun);
                    if (result.modified) {
    ...
    async detectAudience(filePath) {
        try {
            ...
            const technicalScore = technicalIndicators.filter(indicator => lowerContent.includes(indicator)).length;
            const nonTechnicalScore = nonTechnicalIndicators.filter(indicator => lowerContent.includes(indicator)).length;
     ...
     * Add cross-references to related documentation
     */
    async addCrossReferences(filePath, relatedFiles, dryRun) {
        try {
            const content = await fs.readFile(filePath, 'utf-8');
